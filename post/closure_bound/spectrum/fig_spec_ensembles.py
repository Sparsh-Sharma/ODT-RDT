...
def fig_op(op):
    ...
    kref = np.array([50.0, 5000.0])
    AMP = 9.5e-2 * op["Etot_last"].max()      # slope guide, lifted clear
    ...
    a.text(700, 8.51 * AMP * (1000 / 200.0) ** (-5 / 3),
...
def fig_aniso(on, off):
    ...
    specs = (("c2", r"upwash $\bar\kappa_2$", "-", "+", "0.82"),
             ("c1", r"streamwise $\bar\kappa_1$", (0, (5, 2)), "x", "0.9"))
    for key, lab, ls, mk, band in specs:
        ...
        ax.plot(t, r, color="k", ls=ls, lw=1.2, marker=mk, ms=5.0,
                mfc="k", mew=1.1, markevery=5, label=lab, zorder=3)
