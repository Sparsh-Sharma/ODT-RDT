...
%DIF DEL Alan_1909.tex          Sun Sep 20 11:13:06 2026
%DIF ADD main_v4_overleaf.tex   Sun Sep 20 11:35:29 2026
...
\DIFdelbegin \DIFdel{The strain-coupled formulation of the
first part modifies the model's }\emph{\DIFdel{continuous}} %DIFAUXCMD
\DIFdel{advancement---the
...
part in turn modifies the }\emph{\DIFdel{discrete}} %DIFAUXCMD
\DIFdel{process, giving relaxation its
own clock.
...
% \end{enumerate}
\end{itemize}
\DIFaddbegin \DIFadd{The strain-coupled formulation of the first part modifies the model's
}\emph{\DIFadd{continuous}} \DIFadd{advancement, yielding the generalized momentum
...
}\emph{\DIFadd{discrete}} \DIFadd{process, giving relaxation its own clock.
...
coincide. \DIFaddbegin \DIFadd{A prime marks the fluctuating part of a field where a mean
...
\DIFdelbegin \DIFdel{The production term of the fluctuation momentum equation, $-A_{ij}u_j$,
is a pointwise linear combination of the local velocity components and is
therefore exactly representable on the line. }\DIFdelend Between eddy events the molecular evolution is replaced by
\begin{equation}
  \partial_t u_i
...
uniform mean gradient: molecular diffusion as in standard ODT\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend the
exact production $-A_{ij}u_j$\DIFdelbegin \DIFdel{(whose
second moment is the production
}\DIFdelend \DIFaddbegin \DIFadd{---a pointwise linear combination of the
local velocity components, exactly representable on the line, whose
contribution to the Reynolds-stress equation is }\DIFaddend $\mathcal{P}_{ij}$ of
\eqref{eq:rstransport}\DIFdelbegin \DIFdel{), and, }\DIFdelend \DIFaddbegin \DIFadd{---and, }\DIFaddend in place of the fluctuating pressure
...
The \DIFaddbegin \DIFadd{strain }\DIFaddend forcing preserves the structural properties on which the model
...
event exactly as in the standard model, so they hold verbatim. \DIFdelbegin \DIFdel{For a
mean gradient}\DIFdelend \DIFaddbegin \DIFadd{Both
$A_{ij}$, the imposed mean gradient, and $B_{ij}$, formed from the
line-averaged Reynolds stress (\S\,\ref{sec:line-realisation}), are
}\DIFaddend uniform along the line\DIFdelbegin \DIFdel{the operator
}\DIFdelend \DIFaddbegin \DIFadd{, so }\DIFaddend $\mathcal{A}_{ij}\equiv-A_{ij}+B_{ij}$ \DIFdelbegin \DIFdel{acts pointwise, so }\DIFdelend \DIFaddbegin \DIFadd{is a
constant matrix; }\DIFaddend over a non-diffusive sub-step \DIFaddbegin \DIFadd{it advances the profile
by }\DIFaddend the \DIFdelbegin \DIFdel{profile is advanced by a }\DIFdelend $y$-independent \DIFdelbegin \DIFdel{matrix,
}\DIFdelend \DIFaddbegin \DIFadd{map
}\DIFaddend $u_i(y,t)=[\exp\!\int_{t_0}^{t}\mathcal{A}\,\mathrm{d}t']_{ij}\,
u_j(y,t_0)$\DIFdelbegin \DIFdel{---continuous profiles therefore }\DIFdelend \DIFaddbegin \DIFadd{, so continuous profiles }\DIFaddend map
...
$B_{ij}$ is not. Constructing it requires the \DIFdelbegin \DIFdel{second-moment }\DIFdelend \DIFaddbegin \DIFadd{Reynolds-stress }\DIFaddend and
...
\begin{equation}
  ...
  \Pi_{ij}=\Big\langle \tfrac{p'}{\rho}
            \big(\partial_j u_i\DIFdelbegin \DIFdel{'}\DIFdelend +\partial_i u_j\DIFdelbegin \DIFdel{'}\DIFdelend \big)\Big\rangle,
...
\begin{equation}
  \frac{1}{\rho}\nabla^2 p'
  = \DIFdelbegin %DIFDELCMD < \underbrace{-2\,A_{lm}\,\partial_l u_m'}%%%
\DIFdelend \DIFaddbegin \underbrace{-2\,A_{lm}\,\partial_l u_m}\DIFaddend _{\text{rapid}}
    \;-\;
    \DIFdelbegin %DIFDELCMD < \underbrace{\partial_l\partial_m\!\left(u_l' u_m'
%DIFDELCMD <      -\langle u_l' u_m'\rangle\right)}%%%
\DIFdelend \DIFaddbegin \underbrace{\partial_l\partial_m\!\left(u_l u_m
     -\langle u_l u_m\rangle\right)}\DIFaddend _{\text{slow}} ,
  \label{eq:poisson}
\end{equation}
...
\subsubsection{\DIFdelbegin \DIFdel{Isotropy-based closure}\DIFdelend \DIFaddbegin \DIFadd{Closure of the pressure contribution}\DIFaddend }\label{sec:iso-closure}
...
\begin{equation}
  \Pi\DIFdelbegin \DIFdel{^{(\mathrm r),\mathrm{iso}}}\DIFdelend \DIFaddbegin \DIFadd{^{(\mathrm r),\mathrm{IP}}}\DIFaddend _{ij}
  = \tfrac{4}{5}\,k_t\,S_{ij}
...
\DIFaddend For anisotropic turbulence $\mathcal{R}_{ijmn}$ must be modelled as a
tensor function of $R_{ij}$; the general representation linear in the
anisotropy is \DIFdelbegin \DIFdel{that of \mbox{%DIFAUXCMD
\citet{LaunderReeceRodi1975}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the quasi-isotropic model of \mbox{%DIFAUXCMD
\citet{LaunderReeceRodi1975}
}\hskip0pt%DIFAUXCMD
(LRR),
}\begin{equation}
  \DIFadd{\Pi^{(\mathrm r),\mathrm{QI}}_{ij} = C_2\,k_t S_{ij}
  + C_3\,k_t\big(b_{ik}S_{jk}+b_{jk}S_{ik}
  -\tfrac{2}{3}b_{mn}S_{mn}\delta_{ij}\big)
  + C_4\,k_t\big(b_{ik}W_{jk}+b_{jk}W_{ik}\big),
  \label{eq:lrr}
}\end{equation}
\DIFadd{with anisotropy $b_{ij}=R_{ij}/2k_t-\tfrac13\delta_{ij}$, mean rotation
$W_{ij}=\tfrac12(A_{ij}-A_{ji})$ (zero for the irrotational strains
considered), and $C_2=4/5$}\DIFaddend , \DIFdelbegin \DIFdel{to which the construction reduces in form.
We take the isotropic closure \eqref{eq:iso-closure}---the
isotropisation-of-production }\DIFdelend \DIFaddbegin \DIFadd{$C_3=7/4$, $C_4=131/100$; the value
...
\eqref{eq:crow} }\DIFaddend (IP) \DIFdelbegin \DIFdel{model---as the starting point, and
assess the anisotropic correction in }\DIFdelend \DIFaddbegin \DIFadd{is retained only for the onset consistency check
of }\DIFaddend \S\,\DIFdelbegin \DIFdel{\ref{sec:verification}}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:onset}; every numerically simulated result in the paper
uses the anisotropic closure \eqref{eq:lrr} (QI)}\DIFaddend .
...
$\mathcal{A}_{ij}=-A_{ij}+B_{ij}$, where the production is exact and $B_{ij}$ is
...
unchanged. Step (a), the continuous advancement, carries the strain-driven physics
(the exact production and the rapid pressure--strain); step (b), the
discrete events, carries the turbulence-driven physics (the cascade and
...
of \S\,\ref{sec:limits}. Standard ODT is recovered exactly when
$A_{ij}=0$.
...
\begin{equation}
  ...
  = \mathcal{P}_{ij}+\Pi\DIFdelbegin \DIFdel{^{(\mathrm r),\mathrm{iso}}}\DIFdelend \DIFaddbegin \DIFadd{^{(\mathrm r),\mathrm{IP}}}\DIFaddend _{ij}
...
plane strain\DIFaddbegin \DIFadd{~\eqref{eq:plane-strain}}\DIFaddend , onset rates $\mp\tfrac{8}{15}k_ta$ for the stretched and
compressed components with the neutral component unchanged, and, under
axisymmetric strain\DIFaddbegin \DIFadd{~\eqref{eq:axi-strain}}\DIFaddend , $+\tfrac{4}{15}k_t\gamma$ for each lateral
...
than of the construction. \DIFdelbegin \DIFdel{We integrate the
model's second-moment (}\DIFdelend \DIFaddbegin \DIFadd{For each canonical strain we integrate the
}\DIFaddend Reynolds-stress \DIFdelbegin \DIFdel{) equationalong
each canonical strain with two closures: the isotropisation-of-production
closure }\DIFdelend \DIFaddbegin \DIFadd{equation~\eqref{eq:rstransport} forward in time, closing
the rapid pressure--strain two ways---the isotropy-based form
}\DIFaddend \eqref{eq:crow} (IP\DIFdelbegin \DIFdel{, with $C_2=3/5$}\DIFdelend ) and the \DIFdelbegin \DIFdel{quasi-isotropic model of \mbox{%DIFAUXCMD
\citet{LaunderReeceRodi1975} }\hskip0pt%DIFAUXCMD
(LRR-QI, abbreviated
LRR) ,
}\begin{displaymath}
  \DIFdel{\Pi^{(\mathrm r)}_{ij} = C_2\,k_t S_{ij}
  + C_3\,k_t\big(b_{ik}S_{jk}+b_{jk}S_{ik}
  -\tfrac{2}{3}b_{mn}S_{mn}\delta_{ij}\big)
  + C_4\,k_t\big(b_{ik}W_{jk}+b_{jk}W_{ik}\big),
  %DIFDELCMD < \label{eq:lrr}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdel{with anisotropy $b_{ij}=R_{ij}/2k_t-\tfrac13\delta_{ij}$, mean rotation
$W_{ij}=\tfrac12(A_{ij}-A_{ji})$ (zero for the irrotational strains
...
the exact $\tfrac45k_tS_{ij}$ of \eqref{eq:crow}, so LRR shares the constant-free onset slope}\DIFdelend \DIFaddbegin \DIFadd{anisotropic closure \eqref{eq:lrr} (QI, or
LRR) of \S\,\ref{sec:iso-closure}---to obtain the component energies
$R_{ij}(e)$ as functions of the accumulated total strain
$e=\int_0^{t}S\,\mathrm{d}t'$, plotted below}\DIFaddend . The benchmark is the exact rapid-distortion
...
\begin{figure}
  ...
  SC-ODT \DIFdelbeginFL \DIFdelFL{second-moment }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Reynolds-stress }\DIFaddendFL equation\DIFaddbeginFL \DIFaddFL{~\eqref{eq:rstransport} }\DIFaddendFL with the LRR closure \eqref{eq:lrr} and the IP
...
\begin{equation}
  ...
  = \frac{\rho}{2\pi}\,A_{kl}\!\int
    ...
    \DIFdelbegin \DIFdel{\frac{\partial u'_l}{\partial x'_k}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\partial u_l}{\partial x'_k}}\DIFaddend (\boldsymbol{x}')\,
    \mathrm{d}\boldsymbol{x}' .
  \label{eq:prapid}
\end{equation}
The rapid pressure--strain correlation that redistributes energy among the
Reynolds-stress components is
\DIFdelbegin \DIFdel{$\Pi^{(\mathrm r)}_{ij}
   = \overline{(p'_{\mathrm r}/\rho)\,(\partial u'_i/\partial x_j
     + \partial u'_j/\partial x_i)}$}\DIFdelend \DIFaddbegin \DIFadd{$\Pi^{(\mathrm r)}_{ij}
   = \overline{(p'_{\mathrm r}/\rho)\,(\partial u_i/\partial x_j
     + \partial u_j/\partial x_i)}$}\DIFaddend .
Substituting \eqref{eq:prapid} and using homogeneity to pass to the
velocity spectrum tensor
$\Phi_{mn}(\boldsymbol{\kappa})$
(the Fourier transform of \DIFdelbegin \DIFdel{$\overline{u'_m(\boldsymbol{x})\,
u'_n(\boldsymbol{x}+\boldsymbol{r})}$}\DIFdelend \DIFaddbegin \DIFadd{$\overline{u_m(\boldsymbol{x})\,
u_n(\boldsymbol{x}+\boldsymbol{r})}$}\DIFaddend ),
...
same observables, the \DIFaddbegin \DIFadd{preliminary }\DIFaddend strain-coupled model\DIFaddbegin \DIFadd{, SC-ODT, }\DIFaddend carries
...
sub-scales. The intervention broadly \DIFdelbegin \DIFdel{reproduced }\DIFdelend \DIFaddbegin \DIFadd{reproduces }\DIFaddend the phenomenology: depth that reaches the measured scales is necessary and
