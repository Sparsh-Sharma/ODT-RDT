...
\usepackage{xcolor}\usepackage[margin=1in]{geometry}\usepackage{natbib}
\usepackage{tikz}
...
\begin{abstract}
The rapid distortion of turbulence by a mean strain arises in flows from
wind-tunnel contractions to the stagnation regions of lifting surfaces.
Its economical prediction is limited by a closure problem: the rapid
pressure--strain term is a non-local three-dimensional spectral integral,
while affordable models carry at most one-dimensional or single-point
information. We present a strain-coupled formulation of one-dimensional
turbulence (ODT). As its analytical core we examine what a single line of
turbulence data determines about the rapid pressure--strain term. The
obstruction to a single-line closure is stated exactly, sharp bounds on
the rapid term are obtained from line data alone, and the assumption of
axisymmetry about the line is measured rather than imposed: it is exact
for axisymmetric distortion and its error under plane strain is quantified
as a function of accumulated strain. A strained-box direct numerical
simulation with an exact rapid-distortion companion supplies an independent
spectral benchmark. The comparison shows that the correct linear reference
for line-projected spectra is broadband rather than a rigid spectral
translation, and localises the model's remaining deficiency in its
wavenumber-uniform rapid kinematics; the deficiency is quantified by a
distance-from-linear-theory envelope as a function of strain rapidity.
Hierarchical application of the model's energy-redistributing kernel to
eddy sub-scales provides scale-local relaxation at the scales of the
imbalance. Decoupling the relaxation from the eddy clock removes its
deep-strain fade while a size cap preserves the one-point physics. In a
five-way comparison against the DNS and exact linear theory the mechanism
moves the model's spectral allocation of anisotropy towards the DNS at
moderate strain rapidity; the saturation at high rapidity is measured and
stated. The computed distorted spectrum is then supplied to Amiet's
leading-edge-noise theory in place of the frozen von~K\'arm\'an inflow of
standard practice. It lowers the predicted radiated level by $5$--$20$\,dB
at the energy-containing frequencies for total strains $0.5$--$2$, both in
a geometry-free kernel ratio and in an absolute prediction. This is the
magnitude and sign of the term omitted by frozen-spectrum practice, and
it is opposite to the surface-blocking correction with which it must be
combined.
\end{abstract}
...
its own coordinate. Reconciling the inherently three-dimensional, non-local pressure physics
of rapid distortion with the one-dimensional, local structure of ODT is the
modelling problem addressed here.

The paper reports six results, each framed so that it can be tested.
First, a strain-coupled formulation of ODT (SC-ODT)
...
constant (\S\,\ref{sec:verification}). Second, the linear reference
appropriate to the model's observables: exact RDT projected onto a line is
broadband rather than a rigid spectral translation, so the reference against
which any line model must be judged already differs from the model's own
kinematics at the linear level (\S\,\ref{sec:spec-linref}). Third, an
independent spectral benchmark consisting of a strained-box direct numerical
simulation together with exact-RDT companion calculations, reduced to the
same one-dimensional observables. A three-way comparison localises the
model's remaining deficiency in its wavenumber-uniform rapid kinematics and
supplies a distance-from-linear-theory envelope as a function of strain
rapidity $Sk_t/\varepsilon$ (\S\,\ref{sec:box-dns}). Fourth, a treatment of
what a single line can determine about the rapid pressure--strain term (a
non-local three-dimensional object). The obstruction is stated exactly,
sharp bounds are obtained from line data alone, and the axisymmetry
hypothesis is measured rather than assumed: it is exact for axisymmetric
distortion and carries a quantified error under plane strain
(\S\,\ref{sec:gate}). Fifth, the measured limits of the model and a
mechanism that moves them (\S\,\ref{sec:limits}). A transmission diagnostic
shows the strain-induced anisotropy crossing two triplet-map generations
(a factor of nine in scale) undiminished; a controlled family of
interventions rules out acceptance gating and map reweighting as remedies.
Hierarchical application of the energy-redistributing kernel to eddy
sub-scales supplies scale-local relaxation at the scales of the imbalance.
The mechanism's deep-strain fade is traced to its event-locked timing and
is removed by an independent, size-capped relaxation clock, leaving a
single measured allocation bound. Sixth, the consequence for the
motivating application (\S\,\ref{sec:lenoise}): the computed distorted
spectrum is supplied to Amiet's leading-edge-noise theory in place of the
frozen inflow of standard practice, both as a ratio from which aerofoil
geometry and observer cancel and as an absolute prediction at the reference
operating point. The calculation quantifies the $5$--$20$\,dB term omitted
by prescribed-spectrum practice; the assembled chain is compared with four
published experiments and lies within $1$--$2$\,dB rms where the facilities
are cleanest. Throughout we show where the model agrees with reference data
and, where it does not, by how much and for what reason.

The paper falls into three parts. The first concerns the model and what a
line can know: \S\,\ref{sec:formulation} reviews standard ODT and derives
the strain-coupled formulation; \S\,\ref{sec:verification} verifies it
against exact rapid-distortion solutions and validates it at the moment
level against the DNS of \citet{LeeReynolds1985}; \S\,\ref{sec:val-spectrum}
constructs the linear reference, examines the emergent distorted spectrum
and its operating point, and carries out the strained-box DNS benchmark;
\S\,\ref{sec:gate} treats the single-line closure question. The second
part, \S\,\ref{sec:limits}, measures the model's limits and establishes
scale-local relaxation---timed by its own clock---as the mechanism that
moves them. The third part, \S\,\ref{sec:lenoise}, applies the computed
distortion to the leading-edge-noise input.
...
\citet{ChenMeneveauKatz2006} and the complete intervention family behind
...
\begin{figure}
  ...
  spectral peak (dashed) against the rigid-translation law (grey dotted):
...
For the application that motivated the work the model supplies a
distorted, anisotropic, scale-resolved upwash spectrum, evolved rather than
prescribed, at a cost two orders of magnitude below that of a strained DNS
ensemble. Section~\ref{sec:lenoise} carries the spectrum through Amiet's
theory to the radiated level. The assembled chain reproduces four published far-field experiments to
