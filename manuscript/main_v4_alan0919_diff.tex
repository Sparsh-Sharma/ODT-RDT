...
%DIF DEL C:/Users/shar_sp/AppData/Local/Temp/alan_base.tex   Sat Sep 19 18:06:49 2026
%DIF ADD main_v4_overleaf.tex                                Sat Sep 19 18:06:06 2026
...
\affiliation{
\aff{1}German Aerospace Center (DLR), 38108 Braunschweig, Germany
...
\begin{keywords}
one-dimensional turbulence; leading-edge noise; turbulence modelling
\end{keywords}
...
they reproduce a prescribed spectrum rather than evolving one. A model that predicts the distorted spectrum, including the nonlinear
relaxation that linear transforms omit, would supply an element that is
at present missing---here and wherever distorted turbulence feeds a
...
of mapping events \DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{interleaved with a continuous, one-dimensional
specialisation of viscous transport; at each event }\DIFaddend a kernel mechanism
redistributes energy among components, emulating pressure scrambling
\citep{WunschKerstein2001,Kerstein2001}.
\DIFaddbegin 

\DIFaddend Generalised to variable density
...
inexpensive generator of scale-resolved turbulence statistics. But standard ODT
...
The paper reports six results:
\begin{itemize}
\item A strain-coupled formulation of ODT (SC-ODT)
...
kinematics as a dilatation of the domain. The construction preserves the
model's structural invariants and, under the stated closure, reproduces
the rapid-distortion evolution of the component energies with no adjustable
constant (\S\,\ref{sec:verification}).

\item Validation against the strained-turbulence experiment of
\citet{ChenMeneveauKatz2006} (CMK), summarised in \S\,\ref{sec:cmk}
with details in the supplementary material (\S\,S1).

\item The linear reference appropriate to the model's observables: exact
RDT projected onto a line is broadband rather than a rigid spectral
translation, so the reference against which any line model must be judged
already differs from the model's own kinematics at the linear level
(\S\,\ref{sec:spec-linref}).

\item An independent spectral benchmark consisting of a strained-box direct
numerical simulation together with exact-RDT companion calculations,
reduced to the same one-dimensional observables. A three-way comparison
localises the model's remaining deficiency in its wavenumber-uniform rapid
kinematics and supplies a distance-from-linear-theory envelope as a
function of strain rapidity $Sk_t/\varepsilon$ (\S\,\ref{sec:box-dns}).

\item A treatment of what a single line can determine about the rapid
pressure--strain term (a non-local three-dimensional object). The
obstruction is stated exactly, sharp bounds are obtained from line data
alone, and the axisymmetry hypothesis is measured rather than assumed: it
is exact for axisymmetric distortion and carries a quantified error under
plane strain (\S\,\ref{sec:gate}).

\item The measured limits of the model and a mechanism that \DIFdelbegin \DIFdel{moves }\DIFdelend \DIFaddbegin \DIFadd{extends }\DIFaddend them
(\S\,\ref{sec:limits}). A transmission diagnostic shows the strain-induced
anisotropy crossing two triplet-map generations (a factor of nine in
scale) undiminished; a controlled family of interventions rules out
acceptance gating and map reweighting as remedies. Hierarchical application
of the energy-redistributing kernel to eddy sub-scales supplies scale-local
relaxation at the scales of the imbalance. The mechanism's deep-strain fade
is traced to its event-locked timing and is removed by an independent,
size-capped relaxation clock, leaving a single measured allocation bound.

\item The consequence for the motivating application
...
and as an absolute prediction at the reference operating point. The
calculation quantifies the $5$--$20$\,dB term omitted by prescribed-spectrum
practice; the assembled chain is compared with four published experiments
and lies within $1$--$2$\,dB rms where the facilities are cleanest.
\end{itemize}
Throughout we show where the model agrees with reference data and, where it
does not, by how much and for what reason.


The paper falls into three parts\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{. The strain-coupled formulation of the
first part modifies the model's }\emph{\DIFadd{continuous}} \DIFadd{advancement---the
production and rapid pressure--strain of \eqref{eq:cont-evolution}---and
leaves the discrete eddy events unchanged; the mechanism of the second
part in turn modifies the }\emph{\DIFadd{discrete}} \DIFadd{process, giving relaxation its
own clock.
}\DIFaddend \begin{itemize}
\item The model and what a line can know: \S\,\ref{sec:formulation}
...
\item \S\,\ref{sec:limits} measures the model's limits and establishes
scale-local relaxation---timed by its own clock---as the mechanism that
\DIFdelbegin \DIFdel{moves }\DIFdelend \DIFaddbegin \DIFadd{extends }\DIFaddend them.

\item \S\,\ref{sec:lenoise} applies the computed distortion as a
correction to the leading-edge-noise input.
% \end{enumerate}
\end{itemize}
Section~\ref{sec:conclusions} concludes. Supplementary material contains
the validation against the strained-turbulence experiment of
\citet{ChenMeneveauKatz2006} (CMK) and the complete set of intervention categories, labelled by category.variant (Appendix~\ref{app:numerics}).
...
on a one-dimensional line; that incompatibility is the focus of this section.
...
are captured by the discrete eddy events of the standard model, reviewed first.
...
field $u_i(y,t)$ about this mean, so that the total velocity is
$U_i+u_i$ with $\overline{u_i}=0$; $R_{ij}\equiv\overline{u_iu_j}$ is the
...
\end{scope}
\end{tikzpicture}
  \caption{The model at a glance.
(\textit{a}) Instantaneous eddy event on $[y_0,y_0+l]$: the triplet map compresses and triplicates the local field; the additive kernel $K(y)$ redistributes energy among components, both conserving momentum and total energy.
(\textit{b}) Eddy events (Poisson), the vertical bars marking the spatial range of each event, punctuating the continuous diffusion and mean-strain forcing (grey arrows).
(\textit{c}) Advancement cycle at a state-dependent rate; the $k^{-5/3}$ cascade emerges as an outcome of repeated sampling, consistent with inertial-range phenomenology.}
...
(figure~\ref{fig:odt-schematic})\DIFdelbegin \DIFdel{: between }\DIFdelend \DIFaddbegin \DIFadd{. Between }\DIFaddend events the fields
obey ordinary one-dimensional diffusion,
\DIFdelbegin \DIFdel{$(\partial_t-\nu\,\partial_y^2)\,u_i=0$, }\DIFdelend \DIFaddbegin \begin{equation}
  \DIFadd{(\partial_t-\nu\,\partial_y^2)\,u_i=0,
  \label{eq:odt-diffusion}
}\end{equation}
\DIFaddend with kinematic viscosity $\nu$; and a stochastic sequence of
instantaneous \emph{eddy events} represents the effect of individual
turbulent overturns.
...
$\lambda(y_0,l;t)$ follows the standard eddy time-scale construction
\DIFdelbegin \DIFdel{applied to }\DIFdelend \DIFaddbegin \DIFadd{governed by }\DIFaddend the $Q_i$, with a rate constant, a viscous cutoff and a
large-eddy suppression parameter as the only tunable constants; the
inertial-range cascade and the $k^{-5/3}$ spectrum are \emph{outcomes}
of the sampling, not inputs
\citep{Kerstein1999,StephensLignell2021}. One diagnostic caveat of
\citet{Kerstein2001} is respected throughout: the line velocity does
not advect fluid, so diagonal component spectra and variances are read
from the fields while shear stresses are read from eddy-induced
transport.

\subsection{\DIFdelbegin \DIFdel{Rapid }\DIFdelend \DIFaddbegin \DIFadd{The strain-coupled line equation }\DIFaddend and \DIFdelbegin \DIFdel{slow pressure--strain, and linear rapid
distortion}\DIFdelend \DIFaddbegin \DIFadd{its structural
admissibility}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{sec:rapid-slow}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:strain-forcing}
\DIFaddend 

\DIFdelbegin \DIFdel{The Reynolds-stress transport equation for a fluctuating field about a
mean with constant gradient }\DIFdelend \DIFaddbegin \DIFadd{Standard ODT carries no mean strain: between events the field obeys the
pure diffusion \eqref{eq:odt-diffusion}, and the eddy events supply only
the turbulence-driven cascade and return to isotropy. The strain-coupled
formulation generalises the }\emph{\DIFadd{continuous}} \DIFadd{advancement to carry the
distortion by an imposed mean velocity gradient }\DIFaddend $A_{ij}$\DIFdelbegin \DIFdel{reads, in homogeneous turbulence,
}\begin{displaymath}
  \DIFdel{\frac{\mathrm{d}R_{ij}}{\mathrm{d}t}
  ...
  %DIFDELCMD < \label{eq:rstransport}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{, which is
prescribed---the input to the problem---rather than evolved. Two
irrotational homogeneous strains are studied, both admitting exact
rapid-distortion solutions and isolating the physics of interest.
}\emph{\DIFadd{Plane strain}}\DIFadd{,
}\begin{equation}
  \DIFadd{\mathsf{A}=\operatorname{diag}(a,-a,0),\qquad a>0,
  \label{eq:plane-strain}
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{with $\varepsilon_{ij}$ the dissipation tensor \mbox{%DIFAUXCMD
\citep{Pope2000}}\hskip0pt%DIFAUXCMD
. The
fluctuating pressure satisfies a Poisson equation whose source splits
into a part linear in the
mean gradient and a part quadratic in the
fluctuations \mbox{%DIFAUXCMD
\citep{Pope2000}}\hskip0pt%DIFAUXCMD
, }\begin{displaymath}
  \DIFdel{\frac{1}{\rho}\nabla^2 p'
...
\DIFdel{and the pressure--strain correlation inherits the decomposition
$\Pi_{ij}=\Pi^{(\mathrm r)}_{ij}+\Pi^{(\mathrm s)}_{ij}$ into a
}\emph{\DIFdel{rapid}} %DIFAUXCMD
\DIFdel{part driven by the mean strain and a }\emph{\DIFdel{slow}} %DIFAUXCMD
\DIFdel{part driven
by the turbulence alone. The slow part relaxes the Reynolds stress
towards isotropy---the mechanism that the
kernel
\eqref{eq:redistribution}--\eqref{eq:ci} reproduces in model form. The
rapid part is linear in $A_{lm}$ and is determined by the velocity spectrum tensor $\Phi_{ij}(\boldsymbol{\kappa})$; the standard model
represents neither $\mathcal{P}_{ij}$ nor $\Pi^{(\mathrm r)}_{ij}$. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{When the strain outruns the eddy turnover---the rapid limit---the
response is computed exactly by linear theory
\mbox{%DIFAUXCMD
\citep{BatchelorProudman1954,Hunt1973,HuntCarruthers1990}}\hskip0pt%DIFAUXCMD
.
Writing the fluctuation as Fourier modes that follow the mean flow, each mode of
time-dependent wavevector $\boldsymbol{\kappa}(t)$ and amplitude
$\hat{u}_i$ evolves, in the
inviscid rapid limit, according to
}\begin{displaymath}
  \DIFdel{\frac{\mathrm{d}\kappa_i}{\mathrm{d}t}=-A_{ji}\,\kappa_j,
  \qquad
  \frac{\mathrm{d}\hat{u}_i}{\mathrm{d}t}
  =\left(\frac{2\,\kappa_i \kappa_m}{\kappa^2}-\delta_{im}\right)
   A_{mn}\,\hat{u}_n .
  %DIFDELCMD < \label{eq:rdt}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{is the strain of a two-dimensional stagnation region: with $x_1$
...
leading-edge noise. }\emph{\DIFadd{Axisymmetric strain}}\DIFadd{,
}\begin{equation}
  \DIFadd{\mathsf{A}=\operatorname{diag}(-\tfrac12\gamma,-\tfrac12\gamma,\gamma),
  \qquad \gamma>0,
  \label{eq:axi-strain}
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{The bracketed operator combines the }\DIFdelend \DIFaddbegin \DIFadd{is the strain of a contraction or a three-dimensional stagnation flow and
...
and the line is taken along $x_2$. The modelling task is to add to the
continuous advancement the two processes through which $A_{ij}$
acts---an exact }\DIFaddend production \DIFdelbegin \DIFdel{$-A_{im}\hat u_m$ with the rapid pressure response, the solenoidal projection
$P_{ij}=\delta_{ij}-\kappa_i\kappa_j/\kappa^2$ applied to
the strained
...
pressure response is }\emph{\DIFdel{non-local}} %DIFAUXCMD
\DIFdel{and intrinsically
}\emph{\DIFdel{three-dimensional}}%DIFAUXCMD
\DIFdel{: it requires all of
$\kappa_1,\kappa_2,\kappa_3$, whereas a single line resolves only the
wavenumber along its own coordinate,
so \eqref{eq:rdt} cannot be
evaluated on the line as written. Second, the rapid/slow decomposition
furnishes the organising principle of the formulation: the slow part is already carried by the kernel, and it remains to supply the
production
}\DIFdelend and a \DIFdelbegin \DIFdel{rapid redistribution consistent with \eqref{eq:rdt}, expressed in
quantities available on the line}\DIFdelend \DIFaddbegin \DIFadd{modelled rapid pressure--strain---while
leaving the eddy events unchanged}\DIFaddend .

\DIFdelbegin \subsection{\DIFdel{The strain-coupled line equation and its structural
admissibility}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec:strain-forcing}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The production term of the fluctuation momentum equation, $-A_{ij}u_j$,
...
\end{equation}
\DIFdelbegin \DIFdel{in which }\DIFdelend \DIFaddbegin \DIFadd{the line form of the fluctuation momentum equation for a spatially
uniform mean gradient: molecular diffusion as in standard ODT, the
exact production }\DIFaddend $-A_{ij}u_j$ \DIFaddbegin \DIFadd{(whose second moment }\DIFaddend is the production
\DIFdelbegin \DIFdel{and $B_{ij}u_j$ is the
}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{P}_{ij}$ of \eqref{eq:rstransport}), and, in place of the
fluctuating pressure gradient, the modelled }\DIFaddend rapid pressure--strain
operator \DIFaddbegin \DIFadd{$B_{ij}u_j$ }\DIFaddend constructed in \S\,\ref{sec:rapid-kernel}\DIFdelbegin \DIFdel{; the
}\DIFdelend \DIFaddbegin \DIFadd{. The
}\DIFaddend eddy events \eqref{eq:event}--\eqref{eq:ci} are retained without
...
The forcing preserves the structural properties on which the model
rests; we record the argument as a consistency check. Measure
preservation, scale locality and the kernel identities
\eqref{eq:kernel-identities} are geometric properties of $f$ and $K$
on a fixed domain, and the operator splitting applies the discrete
event exactly as in the standard model, so they hold verbatim. For a
mean gradient uniform along the line the operator
$\mathcal{A}_{ij}\equiv-A_{ij}+B_{ij}$ acts \DIFdelbegin \DIFdel{pointwise---the
}\DIFdelend \DIFaddbegin \DIFadd{pointwise, so over a
}\DIFaddend non-diffusive sub-step \DIFdelbegin \DIFdel{solution
$u_i(y,t)=[\exp\!\int_{t_0}^{t}\mathcal{A}\,\mathrm{d}t']_{ij}\,
u_j(y,t_0)$ is }\DIFdelend \DIFaddbegin \DIFadd{the profile is advanced by }\DIFaddend a $y$-independent
\DIFdelbegin \DIFdel{matrix---so continuous profiles }\DIFdelend \DIFaddbegin \DIFadd{matrix,
$u_i(y,t)=[\exp\!\int_{t_0}^{t}\mathcal{A}\,\mathrm{d}t']_{ij}\,
u_j(y,t_0)$---continuous profiles therefore }\DIFaddend map
to continuous profiles while the molecular term is smoothing; and the
forcing displaces no fluid along $y$, leaving the domain measure
untouched (the deliberate domain straining is the separate operation
of \S\,\ref{sec:domain-strain}).
...
\DIFaddbegin \subsection{\DIFadd{Rapid and slow pressure--strain, and linear rapid
distortion}}\label{sec:rapid-slow}

\DIFadd{The production in \eqref{eq:cont-evolution} is exact; the rapid operator
$B_{ij}$ is not. Constructing it requires the second-moment and
rapid-distortion physics it must reproduce, set out here. The
Reynolds-stress transport equation for a fluctuating field about a
mean with constant gradient $A_{ij}$ reads, in homogeneous turbulence,
}\begin{equation}
  \DIFadd{\frac{\mathrm{d}R_{ij}}{\mathrm{d}t}
  = \mathcal{P}_{ij} + \Pi_{ij} - \varepsilon_{ij},
  \qquad
  \mathcal{P}_{ij}=-A_{ik}R_{kj}-A_{jk}R_{ki},
  \quad
...
\DIFadd{with $\varepsilon_{ij}$ the dissipation tensor \mbox{%DIFAUXCMD
\citep{Pope2000}}\hskip0pt%DIFAUXCMD
. The
fluctuating pressure satisfies a Poisson equation whose source splits
...
\DIFadd{and the pressure--strain correlation inherits the decomposition
$\Pi_{ij}=\Pi^{(\mathrm r)}_{ij}+\Pi^{(\mathrm s)}_{ij}$ into a
}\emph{\DIFadd{rapid}} \DIFadd{part driven by the mean strain and a }\emph{\DIFadd{slow}} \DIFadd{part driven
by the turbulence alone. The slow part relaxes the Reynolds stress
...
\DIFadd{When the strain outruns the eddy turnover---the rapid limit---the
response is computed exactly by linear theory
\mbox{%DIFAUXCMD
\citep{BatchelorProudman1954,Hunt1973,HuntCarruthers1990}}\hskip0pt%DIFAUXCMD
. Writing the
fluctuation as Fourier modes with wavevectors convected by the mean
flow, each mode of time-dependent wavevector $\boldsymbol{\kappa}(t)$ and amplitude
$\hat{u}_i$ evolves, in the inviscid rapid limit, according to
}\begin{equation}
  \DIFadd{\frac{\mathrm{d}\kappa_i}{\mathrm{d}t}=-A_{ji}\,\kappa_j,
  \qquad
  \frac{\mathrm{d}\hat{u}_i}{\mathrm{d}t}
  =\left(\frac{2\,\kappa_i \kappa_m}{\kappa^2}-\delta_{im}\right)
   A_{mn}\,\hat{u}_n .
...
\DIFadd{The bracketed operator combines the production $-A_{im}\hat u_m$ with the
rapid pressure response, the solenoidal projection
$P_{ij}=\delta_{ij}-\kappa_i\kappa_j/\kappa^2$ applied to the strained
field; the factor of two is fixed by continuity, since only that
coefficient makes $\mathrm{d}(\kappa_i\hat u_i)/\mathrm{d}t$ vanish under
\eqref{eq:rdt}. Two features are central to what follows. First, the
pressure response is }\emph{\DIFadd{non-local}} \DIFadd{and intrinsically
}\emph{\DIFadd{three-dimensional}}\DIFadd{: it requires all of
$\kappa_1,\kappa_2,\kappa_3$, whereas a single line resolves only the
...
already carried by the eddy kernel and the production by
\eqref{eq:cont-evolution}; what remains is to construct the rapid
redistribution $B_{ij}$ consistent with \eqref{eq:rdt} from quantities
available on the line---the task of \S\,\ref{sec:rapid-kernel}.
}

\DIFaddend \subsection{The rapid redistribution operator}\label{sec:rapid-kernel}
...
\subsubsection{Isotropy-based closure}\label{sec:iso-closure}
...
\begin{equation}
  ...
  \label{eq:crow}
\end{equation}
\DIFaddbegin \DIFadd{which is }\DIFaddend the exact response of initially isotropic turbulence to a weak rapid
...
We take the isotropic closure \eqref{eq:iso-closure}---the
isotropisation-of-production (IP) model---as the starting point, and
assess the anisotropic correction in \S\,\ref{sec:verification}.
...
\begin{equation}
  ...
  \label{eq:iso-B}
\end{equation}
$B_{ij}$ is evaluated from the running Reynolds stress \DIFdelbegin \DIFdel{and applied
continuously, paced by the mean strain, exactly as the productionis. This is deliberate: the rapid pressure--strain is by construction the
}\DIFdelend \DIFaddbegin \DIFadd{at each sub-step
and enters the continuous advancement \eqref{eq:cont-evolution}
alongside the production. Phenomenologically it represents the }\DIFaddend part of
the redistribution driven by the mean gradient\DIFdelbegin \DIFdel{and therefore acts
}\DIFdelend \DIFaddbegin \DIFadd{, which proceeds
continuously }\DIFaddend at the strain rate, in contrast to the slow \DIFdelbegin \DIFdel{redistribution }\DIFdelend \DIFaddbegin \DIFadd{return to
isotropy }\DIFaddend that the eddy kernel supplies at the eddy rate. Strain-driven effects (production and
...
domain}\DIFdelbegin \DIFdel{---integrate
}\DIFdelend \DIFaddbegin \DIFadd{---time advance
}\DIFaddend $\partial_t u_i=\nu\partial_y^2u_i+\mathcal{A}_{ij}u_j$ with
$\mathcal{A}_{ij}=-A_{ij}+B_{ij}$, \DIFdelbegin \DIFdel{the production }\DIFdelend \DIFaddbegin \DIFadd{where the production is }\DIFaddend exact and $B_{ij}$ \DIFaddbegin \DIFadd{is
}\DIFaddend given by \eqref{eq:lyapunov} from the running Reynolds stress and the
spectral \DIFdelbegin \DIFdel{closure (the IP form \eqref{eq:iso-B}; the LRR alternative }\DIFdelend \DIFaddbegin \DIFadd{closure---the LRR closure }\DIFaddend of \S\,\ref{sec:verification}\DIFdelbegin \DIFdel{is used for the strained results), while }\DIFdelend \DIFaddbegin \DIFadd{, with
the isotropic form \eqref{eq:iso-B} its onset limit---while }\DIFaddend dilating the domain
...
unchanged. Step (a)\DIFdelbegin \DIFdel{carries the mean-strain physics
}\DIFdelend \DIFaddbegin \DIFadd{, the continuous advancement, carries the strain-driven physics
(the exact production and the rapid pressure--strain)}\DIFaddend ; step (b)\DIFaddbegin \DIFadd{, the
discrete events, }\DIFaddend carries the turbulence-driven physics \DIFdelbegin \DIFdel{; standard }\DIFdelend \DIFaddbegin \DIFadd{(the cascade and
the slow return to isotropy). The strain coupling thus modifies only the
continuous advancement and leaves the discrete events unchanged; the
discrete process is itself modified later, by the relaxation mechanism
of \S\,\ref{sec:limits}. Standard }\DIFaddend ODT is recovered exactly when
$A_{ij}=0$.
...
analytically and by the numerical solution below. \emph{Validation} asks whether the
...
One point of identification should to be kept in view throughout this section
...
figures refer to ``SC-ODT'' they mean this formulation; ``standard ODT''
is its strain-off ($A_{ij}=0$) limit, and the relaxation mechanism of
\S\,\ref{sec:limits} (``clock-relaxed ODT'') does not enter until
Part~II.

\subsection{\DIFdelbegin \DIFdel{Canonical strains and the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend exact onset rate}\label{sec:onset}

\DIFdelbegin \DIFdel{Two irrotational homogeneous strains admit }\DIFdelend \DIFaddbegin \DIFadd{The two canonical strains of \S\,\ref{sec:strain-forcing}---plane strain
\eqref{eq:plane-strain} and axisymmetric strain
\eqref{eq:axi-strain}---admit }\DIFaddend exact rapid-distortion solutions and
isolate the physics of interest.
\DIFdelbegin \emph{\DIFdel{Plane strain}}%DIFAUXCMD
\DIFdel{,
}\begin{displaymath}
  \DIFdel{\mathsf{A}=\operatorname{diag}(a,-a,0),\qquad a>0,
  %DIFDELCMD < \label{eq:plane-strain}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdel{is the strain of a two-dimensional stagnation region: with $x_1$
tangential to the surface, $x_2$ normal to it and $x_3$ spanwise, the
component compressed by the strain is the wall-normal (upwash) component
$u_2$, whose amplification is the pre-impact distortion relevant to
leading-edge noise. }\emph{\DIFdel{Axisymmetric strain}}%DIFAUXCMD
\DIFdel{,
}\begin{displaymath}
  \DIFdel{\mathsf{A}=\operatorname{diag}(-\tfrac12\gamma,-\tfrac12\gamma,\gamma),
  \qquad \gamma>0,
  %DIFDELCMD < \label{eq:axi-strain}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdel{is the strain of a contraction or a three-dimensional stagnation flow and
provides an independent test in which two components are amplified and
one suppressed. For both, $\mathsf{A}$ is symmetric, so the
principal-axis alignment assumed in \S\,\ref{sec:domain-strain} holds;
...
no adjustable constant. This is a
closed-form prediction the model must satisfy identically. Evaluated for the two canonical strains it gives, under
...
than of the construction. We integrate the model\DIFdelbegin \DIFdel{moment }\DIFdelend \DIFaddbegin \DIFadd{'s second-moment (Reynolds-stress) }\DIFaddend equation along
each canonical strain with two closures: the isotropisation-of-production
closure \DIFdelbegin \DIFdel{\eqref{eq:iso-closure} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:crow} }\DIFaddend (IP, with $C_2=3/5$) and the
quasi-isotropic model of \citet{LaunderReeceRodi1975} (LRR-QI, abbreviated
LRR),
\begin{equation}
...
evolved modal covariance. This integration is independent of the \DIFdelbegin \DIFdel{moment
models and
}\DIFdelend \DIFaddbegin \DIFadd{IP and
LRR closures and }\DIFaddend reproduces the onset slope \eqref{eq:onset-rate} to within
...
\begin{figure}
  ...
  SC-ODT \DIFdelbeginFL \DIFdelFL{moment }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{second-moment }\DIFaddendFL equation with the LRR closure \eqref{eq:lrr} and the IP
  closure \DIFdelbeginFL \DIFdelFL{\eqref{eq:iso-closure}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\eqref{eq:crow}}\DIFaddendFL . Line style denotes the source (solid
...
\begin{figure}
  ...
  %Production without the rapid term overstates the onset amplification rate by $\tfrac52$ and forces almost all the energy into the upwash component; the rapid term restores the physical, bounded amplification.
  }
...
The preceding checks verify the \emph{formulation}; the
\emph{implementation}---in which the operator is not prescribed but
reconstructed at every substep (line Reynolds stress from the resolved
field, closure, Lyapunov solve \eqref{eq:lyapunov}, pointwise
application of $\mathcal{A}_{ij}$)---is verified in the supplementary
material (\S\,S5, figure~S5). Run in the rapid-distortion limit from a
statistically isotropic resolved initial field, the solver lies on the
LRR moment trajectory over $0\le e\le4$ to within $10^{-4}$ in each
component fraction and $0.3\%$ in the kinetic-energy amplification, so
the per-substep chain is correct at every state of anisotropy;
agreement with the \emph{exact} solution would indicate a fault, since
with events and diffusion suppressed the line carries no information
beyond the single-point statistics the closure itself uses. The test
requires the resolved spectral initialisation and strain-resolved
timestep bound recorded in Appendix~\ref{app:numerics}. For
definiteness, \emph{every strained computation in the remainder of the
paper uses the LRR closure}, whose error against the exact solution is
quantified in \S\,\ref{sec:finite-strain} at every strain reached; the
sensitivity of the spectral conclusions to this closure choice is not
estimated but measured, through the exact-RDT comparisons of
...
uncertainty rather than leaving it implicit.
...
the CMK comparison; higher-Reynolds-number strained
...
they reach ($c\le1.65$, where $c=\exp\int S\,\mathrm{d}t$ is the deformation ratio, and $\ln c$ is the total strain $e$). A direct simulation of the
CMK straining cycle
...
The
comparison is based on the $c$ dependence of the anisotropy $b_{ij}$ as a function of the deformation
ratio. %$c=\exp\int S\,\mathrm{d}t$ (so $\ln c$ is the total strain $e$ of the present notation), 
computed from the single-point component
...
$\exp(A_{22}t_f)$, where $t_f$ is the total straining time, to better than $0.3\%$, confirming the kinematic
...
\begin{figure}
  ...
  (lines; ensemble mean over $1000$ realisations, \DIFdelbeginFL \DIFdelFL{standard-error band}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{standard error within
  the line width}\DIFaddendFL ) against the DNS of \citet{LeeReynolds1985} (open symbols, digitised
  from their figure~5.4). As noted in \S\,\ref{sec:level1a}, here and
  henceforth all strained computations use the LRR closure. Line style and marker denote the component
...
\begin{figure}
  ...
  strain: SC-ODT (solid, $1000$ realisations, \DIFaddbeginFL \DIFaddFL{grey }\DIFaddendFL standard-error band)
  against exact linear rapid distortion (long-dashed,
  wavevector-ensemble integration), the IP (dotted) and \DIFdelbeginFL \DIFdelFL{LRR-QI
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{LRR
  }\DIFaddendFL (short-dashed) closures, and the DNS of \citet{LeeReynolds1985} (open
...
benchmark that addresses the model-versus-itself comparison---a
...
combination induces a rigid translation of the
line spectrum towards higher wavenumber that transfer no energy between scales. We emphasise at the outset that this
...
near-equilibrium limit applies and where improvement over linear theory is
...
which permits parametric sweeps over inflow condition.
...
\begin{figure}
  ...
  spectral peak (dashed) against the rigid-translation law (grey dotted), with the geometric $\pm\sigma$ ensemble spread shaded:
  the broadband distortion that linear theory cannot represent, at the
  operating point and with the noise averaged out.}
  \label{fig:spec-op}
\end{figure}

\subsection{Validation against the CMK strained-turbulence experiment}\label{sec:cmk}
...
benchmark at laboratory Reynolds number is provided by the CMK plane-strain
experiment, whose straining phase
...
residual attributed to the moment closure, not the implementation (\S\,\ref{sec:level1a}). With eddies and viscosity active the component
...
\begin{table}
  ...
  state. The band below $0.75\,\kappa_c$ rests on a few box modes so those results are
  indicative only.}
  \label{tab:threeway}
\end{table}

The structure of the tabulated results is the finding. Exact linear theory concentrates
...
splitting $\phi_{11}/\phi_{33}-1$, a combination of the two transverse
components just shown, carries the same reading (not plotted): exact RDT
...
\S\,\ref{sec:gate}, whose introduction is thereby motivated by a measured
deficit rather than by its association with an identified physical process.
...
\begin{figure}
  ...
  $\Delta b_{33}$. SC-ODT ($\blacksquare$, $1024$ realisations)
  against exact projected RDT ($\blacklozenge$) and DNS ($128^3$,
  $\bullet$, four realisations). The SC-ODT curves are relatively flat in every
...
\begin{figure}
  ...
  exact-RDT-distorted von K\'arm\'an fit, against total strain, for low
  and high onset strain-rate parameters (medians over $1024$
...
\eqref{eq:poisson} and \eqref{eq:Mijkl} are exact; no modelling has entered. The
...
\DIFdelbegin \DIFdel{ODT evolves a velocity
vector }\DIFdelend \DIFaddbegin \DIFadd{In the present formulation the line carries the fluctuation velocity
}\DIFaddend $\boldsymbol{u}'(y)$ \DIFdelbegin \DIFdel{on a }\DIFdelend \DIFaddbegin \DIFadd{about a prescribed uniform mean gradient---standard
ODT can also evolve a mean profile---on a }\DIFaddend single line aligned
...
\begin{equation}
  ...
  \label{eq:phi1d}
\end{equation}
obtained by integrating $\Phi_{mn}$ over the two
...
case of assumption A1 (see Appendix~\ref{app:twoscalar}), and uniaxial contraction preserves axisymmetry about the
...
\begin{equation}
  ...
  \label{eq:angular}
\end{equation}
with $\kappa^2=\kappa_2^2+\kappa_\perp^2$, and the weights mixing a resolved
and an unresolved index (e.g.\ $\kappa_1\kappa_2/\kappa^2$) averaging to zero.
Substitution of \eqref{eq:angular} into \eqref{eq:Mijkl} removes the azimuthal
...
themselves. 
Equations \eqref{eq:Piclosed}--\eqref{eq:g3} are therefore the collapsed form on which any single-line statement must be built. What the line data actually determine—and how sharply—is established next.
...
suite confirming tracelessness, the isotropic angular averages
...
\begin{table}
  \centering\small
  \begin{tabular}{lccc}
  constraint level & $\mathcal I_1$ & $\mathcal I_2$ & $\mathcal I_3$\\[2pt]
  line data $+$ solenoidality $+$ realisability & 0.77 & 1.28 & 0.86\\
  $+$ log-Lipschitz slope cap ($\lambda=4$) & \multicolumn{3}{c}{$<5\%$ narrower}\\
  $+$ polarisation cap $|c|\le\gamma a$, $\gamma=0$ (i.e.\ $c=0$) & 0.34 & 0.58 & 0.24\\
  \end{tabular}
  \caption{Kinematic indeterminacy
...
\end{table}
Two features of the Table~\ref{tab:indeterminacy} matter. Smoothness assumptions buy almost
...
As showm in Appenix~\ref{app:forward}, the representation also supplies two \emph{free falsifiers} that the line
...
An error estimate quadratic in the anisotropy ($O(b^2)$), which the symmetric structure of the discarded
harmonic suggests, is superseded by this measurement.
%An $O(b^2)$ error estimate, which the symmetric structure of the discarded harmonic suggests, is superseded by this measurement. 
Second, the
...
\begin{figure}
  ...
  splitting $\phi_{11}/\phi_{33}-1$ for each geometry, and the upwash
  anisotropy $b_{22}$ (grey dash-dot, indistinguishable between
  geometries)---the azimuthal structure that A1 assumes away is what differs,
...
and sets in at rapid strain. LRR\DIFdelbegin \DIFdel{-QI}\DIFdelend  , whose spanwise component is carried by
...
\eqref{eq:g1}--\eqref{eq:g3}; the moment reduction omits it. This
...
justifies the closure used in the simulations and fixes the prefactor in an explicit convention (the coefficient of \eqref{eq:crow-spectral} and the remark following it).
...
omits---and the effects of both omissions are now measured: the wavenumber dependence
...
agree to leading order, and the terms in which they differ are exactly
...
\item Independently of A1's validity DNS, line data bound the rapid term
  sharply; the bound is valid---and verified to bracket the exact
...
\item Rapid and slow redistribution are disjoint: verified by the
  strain-off null and by the rapid-limit invariance of every tested kernel-energy
  allocation rule.
...
anisotropy is allocated uniformly across scales while linear theory and
...
The whole investigation is carried out at a single, moderate turbulence
...
$\kappa_p$ with the resolved decade of smaller scales below it, and two
...
measurement bands in wavenumber $\kappa_2$: a low-wavenumber,
energy-containing band $\kappa_2\in[0.6,2]\,\kappa_p$ (subscript
$\mathrm{lo}$) and a high-wavenumber, fine-scale band
$[6,16]\,\kappa_p$ (subscript $\mathrm{hi}$)---the latter roughly two
triplet-map generations \emph{finer}, since each map compresses scales
by a factor of three. On each band the
...
SC-ODT (C0.2; supplementary material, figure~S3, black; also the black curve of figure~\ref{fig:clock}) exhibits the deficiency of
...
The mechanism was located by a controlled search: every intervention
category---acceptance gating (C1), map-image geometry (C2), sub-scale
relaxation kernels (C3), concurrent relaxation (C4)---was run at $1024$
paired-seed realisations against SC-ODT (C0.2), and each
variant's knob, measured fine-scale effect and remarks are tabulated in
the supplementary material (\S\,S2, table~S1 and figure~S3; the
category.variant labels C0--C5 are defined with the reference
configurations in Appendix~\ref{app:numerics}). The search returns one
positive result and one shared limitation, documented case by case in
the supplementary material and summarised here. Gating and map-image
geometry are null where it matters: the gates modulate the eddy rate,
not the scale allocation, and removing up to $97$ per cent of
small-eddy events leaves the fine-scale anisotropy and energy
unchanged---the measurement band is fed directly by large-eddy
maps, so no acceptance-gating scheme can control transmission at this
Reynolds number. What works is relaxation applied \emph{at the scale
of the imbalance}: recursing the energy-equalising kernel procedure of
\S\,\ref{sec:formulation} into the map images, at the depths that
reach the measured band ($\ell/9$ and $\ell/27$; C3.2, C3.3), produces
the first statistically significant fine-scale isotropisation of the
search ($\mathcal A_{\rm hi}$ from $1.43$ to $1.27$ at $e=1$) with
band energies, the upwash energy fraction and the intermittency tails
untouched. Every eddy-locked refinement of that success---depth tied
to eddy size (C3.4), per-event iteration (C3.5), delocalisation and
stacking (C4)---then shares a single limitation: the fine-scale gains
fade beyond $e\approx3$. Depth sets \emph{where} the relaxation acts
...
combination---more depth, more dosage, more events, other
locations---prevents the fade. The remaining suspect is the clock
itself: relaxation acts at the eddy rate while the strain resupplies
the imbalance continuously. This motivates the final intervention.

\subsection{Decoupling the relaxation clock}\label{sec:clock}

The one feature the eddy-locked formulations do not modify is \emph{when} relaxation acts. We therefore give it its own clock. Whereas the eddy
events are sampled from the state-dependent rate density
$\lambda(y_0,\ell;t)$ of \S\,\ref{sec:odt-review}, the relaxation-only
events form an independent Poisson process with the prescribed rate
density
\begin{equation}
  \lambda_{\mathrm c}(y_0,\ell)
  = \frac{R\,f(\ell)}{L-\ell},
  \qquad
  f(\ell)
  ...
  \label{eq:clock}
\end{equation}
with $R$ the total event rate, the placement $y_0$ uniform over the
...
energy, exactly and per event. The mean rate of the relaxation events is matched to the SC-ODT mean eddy rate, $R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the mean number of eddy events over an SC-ODT realisation of duration
$T$, so the comparison against the eddy-locked variants is
dose-matched by construction.

The result (figure~\ref{fig:clock}) decides the question. With the
clock, the fine-scale imbalance is reduced significantly at every
strain and the effect grows rather than fades: the paired contrasts
reach $-0.30$ in the low band and remain significant in the high band
at $e=3.9$. A constant-rate event sequence keeps pace precisely where the
eddy rate could not, confirming that event-locked timing---not any
per-event property---was the deep-strain limiter.

However, an event sequence that draws from the full candidate density (capped
only at the model's default largest scale $L_{\max}$) is scale-blind
and over-corrects. It also suppresses the one-point anisotropy that
...
Restricting the event sequence to sub-band interval sizes---replacing $f$ in
\eqref{eq:clock} by its restriction to $\ell\le\ell^*$, renormalised
(over-cap draws are resampled), where $\ell^*$ separates the two
measurement bands---resolves the tension. The size-capped clock (the
``clock-relaxed ODT'' of the figures) holds the fine scales at every
strain with no fade ($\mathcal{A}_{\rm hi}=1.30$--$1.44$ against SC-ODT's $1.63$--$1.94$), with contrasts that remain significant
throughout and are strongest at $e=3$ (figure~\ref{fig:clock}\textit{a}).
The one-point trajectory is essentially untouched in the
moderate-strain regime ($u_2^2/2k_t$ within $0.005$ of SC-ODT at $e=1$) and only mildly reduced at the deepest strain
(figure~\ref{fig:clock}\textit{b}).

The mean rate of relaxation events is a single control: quadrupling
the rate extends the upward trend in figure~\ref{fig:clock}\textit{a}
at a proportionate cost in figure~\ref{fig:clock}\textit{b}.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{Figures/fig_clock.pdf}
  \caption{The independent relaxation clock (C5) against the eddy-locked categories. (\textit{a}) Fine-scale-band imbalance $\mathcal A_{\rm hi}$
  against total strain: the eddy-locked variants fade beyond
  $e\approx3$; the clock holds at every strain. (\textit{b}) The
  price: upwash energy fraction. The comparison is the SC-ODT (C0.2) trajectory---the moment-level behaviour validated in
  \S\,\ref{sec:lr-validation}---which the uncapped clock suppresses;
  the size cap at matched rate preserves it, and the $4\times$ rate (C5.3) reduces it in proportion. Medians
  over $1024$ paired-seed realisations; the clock rate is matched to the SC-ODT eddy rate.}
  \label{fig:clock}
\end{figure}

\subsection{The measured missing ingredient}\label{sec:limits-endpoint}

The deficiency measured by the three-way comparison---anisotropy
allocated uniformly over wavenumber---is confirmed by the transmission
...
undiminished. It is unfixable by gating or map reweighting, and is
...
own clock}, with the event-size distribution capped at
$\ell\le\ell^{*}$ (the clock-relaxed ODT of \S\,\ref{sec:clock}),
which alone survives deep strain. One limit remains, and it is
...
The comparison that summarises this part is
...
deficiency of SC-ODT---its uniform allocation of a correct
...
wavenumber- and angle-dependent rapid operator of
\eqref{eq:Piclosed}--\eqref{eq:g3}, identified above as the next step.
...
$(A_0,\kappa_e)$ per strain, fitted to the median line spectra of the SC-ODT
$1024$-realisation ensembles at $Sk_t/\varepsilon\approx0.4$ and
$\approx8$. The fit is to SC-ODT; the clock-relaxed inflow of
\S\,\ref{sec:clock}, fitted identically, is carried alongside from
figure~\ref{fig:expval} onward and the two are compared at
figure~\ref{fig:amiet-rapid}. The
line data pin the resolved-wavenumber structure; the
...
rather than from the fit. The chain is laid out step by step in the
...
\begin{figure}
  ...
  dominates the transit time; $+0.5$ to $+1.9$\,dB at the rapid rate, where
...
are the finding. Each panel plots, against reduced streamwise
...
contribution---deepening the drop at the slow rate (panel~\textit{a})
and slightly lifting it at the rapid rate (panel~\textit{b}). At matched
...
response~ \citep{Sears1941}. The spectra enter in physical units through two matching
...
spectrum shown as an independent check of the unit bridge. The figure
...
Both this figure and the kernel ratio of figure~\ref{fig:lenoise}
carry the SC-ODT inflow as the representative computed spectrum---the
clock-relaxed variant gives an indistinguishable far-field prediction
here, for the reason quantified at figure~\ref{fig:amiet-rapid}---so
the two computed inflows are carried separately only from the
validation of figure~\ref{fig:expval} onward, where the comparison is
the point.
...
\begin{figure}
  ...
  checks ($\pm0.5$\,dB). The effective strain is a geometric estimate (potential-flow stagnation-line accumulation truncated at the eddy scale, varied over $\Lambda/4$--$\Lambda$); the resulting spread in predicted level is at most ${\approx}1$\,dB, comparable to the digitisation uncertainty, and is not shown.}
  \label{fig:expval}
\end{figure}
...
effective strain is negligible and both computed inflows correctly predict \emph{no} correction to the frozen spectrum, while for the NACA0012 they
lower the prediction towards the data at every speed
(bias $+5.7\!\to\!+5.3$\,dB at $40$\,m\,s$^{-1}$,
$+1.8\!\to\!+1.4$\,dB at $60$, $+5.1\!\to\!+4.7$\,dB at $90$), by the
modest amount the geometric effective strain implies; the shift
...
the finite-thickness effect that \citet{PatersonAmiet1976} themselves
identified (breakdown near $U/f t_A\approx1$) and
that \citet{Gershfeld2004} modelled, of a piece with the blocking mechanism
excluded from the present scope; it is insensitive to the choice
of inflow spectrum.
...
\begin{figure}
  ...
  frozen von K\'arm\'an inflow of standard practice. Black with markers (circles:
  SC-ODT; triangles: clock-relaxed ODT): the two computed inflows
  (fitted representations, measured ensemble levels), each with a grey band
  giving its measured spectral rms distance from the RDT family mapped
  to decibels; the bands overlap sufficiently that the two inflows are indistinguishable at this level.}
...
evidence permits. Each panel shows absolute SPL against frequency at
...
cannot be told apart at this level. At total strains of $1$--$2$ the frozen-spectrum
...
\S\,\ref{sec:envelope}---so an evolved inflow outperforms a prescribed one
at the strains where the correction matters, whichever
...
rotors~\citep{MajumdarPeake1998}, where the streamtube contraction supplies the strain and the
blade response the fine-scale weighting---and is left as a falsifiable future target.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{Figures/fig_frozen_cost.pdf}
  \caption{The cost of the frozen-inflow assumption against accumulated
...
Figure~\ref{fig:frozen-cost} assembles the two regimes on a single
...
The scope of this statement is set out as carefully as its content.
...
partially compensating mechanism outside the scope of the present line
model; the calculation also replaces the strain history along
individual streamlines by the canonical constant-rate distortion. What the calculation establishes
...
Figure~\ref{fig:amiet-abs} carries the kernel result through to an
absolute far-field spectrum in physical units, the frozen baseline
coinciding with the standard Amiet--von K\'arm\'an prediction; what
...
present model supplies the distorted-inflow input, with its
...
precisely in the wavenumbers with the greatest influence on the response.
...
identically, and is verified computationally to the level of the closure
...
single-point closure discards, not to the line. The laboratory
counterpart is the plane-strain experiment of
\citet{ChenMeneveauKatz2006}: the model reproduces the qualitative
...
rapid-distortion theory, projected onto a line, rescales the
...
integrated anisotropy but allocates it uniformly over wavenumber, while
...
and grows with strain rapidity, a measured artefact that
...
valid up to $e\approx0.25$--$0.5$ under plane strain and at all
...
The measured limits point to a specific missing ingredient that is thereupon identified and supplied. The strain-induced fine-scale
...
move it is supplementary relaxation applied at the scale of the imbalance---the
model's own energy-equalising kernel, applied hierarchically to eddy
sub-scales. The intervention broadly reproduced the phenomenology: depth that reaches the measured scales is necessary and
...
removed by giving the relaxation its own clock---an independent event sequence
...
closure is the natural future continuation, now motivated by a measured
deficit and pointed at a measured target.

For the application that motivated the work, the model delivers what
the prediction chain otherwise lacks: a distorted, anisotropic, scale-resolved
...
reference at application strains, an evolved inflow outperforms the frozen
...
surface-blocking correction that a complete prediction must
likewise include; predictions inherit a spectral-shape uncertainty of order
the distance envelope at the operating point's strain and rapidity,
a falsifiable model prediction.
...
two, supported on the plane normal to $\boldsymbol{\kappa}$. Upon invoking axisymmetry about $\boldsymbol{e}_2$, mirror symmetry, and zero helicity, here denoted Assumption~A1, it is fixed by two scalar functions of $(\kappa_2,\kappa_\perp)$
...
\subsection{Model configurations and intervention labels}

The reference configurations and every intervention variant of the
measured-limits campaign (\S\,\ref{sec:limits}; supplementary material
\S\,S2) carry a \emph{category.variant} label, used with that label
throughout. The reference configurations are C0.1, standard ODT (the
strain-off limit $A=0$), and C0.2, SC-ODT (the strain-coupled model the
interventions modify); the intervention categories C1--C5 are listed in
table~\ref{tab:cv}. The three headline models of the main text are C0.1
(standard ODT), C0.2 (SC-ODT) and C5.2 (clock-relaxed ODT), and the
five-way comparison of figure~\ref{fig:fourway} is C0.1, C0.2 and C5.2
with exact RDT and DNS. The size cap of the clock-relaxed variant
(C5.2) is $\ell^*=0.05$ on the unit domain, the scale separating the
two transmission-diagnostic bands of \S\,\ref{sec:limits}.

\begin{table}
  \centering\small
  \begin{tabular}{p{0.08\textwidth}p{0.82\textwidth}}
  label & configuration / intervention (knob)\\[2pt]
  \hline
  C0.1 & standard ODT (strain-off limit, $A=0$)\\
  C0.2 & SC-ODT (strain-coupled; the configuration the interventions modify)\\[2pt]
  \hline
  \multicolumn{2}{@{}l}{\emph{C1\quad acceptance gating}}\\[2pt]
  C1.1 & gate on post-event anisotropy ($45$--$98\%$ rejection)\\
  C1.2 & gate on sub-band eddies only ($42\%$, $97\%$ rejection)\\[2pt]
  \hline
  \multicolumn{2}{@{}l}{\emph{C2\quad map-image geometry}}\\[2pt]
  C2.1 & unequal triplet-map images (middle image of relative size $\tfrac23$)\\[2pt]
  \hline
  \multicolumn{2}{@{}l}{\emph{C3\quad sub-scale relaxation kernels (depth)}}\\[2pt]
  C3.1 & one level ($\ell/3$)\\
  C3.2 & two levels ($\ell/9$)\\
  C3.3 & three levels ($\ell/27$)\\
  C3.4 & recursion depth tied to eddy size\\
  C3.5 & two-level pass iterated $3\times$ per event\\[2pt]
  \hline
  \multicolumn{2}{@{}l}{\emph{C4\quad concurrent relaxation}}\\[2pt]
  C4.1 & $N=1$ randomly placed interval per event\\
  C4.2 & $N=3$ randomly placed intervals per event\\
  C4.3 & concurrent $+$ depth: adaptive hierarchy in each of $N=3$ intervals (stacked)\\[2pt]
  \hline
  \multicolumn{2}{@{}l}{\emph{C5\quad independent relaxation clock}}\\[2pt]
  C5.1 & uncapped (rate matched to the eddy rate)\\
  C5.2 & size-capped $\ell\le\ell^*$, adaptive depth inside --- the clock-relaxed ODT\\
  C5.3 & size-capped clock at $4\times$ the eddy rate\\[2pt]
  \hline
  \end{tabular}
  \caption{Category.variant labels for the reference configurations and
  the intervention categories. Measured fine-scale effects and remarks
  are in supplementary table~S1; the four-panel diagnostic is
  figure~S3.}
  \label{tab:cv}
\end{table}
...
$3\times10^{-6}$ on a $16000$-cell grid at the operating point of \S\,\ref{sec:spec-op}. The strained line spectrum ensembles carried into the leading-edge-noise chain of Part~III and the relaxation-clock spectra (supplementary material \S\,S3) are $1024$ realisations per case at each rapidity $Sk_t/\varepsilon\approx0.4$ and $\approx8$. The resolved scale separation of the $\nu=10^{-5}$ ensembles, used for the CMK comparison of supplementary \S\,S1, is $\mathcal L/\eta\approx250$.
