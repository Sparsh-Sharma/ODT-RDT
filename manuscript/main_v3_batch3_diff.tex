...
%DIF DEL C:/Users/shar_sp/AppData/Local/Temp/claude/ODT_batch2/main_v3_baseline_prebatch3.tex   Mon Sep 14 09:31:14 2026
%DIF ADD main_v3.tex                                                                            Mon Sep 14 10:56:30 2026
...
The paper reports \DIFdelbegin \DIFdel{six results, each framed so that it can be tested}\DIFdelend \DIFaddbegin \DIFadd{the following aspects of model formulation and assessment}\DIFaddend :
\begin{itemize}
...
% \end{enumerate}
\end{itemize}
Section~\ref{sec:conclusions} concludes. \DIFdelbegin \DIFdel{Supplementary material contains
the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend validation against the strained-turbulence experiment of \citet{ChenMeneveauKatz2006} (CMK)\DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{, summarised in \S\,\ref{sec:cmk}, is detailed in the supplementary material (\S\,S1), which also contains the }\DIFaddend complete intervention family behind the mechanism ledger.
...
%DIFDELCMD < % =========================== PANEL (b) ===========================
%DIFDELCMD < \begin{scope}[shift={(4.55,0)}]
%DIFDELCMD <   \foreach \yy in {1.55, 2.55, 3.55}
%DIFDELCMD <     \draw[-{Latex[length=1mm,width=0.8mm]},black!22] (0.75,\yy) -- (3.70,\yy);
%DIFDELCMD <   \draw[-{Latex[length=1.4mm,width=1.1mm]}] (0.50,0.75) -- (4.05,0.75);
%DIFDELCMD <   \draw[-{Latex[length=1.4mm,width=1.1mm]}] (0.50,0.75) -- (0.50,4.85);
...
%DIFDELCMD <   \foreach \x/\a/\bb in {0.90/1.35/2.05, 1.20/2.65/3.45, 1.55/1.45/2.95,
%DIFDELCMD <     2.00/3.05/3.95, 2.40/1.55/2.55, 2.80/2.75/3.85, 3.20/1.25/1.95,
%DIFDELCMD <     3.62/2.95/4.05}
%DIFDELCMD <     \draw[line width=0.9pt] (\x,\a) -- (\x,\bb);
%DIFDELCMD <   \node[anchor=west,font=\itshape] at (0.02,5.35) {(b)};
%DIFDELCMD < \end{scope}
%DIFDELCMD < 

%DIFDELCMD < % =========================== PANEL (c) ===========================
%DIFDELCMD < \begin{scope}[shift={(9.05,0)}]
...
%DIFDELCMD <   \draw[ev] (b1) -- (b2);
%DIFDELCMD <   \draw[ev] (b2) -- (b3);
%DIFDELCMD <   \draw[ev] (b3) -- (b4) node[midway,anchor=west,font=\fontsize{7.2}{8}\selectfont,black!60] {accept};
%DIFDELCMD <   \draw[ev] (b3.east) -- ++(0.45,0) |- (b1.east);
%DIFDELCMD <   \node[font=\itshape\fontsize{7.2}{8}\selectfont,black!55,rotate=-90] at (2.98,2.95) {reject};
%DIFDELCMD <   \draw[ev] (b4.west) -- ++(-0.45,0) |- (b1.west);
...
\end{scope}
\end{tikzpicture}
  \DIFaddendFL \caption{The model at a glance.
(\textit{a}) Eddy event on $[y_0,y_0+l]$: \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL triplet map \DIFdelbeginFL \DIFdelFL{+ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{compresses and triplicates the local field; the additive }\DIFaddendFL kernel $K(y)$ \DIFdelbeginFL \DIFdelFL{redistribute }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{redistributes }\DIFaddendFL energy among components\DIFdelbeginFL \DIFdelFL{while }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, both }\DIFaddendFL conserving momentum and total energy.
(\textit{b}) \DIFdelbeginFL \DIFdelFL{Instantaneous eddy }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Eddy }\DIFaddendFL events (Poisson)\DIFdelbeginFL \DIFdelFL{interleaved with }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, the vertical bars marking the spatial range of each event, punctuating the continuous }\DIFaddendFL diffusion and \DIFdelbeginFL \DIFdelFL{continuous }\DIFdelendFL mean-strain forcing \DIFaddbeginFL \DIFaddFL{(grey arrows)}\DIFaddendFL .
(\textit{c}) Advancement cycle \DIFaddbeginFL \DIFaddFL{at a state-dependent rate}\DIFaddendFL ; the $k^{-5/3}$ cascade emerges \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{as an outcome of }\DIFaddendFL repeated sampling\DIFaddbeginFL \DIFaddFL{, consistent with inertial-range phenomenology}\DIFaddendFL .}
...
The
comparison is based on the \DIFdelbegin \DIFdel{$c$ dependence of the }\DIFdelend anisotropy $b_{ij}$ \DIFdelbegin \DIFdel{against }\DIFdelend \DIFaddbegin \DIFadd{as a function of }\DIFaddend the deformation ratio \DIFaddbegin \DIFadd{$c$}\DIFaddend . %$c=\exp\int S\,\mathrm{d}t$ (so $\ln c$ is the total strain $e$ of the present notation), 
...
\S\,\ref{sec:rapid-kernel} acts uniformly on every resolved wavenumber. The combination induces \DIFdelbegin \DIFdel{that transfer no energy between scales rigid translations }\DIFdelend \DIFaddbegin \DIFadd{a rigid translation }\DIFaddend of the line spectrum towards higher wavenumber \DIFaddbegin \DIFadd{that transfers no energy between scales}\DIFaddend . We emphasise at the outset that this
...
relaxation acts. We therefore give it its own clock. Whereas the eddy
...
\eqref{eq:clock} carries no dependence on the flow state. Each event
applies the kernel of \eqref{eq:event} to its interval---the
...
mean eddy rate, $R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the
mean number of eddy events over a baseline realisation of duration
...
$e=1$) and only mildly reduced at the deepest strain. The rate is a
clean dial: four times the rate holds the fine scales harder at a
proportionate one-point cost.
...
\S\,\ref{sec:envelope}---so an evolved inflow beats a prescribed one
at precisely the strains where the correction matters, whichever
...
honest up to $e\approx0.25$--$0.5$ under plane strain and at all
...
reference at application strains, an evolved inflow beats the frozen
...
surface-blocking correction with which a complete prediction must
combine it; predictions inherit a spectral-shape uncertainty of order
