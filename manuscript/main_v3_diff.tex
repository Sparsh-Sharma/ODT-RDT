...
%DIF DEL main_v3.tex           Wed Sep  9 18:12:13 2026
%DIF ADD main_v3_revised.tex   Thu Sep 10 15:07:15 2026
...
alone, and the closure assumption of axisymmetry about the line is \DIFdelbegin \DIFdel{not
assumed but measured---it }\DIFdelend \DIFaddbegin \DIFadd{checked against data rather than assumed---it }\DIFaddend is exact for axisymmetric distortion and its
...
nonlinear relaxation together\DIFdelbegin \DIFdel{, at negligible cost}\DIFdelend .
...
relaxation that linear transforms omit, is \DIFdelbegin \DIFdel{the missing element---here
}\DIFdelend \DIFaddbegin \DIFadd{absent from current practice---here }\DIFaddend and wherever distorted turbulence feeds a downstream prediction, from
...
\citep{SharmaKleinSchmidt2022,MedinaMendez2023}, ODT is an \DIFdelbegin \DIFdel{accurate and
inexpensive }\DIFdelend \DIFaddbegin \DIFadd{established, low-cost }\DIFaddend generator of scale-resolved turbulence \DIFdelbegin \DIFdel{statistics---exactly
...
The paper makes six contributions\DIFdelbegin \DIFdel{, each stated so as to be falsifiable}\DIFdelend .
...
term, which is exactly a non-local three-dimensional object?---in a form \DIFdelbegin \DIFdel{deliberately more modest than }\DIFdelend \DIFaddbegin \DIFadd{short of }\DIFaddend a closure claim: the obstruction is stated
...
factor of nine in scale---undiminished, and \DIFdelbegin \DIFdel{a controlled family of interventions falsifies acceptance gating
and map reweighting as remedies. Hierarchical }\DIFdelend \DIFaddbegin \DIFadd{hierarchical }\DIFaddend application of the
...
\DIFdelbegin \DIFdel{Throughout, the posture is the same: where the model is right we show
...
no adjustable constant. This is \DIFdelbegin \DIFdel{the sharpest verification available: }\DIFdelend a closed-form prediction the model must satisfy identically\DIFdelbegin \DIFdel{, not
...
pressure--strain is therefore \DIFdelbegin \DIFdel{not a refinement but a necessity}\DIFdelend \DIFaddbegin \DIFadd{necessary}\DIFaddend , and the accuracy of the method for a given configuration is controlled by the
...
\begin{figure}
  ...
  the rapid term \DIFdelbeginFL \DIFdelFL{restores }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{keeps }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{physical, bounded }\DIFdelendFL amplification \DIFaddbeginFL \DIFaddFL{physical}\DIFaddendFL .}
...
$3.32$); the measured line compression agrees with $\exp(A_{22}t_f)$\DIFaddbegin \DIFadd{, $t_f$ the total straining time, }\DIFaddend to better than $0.3\%$, confirming the kinematic
...
essentially independent of $S^{*}$---a \DIFdelbegin \DIFdel{clean, }\DIFdelend parameter-free target---whereas in plane strain the unstrained component $b_{11}$ is
...
(\S\S\,\ref{sec:spec-bvc}--\ref{sec:spec-op}); and it closes with the benchmark that \DIFdelbegin \DIFdel{meets }\DIFdelend \DIFaddbegin \DIFadd{addresses }\DIFaddend the model-versus-itself \DIFdelbegin \DIFdel{objection head on---a
...
line spectrum towards higher wavenumber. \DIFdelbegin \DIFdel{We emphasise at the outset that this
}\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend rigid translation is a property of the SC-ODT \emph{line}---uniform
...
\DIFaddend \subsection{The linear reference for the projected spectrum: exact
...
extends to substantially higher wavenumber than the baseline---energy is \DIFdelbegin \DIFdel{genuinely }\DIFdelend carried to small scales by compression and cascade---but the
...
near-equilibrium limit applies and where \DIFdelbegin \DIFdel{beating }\DIFdelend \DIFaddbegin \DIFadd{improvement over }\DIFaddend linear theory is meaningful. The spectral \emph{peak} corroborates the regime reading
...
wavenumber, not only the spectral shape at high $\kappa$. The \DIFdelbegin \DIFdel{cost is
the practical justification of the approach: the }\DIFdelend operating-point ensemble completes in a few hundred core-hours, roughly two orders of magnitude below a strained DNS ensemble at comparable Reynolds number, which \DIFdelbegin \DIFdel{is what makes }\DIFdelend \DIFaddbegin \DIFadd{permits }\DIFaddend parametric sweeps over inflow condition\DIFdelbegin \DIFdel{feasible}\DIFdelend .
...
\begin{figure}
  ...
  spectral peak (dashed) against the rigid-translation law (grey dotted)\DIFaddbeginFL \DIFaddFL{, with the geometric $\pm\sigma$ ensemble spread shaded}\DIFaddendFL : the broadband distortion that linear theory cannot represent, at the
  operating point and with the noise averaged out.}
  \label{fig:spec-op}
\end{figure}

\subsection{Validation against \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a laboratory }\DIFaddend strained-turbulence experiment\DIFdelbegin \DIFdel{of
...
kinetic-energy amplification $1.72$ against the exact $1.69$, the residual \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{attributed to }\DIFaddend the moment closure\DIFdelbegin \DIFdel{'s}\DIFdelend , not the implementation \DIFdelbegin \DIFdel{'s
...
\S\,\ref{sec:gate}, whose exercise is thereby motivated by \DIFdelbegin \DIFdel{a measured deficitrather than by completeness}\DIFdelend \DIFaddbegin \DIFadd{this measured deficit}\DIFaddend .
...
largest \DIFdelbegin \DIFdel{precisely }\DIFdelend where the eddies matter least localises the deficiency in
...
the analytical question of this paper, and this section answers it \DIFdelbegin \DIFdel{in a form that is deliberately more modest, and more defensible, than a }\DIFdelend \DIFaddbegin \DIFadd{short of a }\DIFaddend closure claim: we state exactly what obstructs a single-line representation, we
...
energy is distributed over the unresolved wavevector directions. This is the crux of the present question\DIFdelbegin \DIFdel{, and we make }\DIFdelend \DIFaddbegin \DIFadd{; we state }\DIFaddend the required assumption \DIFdelbegin \DIFdel{explicit
rather than implicit}\DIFdelend \DIFaddbegin \DIFadd{explicitly}\DIFaddend .
...
effective polarisation reaches order one, so the \DIFdelbegin \DIFdel{honest }\DIFdelend \DIFaddbegin \DIFadd{applicable }\DIFaddend bound in the strained regime is the uncapped one.
...
The \DIFdelbegin \DIFdel{decisive }\DIFdelend evidence for that attribution is a control experiment: the
...
representation. For the applications\DIFdelbegin \DIFdel{this maps cleanly}\DIFdelend : a rounded,
...
is set out here, with the two \DIFdelbegin \DIFdel{decisive }\DIFdelend \DIFaddbegin \DIFadd{principal }\DIFaddend figures (the relaxation clock
...
exactly the first that reach the measured band. \DIFdelbegin \DIFdel{The bookkeeping is clean
throughout---band }\DIFdelend \DIFaddbegin \DIFadd{Band }\DIFaddend energies move by \DIFaddbegin \DIFadd{only }\DIFaddend a few per cent, the upwash energy fraction and the intermittency tails are \DIFdelbegin \DIFdel{unchanged---and }\DIFdelend \DIFaddbegin \DIFadd{unchanged, and }\DIFaddend the pre-strain
...
$e=1$) and only mildly reduced at the deepest strain. \DIFdelbegin \DIFdel{The rate is a
clean dial}\DIFdelend \DIFaddbegin \DIFadd{Increasing the rate trades off predictably}\DIFaddend : four times the rate holds the fine scales harder at a proportionate one-point cost.
...
deficiency measured there---anisotropy }\DIFdelend \DIFaddbegin \DIFadd{comparison---anisotropy }\DIFaddend allocated uniformly over wavenumber---is confirmed here in transmission form, is unfixable by
...
which alone survives deep strain). One limit remains, and it is structural\DIFdelbegin \DIFdel{rather than curable by any amount of relaxation}\DIFdelend : an isotropising mechanism is bounded at component equality \DIFaddbegin \DIFadd{and no amount of relaxation removes it}\DIFaddend , whereas exact
...
integrated anisotropy---is starkest \DIFdelbegin \DIFdel{precisely }\DIFdelend where the eddies matter least. Second, at moderate rapidity the mechanism tilts the model's
...
the aerofoil geometry and response. This is \DIFdelbegin \DIFdel{the sharpest }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend question the distortion model can be asked without importing an acoustic solver:
...
Figure~\ref{fig:lenoise} shows the result\DIFdelbegin \DIFdel{, and its sign and structure
are the finding}\DIFdelend . At matched upwash variance the distorted spectrum is a
...
proceeds. The distortion \DIFdelbegin \DIFdel{, in short, drains the very }\DIFdelend \DIFaddbegin \DIFadd{therefore removes the upwash }\DIFaddend content the aerofoil responds to. The total change adds the variance term: at the
...
\emph{is} the standard Amiet--von K\'arm\'an prediction, \DIFdelbegin \DIFdel{not a
recalibrationof it}\DIFdelend \DIFaddbegin \DIFadd{with no recalibration}\DIFaddend : the $e=0$ member of the representation is the
...
figure~\ref{fig:amiet-rapid}\DIFdelbegin \DIFdel{, and the claim is worded exactly as the
evidence permits}\DIFdelend . At total strains of $1$--$2$ the frozen-spectrum
...
\S\,\ref{sec:envelope}---so an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{improves on }\DIFaddend a prescribed one at \DIFdelbegin \DIFdel{precisely }\DIFdelend the strains where the correction matters, whichever variant of the model evolves it. Between the two variants the
...
is \DIFdelbegin \DIFdel{therefore not a noise prediction but }\DIFdelend the magnitude and sign of the term that frozen-spectrum practice omits: order $5$--$20$\,dB at the
energy-containing reduced frequencies for total strains of
$0.5$--$2$\DIFdelbegin \DIFdel{---far too large to neglect, and of }\DIFdelend \DIFaddbegin \DIFadd{---of }\DIFaddend the opposite sign to the
blocking correction with which it must ultimately be combined.
Figure~\ref{fig:amiet-abs} shows that the chain from model output to absolute far-field level is complete\DIFdelbegin \DIFdel{and cheap}\DIFdelend ; what remains for a
...
supplies the distorted-inflow input\DIFdelbegin \DIFdel{at negligible cost}\DIFdelend , with its
...
closure claim: line data bound the rapid term sharply, the bounds \DIFdelbegin \DIFdel{are
honest }\DIFdelend \DIFaddbegin \DIFadd{hold }\DIFaddend up to $e\approx0.25$--$0.5$ under plane strain and at all
...
reference at application strains, an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{improves on }\DIFaddend the frozen one by a margin that grows with strain. Relative to the
...
stated here rather than discovered later}\DIFdelend .
