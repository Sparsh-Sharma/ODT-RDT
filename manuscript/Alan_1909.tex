...
exact production $-A_{ij}u_j$ (whose second moment is the production
$\mathcal{P}_{ij}$ of \eqref{eq:rstransport}), and, in place of the
...
$\mathcal{A}_{ij}\equiv-A_{ij}+B_{ij}$ acts pointwise, so over a
non-diffusive sub-step the profile is advanced by a $y$-independent
matrix,
$u_i(y,t)=[\exp\!\int_{t_0}^{t}\mathcal{A}\,\mathrm{d}t']_{ij}\,
u_j(y,t_0)$---continuous profiles therefore map
...
than of the construction. We integrate the model's second-moment (Reynolds-stress) equation along
...
\begin{figure}
  ...
  SC-ODT second-moment equation with the LRR closure \eqref{eq:lrr} and the IP
