...
%DIF DEL C:/Users/shar_sp/AppData/Local/Temp/claude/ODT_batch2/main_v3_baseline_prebatch4.tex   Mon Sep 14 11:02:54 2026
%DIF ADD main_v3.tex                                                                            Mon Sep 14 14:42:14 2026
...
The paper reports the following aspects of model formulation and assessment:
\begin{itemize}
...
% \end{enumerate}
\end{itemize}
Section~\ref{sec:conclusions} concludes. The validation against the strained-turbulence experiment of \citet{ChenMeneveauKatz2006} (CMK), summarised in \S\,\ref{sec:cmk}, is detailed in the supplementary material (\S\,S1), which also contains the complete intervention family behind the mechanism ledger.
...
\begin{scope}[shift={(9.05,0)}]
  ...
  \node[box]              (b2) at (1.35,2.95) {sample candidate eddy $(y_0,l)$; infer timescale $\tau(l)$ from the local field};
...
(\textit{a}) Eddy event on $[y_0,y_0+l]$: the triplet map compresses and triplicates the local field; the additive kernel $K(y)$ redistributes energy among components, both conserving momentum and total energy.
...
$C_2=3/5$; IP) and the quasi-isotropic model of \citet{LaunderReeceRodi1975} (\DIFaddbegin \DIFadd{LRR-QI, abbreviated }\DIFaddend LRR),
\begin{equation}
...
The
comparison is based on the anisotropy $b_{ij}$ as a function of the deformation ratio $c$. %$c=\exp\int S\,\mathrm{d}t$ (so $\ln c$ is the total strain $e$ of the present notation), 
...
$\exp(A_{22}t_f)$, $t_f$ the total straining time, to better than $0.3\%$, confirming the kinematic
...
\S\,\ref{sec:rapid-kernel} acts uniformly on every resolved wavenumber. The combination induces a rigid translation of the line spectrum towards higher wavenumber that transfers no energy between scales. We emphasise at the outset that this
...
subsection are medians over $1024$ realisations per case with bootstrap confidence intervals \DIFaddbegin \DIFadd{(Appendix~\ref{app:numerics})}\DIFaddend ; median statistics are used because the strained band
...
\S\,\ref{sec:gate}, whose introduction exercise is thereby motivated by a measured
...
\begin{figure}
  ...
  exact-RDT-distorted von K\'arm\'an fit, against total strain, for low and high onset strain-rate parameters (medians over $1024$
...
\paragraph{Assumption A1 (axisymmetry of the undistorted spectrum about the line; see Appendix~\ref{app:twoscalar}).} The spectrum tensor of the incoming turbulence, before the rapid
...
\begin{center}\small
\begin{tabular}{lccc}
constraint level & $\mathcal I_{1}$ & $\mathcal I_{2}$ & $\mathcal I_{3}$\\[1pt]
...
effective polarisation reaches order one, so the \DIFdelbegin \DIFdel{honest }\DIFdelend \DIFaddbegin \DIFadd{valid }\DIFaddend bound in the strained regime is the uncapped one.

As shown in Appendix~\ref{app:forward}, the representation also supplies two \emph{free falsifiers} that the line itself reports: under A1, $\phi_{11}(\kappa_2)=\phi_{33}(\kappa_2)$ at
...
\begin{figure}
  ...
  $\Phi_{22}$: it grows under plane strain but under axisymmetric strain stays at the isotropic floor (grey dotted): the nonzero $m=2$ residue an isotropic field produces at this box resolution, $0.133$. (\textit{b}) Transverse
...
($x$-dependent) and anisotropy ($c$-scalar) information that the moment closure omits---and the effects of both omissions are now measured: the wavenumber dependence
...
measurement bands \DIFdelbegin \DIFdel{, $\kappa_2\in[0.6,2]\,\kappa_p$ (the }\DIFdelend \DIFaddbegin \DIFadd{in wavenumber $\kappa_2$: a low-wavenumber,
...
drawn---and then recursively to $9$ and $27$ sub-regions. One \DIFaddbegin \DIFadd{sub-}\DIFaddend level ($\ell/3$ for an eddy of size $\ell$) is \DIFdelbegin \DIFdel{null}\DIFdelend \DIFaddbegin \DIFadd{inconsequential}\DIFaddend : for the large eddies that
...
The one \DIFdelbegin \DIFdel{axis }\DIFdelend \DIFaddbegin \DIFadd{feature }\DIFaddend the eddy-locked \DIFdelbegin \DIFdel{family cannot test }\DIFdelend \DIFaddbegin \DIFadd{formulations do not modify }\DIFaddend is \emph{when} relaxation acts. We therefore give it its own clock. Whereas the eddy
events are sampled from the state-dependent rate density
$\lambda(y_0,\ell;t)$ of \S\,\ref{sec:odt-review}, the relaxation-only
events form an independent Poisson stream with the prescribed rate
density
\begin{equation}
...
\eqref{eq:clock} carries no dependence on the flow state. \DIFaddbegin \DIFadd{This candidate density is the model's presumed }\emph{\DIFadd{sampling}} \DIFadd{distribution, not the realised distribution of accepted eddies, which the acceptance rule reshapes; it is used because it is convenient to sample and carries the gross features of the physical eddy-size distribution. }\DIFaddend Each event applies the kernel of \eqref{eq:event} to its interval---the
...
energy, exactly and per event. The rate is matched to the baseline's
mean eddy rate, $R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the mean number of eddy events over \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an SC-ODT }\DIFaddend baseline realisation of duration
...
property, was the deep-strain limiter. But the \DIFdelbegin \DIFdel{uncapped stream is
}\DIFdelend \DIFaddbegin \DIFadd{stream drawing the full candidate density---capped only at the model's default largest scale $L_{\max}$---is }\DIFaddend scale-blind and overcorrects: it also suppresses the one-point
...
behaviour validated in \S\,\ref{sec:lr-validation}. Restricting the stream to sub-band interval
sizes---replacing $f$ in \eqref{eq:clock} by its restriction to
$\ell\le\ell^{*}$, renormalised (over-cap draws are resampled), with
$\ell^{*}$ the scale separating the two measurement bands---resolves
the tension: the
...
$1.63$--$1.94$, contrasts significant throughout and strongest at $e=3$ \DIFaddbegin \DIFadd{(figure~\ref{fig:clock}}\textit{\DIFadd{a}}\DIFadd{)}\DIFaddend ---while the one-point trajectory is essentially untouched in the moderate-strain regime ($u_2^2/2k_t$ within $0.005$ of the baseline at $e=1$) and only mildly reduced at the deepest strain \DIFaddbegin \DIFadd{(figure~\ref{fig:clock}}\textit{\DIFadd{b}}\DIFadd{)}\DIFaddend . The rate is a \DIFdelbegin \DIFdel{clean dial}\DIFdelend \DIFaddbegin \DIFadd{single control}\DIFaddend : four times the rate holds the fine scales harder\DIFaddbegin \DIFadd{, }\DIFaddend at a proportionate \DIFaddbegin \DIFadd{reduction of the }\DIFaddend one-point \DIFdelbegin \DIFdel{cost}\DIFdelend \DIFaddbegin \DIFadd{anisotropy}\DIFaddend .
...
\begin{figure}
  ...
  variance change ($-3.5$ to $-7.3$\,dB at the slow rate, where decay dominates the transit \DIFaddbeginFL \DIFaddFL{time}\DIFaddendFL ; $+0.5$ to $+1.9$\,dB at the rapid rate, where
...
are the finding. At matched upwash variance the distorted spectrum is a
...
the reallocation; at the slow rate viscous decay over the longer transit \DIFaddbegin \DIFadd{time }\DIFaddend deepens it.
...
\begin{figure}
  ...
  the exact Sears response \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{Sears1941}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL , for a flat plate of chord $0.39$\,m
...
compact lift-dipole form of Amiet's theory with the exact Sears response \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Sears1941}}\hskip0pt%DIFAUXCMD
...
\begin{figure}
  ...
  scale); the shaded band\DIFaddbeginFL \DIFaddFL{, present where the effective strain is
  non-negligible (panels }\textit{\DIFaddFL{a,b}}\DIFaddFL{), }\DIFaddendFL spans truncation at
  $\Lambda/4$--$\Lambda$.}
  \label{fig:expval}
\end{figure}
...
evidence permits. At total strains of $1$--$2$ the frozen-spectrum
...
\S\,\ref{sec:envelope}---so an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{outperforms }\DIFaddend a prescribed one at \DIFdelbegin \DIFdel{precisely }\DIFdelend the strains where the correction matters, whichever
...
the practice has survived unchallenged. Beyond the window the frozen
...
reference and carry their measured uncertainty with them. What the
methodology adds is the right-hand half of the figure: an inflow that
continues to predict where the frozen assumption fails silently.

The scope \DIFdelbegin \DIFdel{of this statement is set out as carefully as its content.
It concerns }\DIFdelend \DIFaddbegin \DIFadd{is limited to }\DIFaddend the \emph{free} distortion by the mean strain: the
...
absolute far-field level is complete\DIFdelbegin \DIFdel{and cheap}\DIFdelend ; what remains for a
...
closure claim: line data bound the rapid term sharply, the bounds are \DIFdelbegin \DIFdel{honest }\DIFdelend \DIFaddbegin \DIFadd{valid }\DIFaddend up to $e\approx0.25$--$0.5$ under plane strain and at all
...
sub-scales. The intervention family \DIFdelbegin \DIFdel{maps }\DIFdelend \DIFaddbegin \DIFadd{traces }\DIFaddend the mechanism's envelope\DIFdelbegin \DIFdel{precisely}\DIFdelend : depth that reaches the measured scales is necessary and
...
wavenumber- and angle-dependent spectral kernel \DIFdelbegin \DIFdel{derived in
\S\,\ref{sec:gate}}\DIFdelend \DIFaddbegin \DIFadd{$g_n$ of \eqref{eq:g1}--\eqref{eq:g3}, derived in Appendix~\ref{app:kernels}}\DIFaddend ; exercising it in place of the wavenumber-uniform
closure is the natural continuation, now motivated by a measured
deficit and pointed at a measured target.

For the application that motivated the work, the model delivers what the prediction chain \DIFaddbegin \DIFadd{otherwise }\DIFaddend lacks: a distorted, anisotropic, scale-resolved
...
reference at application strains, an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{outperforms }\DIFaddend the frozen one by a margin that grows with strain. Relative to the
...
aerofoil responds to. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend omitted term is \DIFdelbegin \DIFdel{far too large to neglect and }\DIFdelend opposite in sign to the surface-blocking correction with which a complete prediction must combine it; predictions inherit a spectral-shape uncertainty of order
the distance envelope at the operating point's strain and rapidity\DIFdelbegin \DIFdel{,
stated here rather than discovered later}\DIFdelend \DIFaddbegin \DIFadd{. A far-field test that discriminates the relaxation mechanism---one whose acoustic response samples the sub-cap scales at accumulated strains of order unity, such as a turbulence-ingesting rotor---remains the falsifiable target for future work}\DIFaddend .
