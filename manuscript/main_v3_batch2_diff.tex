...
%DIF DEL C:/Users/shar_sp/AppData/Local/Temp/claude/ODT_batch2/main_v3_baseline_prebatch2.tex   Mon Sep 14 08:28:50 2026
%DIF ADD main_v3.tex                                                                            Mon Sep 14 09:15:50 2026
...
The paper reports six results, each framed so that it can be tested:
\begin{itemize}
...
\citet{ChenMeneveauKatz2006} (CMK) and the complete intervention family
...
(\textit{a}) Eddy event on $[y_0,y_0+l]$: triplet map + kernel $K(y)$ redistribute energy among components while conserving momentum and total energy.
(\textit{b}) Instantaneous eddy events (Poisson) interleaved with diffusion and continuous mean-strain forcing.
(\textit{c}) Advancement cycle; the $k^{-5/3}$ cascade emerges from repeated sampling.}
...
The
comparison is based on the $c$ dependence of the anisotropy $b_{ij}$ against the deformation
...
combination induces that transfer no energy between scales rigid translations of the
line spectrum towards higher wavenumber. We emphasise at the outset that this
...
\begin{figure}
  ...
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{low }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{rapid }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{high }\DIFaddendFL onset strain-rate parameters (medians over $1024$
...
\paragraph{Assumption A1 (axisymmetry of the undistorted spectrum about the line\DIFaddbegin \DIFadd{; see Appendix~\ref{app:twoscalar}}\DIFaddend ).} The spectrum tensor of the incoming turbulence, before the rapid
...
\begin{center}\small
\begin{tabular}{lccc}
constraint level & \DIFdelbegin \DIFdel{$\mathcal I_{11}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal I_{1}$ }\DIFaddend & \DIFdelbegin \DIFdel{$\mathcal I_{22}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal I_{2}$ }\DIFaddend & \DIFdelbegin \DIFdel{$\mathcal I_{33}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal I_{3}$}\DIFaddend \\[1pt]
line data $+$ solenoidality $+$ realisability & 0.77 & 1.28 & 0.86\\
$+$ log-Lipschitz slope cap ($\lambda=4$) & \multicolumn{3}{c}{$<5\%$ narrower}\\
$+$ polarisation cap $|c|\le\gamma a$, $\gamma=0$ \DIFaddbegin \DIFadd{(i.e.\ $c=0$) }\DIFaddend & 0.34 & 0.58 & 0.24\\
\end{tabular}
\end{center}
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As shown in Appendix~\ref{app:forward}, the }\DIFaddend representation also supplies two \emph{free falsifiers} that the line itself reports: under A1, $\phi_{11}(\kappa_2)=\phi_{33}(\kappa_2)$ at
...
\begin{figure}
  ...
  $\Phi_{22}$: it grows under plane strain but \DIFaddbeginFL \DIFaddFL{under axisymmetric strain }\DIFaddendFL stays at the isotropic floor (grey dotted)\DIFdelbeginFL \DIFdelFL{under axisymmetric strain}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: the nonzero $m=2$ residue an isotropic field produces at this box resolution, $0.133$}\DIFaddendFL . (\textit{b}) Transverse
...
($x$-dependent) and anisotropy ($c$-scalar) information that the moment closure \DIFdelbegin \DIFdel{discards---and both discards }\DIFdelend \DIFaddbegin \DIFadd{omits---and the effects of both omissions }\DIFaddend are now measured: the wavenumber dependence
...
relaxation acts. We therefore give it its own clock\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. Whereas the eddy
...
presumed candidate-size density of the }\DIFaddend model's \DIFdelbegin \DIFdel{eddy-size distribution, placed
uniformly on the domain, andrelaxed by the same conservative kernel pass with the }\DIFdelend \DIFaddbegin \DIFadd{own eddy sampling
\mbox{%DIFAUXCMD
...
from the eddy statistics entirely}\DIFdelend \DIFaddbegin \DIFadd{of
...
energy, exactly and per event}\DIFaddend . The rate is matched to the baseline's
mean eddy rate, \DIFaddbegin \DIFadd{$R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the
...
\DIFdelbegin \DIFdel{sizes (}\DIFdelend \DIFaddbegin \DIFadd{sizes---replacing $f$ in \eqref{eq:clock} by its restriction to
}\DIFaddend $\ell\le\ell^{*}$, \DIFaddbegin \DIFadd{renormalised (over-cap draws are resampled), with
$\ell^{*}$ }\DIFaddend the scale separating the two measurement \DIFdelbegin \DIFdel{bands) resolves }\DIFdelend \DIFaddbegin \DIFadd{bands---resolves
}\DIFaddend the tension: the
