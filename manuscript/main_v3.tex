...
the scales of the imbalance: in a four-way comparison against the DNS
and exact linear theory on the benchmark observable, it moves the
...
relaxation that linear transforms omit, is the missing element, and the
distortion problem---not the acoustic stage---is the subject of this
paper.
...
inexpensive generator of scale-resolved turbulence statistics---exactly
what a model of strained turbulence must deliver. But standard ODT
cannot represent distortion by a mean strain: its pressure treatment
models only the slow, turbulence-driven return to isotropy, and the
...
The paper makes five contributions, each stated so as to be falsifiable.
First, a strain-coupled formulation of ODT (\S\,\ref{sec:formulation}):
...
(\S\,\ref{sec:box-dns}). Fourth, an answer to the question of principle
--- what can a single line say about the rapid pressure--strain term,
which is exactly a non-local three-dimensional object? --- in a form
deliberately more modest than a closure claim: the obstruction is stated
...
a transmission diagnostic shows the strain-induced anisotropy crossing
two map generations undiminished, a controlled family of interventions
falsifies acceptance gating and map reweighting as remedies, and
hierarchical application of the energy-redistributing kernel to eddy
sub-scales---scale-local relaxation---is shown to supply the missing
ingredient where it reaches the scales of the imbalance, with its own
deep-strain limit measured and stated (\S\,\ref{sec:limits}).
Throughout, the posture is the same: where the model is right we show
against what, and where it is wrong we measure by how much and locate
why.
...
Section~\ref{sec:verification} verifies it against exact
rapid-distortion solutions and validates it at the moment level against
the DNS of \citet{LeeReynolds1985}. Section~\ref{sec:val-spectrum}
constructs the linear reference, examines the emergent distorted
spectrum and its operating point, and carries out the strained-box
DNS benchmark. Section~\ref{sec:gate} treats the single-line closure
question. Section~\ref{sec:limits} measures the model's limits and
establishes scale-local relaxation as the constructive direction.
...
\begin{figure}[!t]
  ...
  closure to $10^{-4}$ in the fractions and $0.3\%$ in the
...
$b$-dependent term, in \S\,\ref{sec:order}. The present model
...
family's known limitation rather than any line-specific artefact.
...
\begin{figure}
  ...
  band) against exact linear rapid distortion (dotted,
  wavevector-ensemble integration), the IP and LRR-QI closures, and the
  DNS of \citet{LeeReynolds1985} (symbols and shaded $S^{*}$ range).
  The fast-strain DNS tracks exact RDT, whose rapid pressure--strain
  feeds the neutral direction; the single-point closures---and the
  model built on them---miss this orientation-carried redistribution,
  the deficiency measured and localised in \S\,\ref{sec:order}.}
  \label{fig:lr-b11}
\end{figure}
...
\subsubsection{The distorted spectrum: strain on against strain
...
the full model (eddies and dilatation active) and a baseline identical
in every respect except that the strain machinery is switched off, so
the line does not compress. Because the two differ only in the presence
...
redistribute energy and the centroid drifts below its initial value. The
full model is lifted clearly above this baseline---compression does push
the spectrum toward higher wavenumber---yet it settles at
$\bar\kappa/\bar\kappa_0\approx1.6$--$2.1$, far below the
rigid-translation line, which rises to $7.4$ at $e=4$. The reason is
visible in the spectra: the full-model spectrum is not a rigid
...
\begin{figure}
  ...
  \caption{Full model against the matched no-strain baseline at
  $\nu=10^{-5}$ (ensemble means over $128$ realisations, geometric
  $\pm\sigma$ bands). (\textit{a}) Final-strain total spectrum
  $E(\kappa_2)$: the full model (strain on) extends to higher wavenumber
  than the baseline (strain off) but remains broadband, not a rigid
  translation. (\textit{b}) Centroid migration: the baseline relaxes
  below unity, the full model is lifted above it by the compression, yet
  stays far below the rigid-translation line (dashed). The gap between
  the two solid curves is the distortion attributable to the mean strain
...
lowest viscosities climb toward it only by piling energy into an
...
parameter settles at $\chi=Sk_t/\varepsilon\approx0.8$ over the strained
trajectory, corroborated within a few tens of per cent by the direct
gradient estimate $\langle2\nu(\partial u_i/\partial y)^2\rangle$: the
operating point sits at $\chi=O(1)$, the regime the leading-edge
stagnation flow occupies, where neither the rapid-distortion nor the
near-equilibrium limit applies and where beating linear theory is
...
wavenumber, not only the spectral shape at high $\kappa$. The cost is
the practical justification of the approach: the operating-point
...
which is what makes parametric sweeps over inflow condition feasible.
...
\begin{figure}
  ...
  (\textit{a}) Final-strain component spectra: smooth rolloff with no
  grid-scale shelf; the upwash $E_2$ amplified above the streamwise
  $E_1$; dotted reference slope $\kappa^{-5/3}$.
  (\textit{b}) Total-spectrum centroid migration against the
  rigid-translation law (dashed): the broadband distortion that linear
  theory cannot represent, at the operating point and with the noise
  averaged out.}
  \label{fig:spec-op}
\end{figure}

\subsection{Validation against the strained-turbulence experiment of
Chen, Meneveau \& Katz}\label{sec:cmk}

The strain-on/strain-off comparison is model-internal; an external
benchmark must confirm that both the rapid-distortion limit and the
departure from it are physical. We use the plane-strain experiment of
\citet{ChenMeneveauKatz2006} (CMK), in which initially near-isotropic
turbulence at $R_\lambda\approx400$ is subjected to a controlled planar
straining cycle and the one-dimensional component spectra are compared
with rapid-distortion theory. We reproduce the \emph{straining} phase of
their cycle (the destraining phases involve a strain reversal outside
the present monotonic configuration); its end corresponds to a total
deformation $D=\exp(\tfrac12\!\int S\,\mathrm{d}t)\approx3$, i.e.
$e\approx2.20$ in the present notation.
...
Figure~\ref{fig:cmk-rdt} shows that it does: the component spectral
...
implementation's (\S\,\ref{sec:level1a}), since with the events
suppressed the solver evolves the closure trajectory ($1.72$) exactly.
The truly inviscid setting is required: any nonzero viscosity
...
anisotropy between the components. Figure~\ref{fig:cmk-aniso} shows the
strain-induced migration of the streamwise and upwash component
...
against $1.5$ for the upwash, both far below the rigid value
$3$---so the upwash component is held back most. This is the qualitative behaviour CMK
report---the strained spectrum departs from the isotropic-translation
expectation with the components responding differently---and it is the
spectral signature of the broadband distortion of
\S\,\ref{sec:spec-bvc}. The comparison is restricted to its qualitative
content, for three reasons stated plainly: the band-limited initial
spectrum differs from CMK's, so absolute levels are not comparable; the
line-resolved one-dimensional spectra and the experiment's
transverse/longitudinal projections are not the same projection of the
spectrum tensor; and the scale separation achieved here
($\mathcal{L}/\eta\approx250$) is below the experimental $\approx930$,
whose compressed dissipation range exceeds feasible line resolution.
Within these limits the model reproduces the rapid-distortion reference
and the qualitative departure mechanism of the benchmark; the
quantitative spectral assessment is carried out against the strained-box
DNS below, where the projection and the initial spectrum are matched by
construction.
...
\begin{figure}
  ...
  rigid-translation law (dashed). The components diverge under
...
\begin{table}
  ...
  strain-coupled ODT  & $+0.103$ & $+0.103$ & $+0.096$ & $+0.073$ & $+0.075$ & $+0.079$ & $+0.076$\\
  \end{tabular}
...
\begin{table}
  ...
  strongest early effect of the family ($\mathcal A_{\rm hi}$
  $1.43\to1.26$); low band significant even at $e=3.9$; fine scales
  overshoot depth-only at deep strain &
  dosage helps early and at large scales, and harms deep-strain fine
  scales wherever it is applied\\
  \end{tabular}
...
is then complete: depth sets \emph{where} the relaxation acts and is
necessary; dosage and delocalisation raise the early gains in
proportion to the total relaxation delivered; and no combination
tested---more depth, more dosage, more events, other
locations---prevents the fine-scale fade beyond $e\approx3$. The
deep-strain fine scales are resupply-limited, not
implementation-limited: relaxation acts at the event rate while the
strain resupplies the imbalance continuously, and at these total
strains no per-event scheme keeps pace. Whether that saturation is a
model artefact or the physical statement that sustained rapid
distortion outruns any cascade-mediated return to isotropy is not
decided by these experiments.
...
The head-to-head that summarises the paper is
...
established the benchmark---for exact linear theory, the DNS, the
strain-coupled model, and the model with the adaptive-depth sub-scale
kernels active, at both strain-rate parameters, from same-binary
ensembles under the identical precursor protocol. Three statements can
be read directly from the shared axes. First, at $Sk_t/\varepsilon=16$
the DNS lies on the exact-RDT curve: the rapid limit is realised in the
benchmark itself, and the deficiency of the baseline model---its
uniform allocation of a correct integrated anisotropy---is starkest
precisely where the eddies matter least. Second, at
$Sk_t/\varepsilon=0.8$ scale-local relaxation tilts the model's
allocation towards the DNS: the anisotropy now decreases with
wavenumber, and the fine-band excess over the DNS is roughly halved
(from $+0.076$ against the DNS value $-0.046$ to $+0.024$). Third, at
$Sk_t/\varepsilon=16$ the same mechanism moves the allocation only
marginally within $e\le1$: at rapid strain the relaxation events cannot
keep pace with the resupply---the transmission finding of this section
restated on the benchmark axes. What the eddy-level correction achieves
and where it saturates are thereby exhibited on the same axes as the
deficiency it addresses; closing the remaining gap is the burden of the
continuous wavenumber-dependent operator identified above.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{Figures/fig_fourway.pdf}
  \caption{The four-way comparison on the benchmark observable of
  \S\,\ref{sec:threeway}: strain-induced upwash anisotropy per
  wavenumber band at $e=1$, for exact linear RDT, DNS, the
  strain-coupled model, and the model with adaptive-depth scale-local
  relaxation, at (\textit{a}) $Sk_t/\varepsilon=0.8$ and (\textit{b})
  $16$ (ODT: $1024$ realisations per case, same binary and precursor
  protocol; DNS and companions as in \S\,\ref{sec:box-dns}). At rapid
  strain the DNS realises the exact-RDT allocation; scale-local
  relaxation moves the model towards the DNS at moderate rapidity and
  only marginally at rapid strain.}
  \label{fig:fourway}
\end{figure}
...
projected upwash spectrum by a factor of four across the resolved range
...
The measured limits point to a specific missing ingredient, and the
paper's final contribution is to identify and supply it. The
strain-induced fine-scale anisotropy is delivered directly by
large-eddy maps, which is why no acceptance-gating or map-reweighting
scheme can control it; what does move it is relaxation applied at the
scale of the imbalance---the model's own energy-equalising kernel,
applied hierarchically to eddy sub-scales. The intervention family
maps the mechanism's envelope precisely: depth that reaches the
measured scales is necessary and sufficient at moderate strain, added
dosage and delocalised application raise the early gains in proportion,
and no combination prevents the fine-scale fade beyond $e\approx3$,
which is resupply-limited rather than implementation-limited. At the
formulation level the continuous analogue of this correction is the
wavenumber- and angle-dependent spectral kernel derived in
\S\,\ref{sec:gate}; exercising it in place of the wavenumber-uniform
closure is the natural next step, now motivated by a measured deficit.
...
magnitude below the cost of a strained DNS ensemble. Supplying it to
the acoustic stage---together with the surface blocking and spanwise
coherence a complete leading-edge prediction requires---is deliberately
left to a companion study, on the model's measured terms: predictions
inherit a spectral-shape uncertainty of order the distance envelope at
the operating point's strain and rapidity, stated here rather than
discovered later.
