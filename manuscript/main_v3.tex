...
$C_2=3/5$; IP) and the quasi-isotropic model of \citet{LaunderReeceRodi1975} (LRR-QI, abbreviated LRR),
\begin{equation}
...
subsection are medians over $1024$ realisations per case with bootstrap confidence intervals (Appendix~\ref{app:numerics}); median statistics are used because the strained band
...
effective polarisation reaches order one, so the valid bound in the strained regime is the uncapped one.
...
drawn---and then recursively to $9$ and $27$ sub-regions. One sub-level ($\ell/3$ for an eddy of size $\ell$) is inconsequential: for the large eddies that
feed the band, $\ell/3$ still sits above it. Two sub-levels produce the first statistically significant fine-scale isotropisation of the family ($\mathcal A_{\rm hi}$ from $1.43$ to $1.27$ at $e=1$, the change significant in both the fine-scale and energy-containing bands), and three sub-levels extend the effect to deep strain: the sub-levels that work, $\ell/9$ and $\ell/27$, are exactly those that reach the measured band. Band energies move by only a few per cent throughout, the upwash energy fraction and the intermittency tails are unchanged, and even at $e=0$, before any strain, the fine-scale imbalance is pulled towards component equality ($\mathcal A_{\rm hi}$ from $0.83$ to $0.95$).
...
This motivates giving relaxation its own clock: the one feature the eddy-locked formulations do not modify is \emph{when} relaxation acts. Whereas the eddy
...
mean eddy rate, $R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the mean number of eddy events over an SC-ODT baseline realisation of duration
...
property, was the deep-strain limiter. But the stream drawing the full candidate density---capped only at the model's default largest scale $L_{\max}$---is scale-blind and overcorrects: it also suppresses the one-point
...
$1.63$--$1.94$, contrasts significant throughout and strongest at $e=3$ (figure~\ref{fig:clock}\textit{a})---while the one-point trajectory is essentially untouched in the moderate-strain regime ($u_2^2/2k_t$ within $0.005$ of the baseline at $e=1$) and only mildly reduced at the deepest strain (figure~\ref{fig:clock}\textit{b}). The rate is a single control: four times the rate holds the fine scales harder, at a proportionate reduction of the one-point anisotropy.
...
\begin{figure}
  ...
  price: upwash energy fraction. The reference is the SC-ODT baseline
...
\begin{figure}
  ...
  variance change ($-3.5$ to $-7.3$\,dB at the slow rate, where decay dominates the transit time; $+0.5$ to $+1.9$\,dB at the rapid rate, where
...
the reallocation; at the slow rate viscous decay over the longer transit time deepens it.
...
\begin{figure}
  ...
  the exact Sears response \citep{Sears1941}, for a flat plate of chord $0.39$\,m
...
compact lift-dipole form of Amiet's theory with the exact Sears response \citep{Sears1941}. The spectra enter in physical units through two matching
...
effective strain is negligible and both computed inflows correctly predict \emph{no} correction to the frozen spectrum, while for the NACA0012 they lower the prediction towards the data at every speed
(bias $+5.7\!\to\!+5.3$\,dB at $40$\,m\,s$^{-1}$,
$+1.8\!\to\!+1.4$\,dB at $60$, $+5.1\!\to\!+4.7$\,dB at $90$), by the modest amount the geometric effective strain implies; the shift
...
\S\,\ref{sec:envelope}---so an evolved inflow outperforms a prescribed one at the strains where the correction matters, whichever
...
scales at accumulated strains of order unity---turbulence-ingesting rotors \citep{MajumdarPeake1998}, where the streamtube contraction supplies the strain and the
...
closure claim: line data bound the rapid term sharply, the bounds are valid up to $e\approx0.25$--$0.5$ under plane strain and at all
...
sub-scales. The intervention family traces the mechanism's envelope: depth that reaches the measured scales is necessary and
...
wavenumber- and angle-dependent spectral kernel $g_n$ of \eqref{eq:g1}--\eqref{eq:g3}, derived in Appendix~\ref{app:kernels}; exercising it in place of the wavenumber-uniform closure---the next step set out in \S\,
\ref{sec:limits-endpoint}---is the natural continuation, now motivated by a measured deficit and pointed at a measured target.

For the application that motivated the work, the model delivers what the prediction chain otherwise lacks: a distorted, anisotropic, scale-resolved
...
reference at application strains, an evolved inflow outperforms the frozen one by a margin that grows with strain. Relative to the
...
the distance envelope at the operating point's strain and rapidity. A far-field test that discriminates the relaxation mechanism---one whose acoustic response samples the sub-cap scales at accumulated strains of order unity, such as a turbulence-ingesting rotor---remains the falsifiable target for future work.
