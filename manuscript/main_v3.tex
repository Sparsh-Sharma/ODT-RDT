...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{Figures/fig_odt_schematic.pdf}
  \caption{The model at a glance. (\textit{a}) An eddy event: on the
  sampled interval $[y_0,y_0+l\,]$ the triplet map compresses each
  profile threefold, inserts three images (the central one reversed,
  images numbered), and the kernel addition $c_iK(y)$ (inset;
  dotted: the mapped profile before it) redistributes kinetic energy
  among the components at conserved per-component momentum and total
  energy; the exterior is untouched. (\textit{b}) The two interleaved
  mechanisms in the space--time plane: eddy events are instantaneous
  (vertical bars: position and size as sampled), arriving as a Poisson
  sequence whose rate density is set by the instantaneous flow state,
  while between events the fields advance by deterministic diffusion
  and, from \S\,\ref{sec:strain-forcing} onward, the continuous
  mean-strain forcing. (\textit{c}) The corresponding advancement
