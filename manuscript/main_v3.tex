...
\begin{figure}
  ...
  (\textit{a}) $e=1$ and (\textit{b}) $e=2$. Purple: exact (inviscid)
  RDT of the model's relaxed initial state---the reference the
  strained-box DNS of \S\,\ref{sec:box-dns} validates at rapid
  rates---with level set by the kernel's exact upwash amplification
  (reproducing the moment-level value to $0.7\%$). Dashed: the frozen
  von K\'arm\'an inflow of standard practice. Blue/red: the standard
  and clock-relaxed computed inflows (fitted representations, measured
  ensemble levels), each shaded by its measured spectral rms distance
...
rapidities, and, unlike the standard model's, their distance does not
