...
relaxation that linear transforms omit, is the missing element---here
and wherever distorted turbulence feeds a downstream prediction, from
...
The paper makes six contributions, each stated so as to be falsifiable.
...
term, which is exactly a non-local three-dimensional object?---in a form
...
factor of nine in scale---undiminished,
and a controlled family of interventions falsifies acceptance gating
and map reweighting as remedies. Hierarchical application of the
...
sampling from this rate, not inputs \citep{Kerstein1999}.
...
\end{scope}
\end{tikzpicture}
  \caption{The model at a glance. (\textit{a}) An eddy event on the
  sampled interval $[y_0,y_0+l\,]$, taking the field from `before' to
  `after': the triplet map compresses the profile threefold and inserts
  three images with the central one reversed (numbered $1$--$3$), and the
  kernel $K(y)=y-f(y)$ (drawn below) adds $c_iK(y)$, redistributing
  kinetic energy among the three velocity components while conserving
  each component's momentum and the total energy; the exterior is
  untouched. (\textit{b}) The two interleaved mechanisms in the
  space--time plane: eddy events are instantaneous (vertical bars, at the
  sampled position $y_0$ and size $l$), arriving as a Poisson sequence
  whose rate density is set by the instantaneous flow state, while
  between events the fields advance by deterministic diffusion (faint
  arrows) and, from \S\,\ref{sec:strain-forcing} onward, the continuous
  mean-strain forcing. (\textit{c}) The advancement cycle of the
  implementation; most sampled candidates are rejected, and the
  $k^{-5/3}$ cascade is an outcome of repeated sampling from this loop,
  not an input.}
...
\begin{figure}
  ...
  growth of both, and the two DNS agree where they overlap; the
...
benchmark that meets the model-versus-itself objection head on---a
...
\begin{figure}
  ...
  (\textit{b}) Migration of the total-spectrum centroid (solid; grey
  band the geometric $\pm\sigma$ of the centroid across the $200$
  realisations) and the spectral peak (dashed) against the
  rigid-translation law (grey dotted):
...
residual being the moment closure's, not the implementation's
(\S\,\ref{sec:level1a}). With eddies and viscosity active the component
