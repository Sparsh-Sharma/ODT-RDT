...
\begin{abstract}
Turbulence is strained before it meets a solid surface, and the
distortion of its spectrum is often what governs the downstream
response; yet reduced-order predictions, leading-edge-noise theory among
them, still take the incoming spectrum as frozen. We develop a
strain-coupled formulation of one-dimensional turbulence that evolves
strained turbulence with full scale resolution at negligible cost,
carrying the rapid distortion and the nonlinear relaxation together. The
obstacle is that the rapid pressure--strain is non-local and
three-dimensional while an ODT line resolves a single wavenumber.
Splitting the distortion physics by structure resolves it: the
production and a spectrally closed rapid operator enter as continuous
forcing, and the eddy events carry the turbulence-driven relaxation.
Measured against exact rapid-distortion theory and a strained-box direct
numerical simulation, the model reproduces the strained anisotropy and
exposes where a single line falls short. That shortfall is stated
exactly, rather than assumed, and traced to one missing ingredient that
a relaxation mechanism supplies. Carried through to leading-edge noise,
the evolved inflow shifts the predicted level by an amount the
frozen-spectrum assumption discards, of a size too large to neglect.
\end{abstract}
...
of mapping events interleaved with a continuous, one-dimensional
specialisation of viscous transport; at each event a kernel mechanism
redistributes energy among components, emulating pressure scrambling
\citep{WunschKerstein2001,Kerstein2001}.

Generalised to variable density
...
\item The measured limits of the model and a mechanism that extends them
...
% \end{enumerate}
\end{itemize}
The strain-coupled formulation of the first part modifies the model's
\emph{continuous} advancement, yielding the generalized momentum
equation \eqref{eq:cont-evolution}, and leaves the discrete eddy events
unchanged; the mechanism of the second part in turn modifies the
\emph{discrete} process, giving relaxation its own clock.
...
coincide. A prime marks the fluctuating part of a field where a mean
must be distinguished; the velocity fluctuation, the only velocity the
line carries, is written $u_i$ without a prime, and $p'$ is the
...
(figure~\ref{fig:odt-schematic}). Between events the fields
...
governed by the $Q_i$, with a rate constant, a viscous cutoff and a
...
Standard ODT carries no mean strain: between events the field obeys the
...
formulation generalises the \emph{continuous} advancement to carry the
distortion by an imposed mean velocity gradient $A_{ij}$, which is
...
acts---an exact production and a modelled rapid pressure--strain---while
leaving the eddy events unchanged.

Between eddy events the molecular evolution is replaced by
\begin{equation}
  \partial_t u_i
  = \nu\,\partial_y^2 u_i \;-\; A_{ij}\,u_j \;+\; B_{ij}\,u_j,
  \label{eq:cont-evolution}
\end{equation}
the line form of the fluctuation momentum equation for a spatially
uniform mean gradient: molecular diffusion as in standard ODT; the
exact production $-A_{ij}u_j$---a pointwise linear combination of the
local velocity components, exactly representable on the line, whose
contribution to the Reynolds-stress equation is $\mathcal{P}_{ij}$ of
\eqref{eq:rstransport}---and, in place of the fluctuating pressure
gradient, the modelled rapid pressure--strain operator $B_{ij}u_j$
constructed in \S\,\ref{sec:rapid-kernel}. The
...
The strain forcing preserves the structural properties on which the model
...
event exactly as in the standard model, so they hold verbatim. Both
$A_{ij}$, the imposed mean gradient, and $B_{ij}$, formed from the
line-averaged Reynolds stress (\S\,\ref{sec:line-realisation}), are
uniform along the line, so $\mathcal{A}_{ij}\equiv-A_{ij}+B_{ij}$ is a
constant matrix; over a non-diffusive sub-step it advances the profile
by the $y$-independent map
\begin{equation}
  u_i(y,t)=\Big[\exp\!\int_{t_0}^{t}\mathcal{A}\,\mathrm{d}t'\Big]_{ij}\,
  u_j(y,t_0),
  \label{eq:strain-map}
\end{equation}
a multiplication of the local velocity vector by a matrix set by the
accumulated strain, uniform along the line. Continuous profiles map
...
The production in \eqref{eq:cont-evolution} is exact; the rapid operator
$B_{ij}$ is not. Constructing it requires the Reynolds-stress and
...
Of the two, the production is supplied exactly on the line: the
pointwise forcing $-A_{ij}u_j$ of \eqref{eq:cont-evolution} evolves the
Reynolds stress at fixed domain length by
\begin{equation}
...
\end{equation}
the production tensor of \eqref{eq:rstransport}, with no model
assumption, so the kinetic-energy budget separates into exact
production, energy-conserving redistribution by the kernels, and
molecular dissipation. Only the rapid pressure--strain
$\Pi^{(\mathrm r)}_{ij}$, determined by the spectrum tensor, requires a
closure; its construction is the subject of the rest of this section.
...
\subsubsection{Closure of the pressure contribution}\label{sec:iso-closure}
...
\begin{equation}
  \Pi^{(\mathrm r),\mathrm{IP}}_{ij}
  = \tfrac{4}{5}\,k_t\,S_{ij}
  \;=\; -\tfrac{3}{5}\,\mathcal{P}_{ij}
  \quad\text{(isotropic $R_{ij}$)},
  \label{eq:crow}
\end{equation}
which is the exact response of initially isotropic turbulence to a weak rapid
...
isotropic limit of \eqref{eq:exact-rps}.

For anisotropic turbulence $\mathcal{R}_{ijmn}$ must be modelled as a
tensor function of $R_{ij}$; the general representation linear in the
anisotropy is the quasi-isotropic model of \citet{LaunderReeceRodi1975}
(LRR),
\begin{equation}
  \Pi^{(\mathrm r),\mathrm{QI}}_{ij} = C_2\,k_t S_{ij}
...
$C_2=4/5$ is fixed by requiring that \eqref{eq:lrr} reduce at isotropy
to the exact $\tfrac45k_tS_{ij}$ of \eqref{eq:crow}, so both closures
share the constant-free onset slope. The isotropy-based form
\eqref{eq:crow} (IP) is retained only for the onset consistency check
of \S\,\ref{sec:onset}; every numerically simulated result in the paper
uses the anisotropic closure \eqref{eq:lrr} (QI).
...
\begin{equation}
  ...
  \label{eq:iso-B}
\end{equation}
$B_{ij}$ is evaluated from the running Reynolds stress at each sub-step
and enters the continuous advancement \eqref{eq:cont-evolution}
alongside the production. Phenomenologically it represents the part of
the redistribution driven by the mean gradient, which proceeds
continuously at the strain rate, in contrast to the slow return to
isotropy that the eddy kernel supplies at the eddy rate. Strain-driven effects (production and
...
the mean-flow stretching on the mesh. This dilating mesh is the
physical-space counterpart of the deforming frame of
\citet{Rogallo1981} used by the benchmark direct simulation of
\S\,\ref{sec:box-dns}: the mean straining is carried by the frame, so
that in it the mean velocity vanishes pointwise and the line field
\emph{is} the full velocity. The eddy events therefore act on the full
velocity in the strained frame, and the kernel conservation laws
\eqref{eq:kernel-identities} hold per event with no mean-flow
correction---the strain enters the fluctuation only through the exact
production of \eqref{eq:cont-evolution}, never through eddy transport of
a mean gradient, which the irrotational (symmetric $A_{ij}$) strains
studied here do not present along the line.
...
mean straining without double counting. Because the dilatation compresses
the resolved scales together with the domain, the integral scale remains a
fixed fraction of the domain length under strain---constant to within
$2\%$ over the range studied ($e\le4$)---so the linear strain forcing does
not make the domain size a physical parameter of the strained state over
the reported window. Two limitations are inherent to
...
composing a \emph{continuous strain--diffusion phase on a dilating
domain} (step~(a)) with \emph{discrete eddy events} (step~(b)). As an
ordered cycle, the operational steps are:
\begin{enumerate}
\item[(a1)] \emph{strain forcing}---advance the velocity profile by the
  $y$-independent map \eqref{eq:strain-map}, whose generator
  $\mathcal{A}_{ij}=-A_{ij}+B_{ij}$ combines the exact production
  $-A_{ij}u_j$ with the modelled rapid pressure--strain $B_{ij}u_j$ of
  \eqref{eq:cont-evolution}, $B_{ij}$ being given by \eqref{eq:lyapunov}
  from the running Reynolds stress and the spectral closure (the LRR
  closure of \S\,\ref{sec:verification}, with the isotropic form
  \eqref{eq:iso-B} its onset limit);
\item[(a2)] \emph{molecular diffusion} \eqref{eq:odt-diffusion}, as in
  standard ODT;
\item[(a3)] \emph{domain straining}---dilate the mesh by
  \eqref{eq:dilatation}, carrying the wavevector kinematics; and
\item[(b)] \emph{discrete eddy events}---sampled from the standard rate
  distribution, applying the triplet map and slow kernel
  \eqref{eq:event}--\eqref{eq:ci}, unchanged.
\end{enumerate}
Step (a), the continuous advancement (a1)--(a3), carries the
strain-driven physics (the exact production and the rapid
pressure--strain); step (b), the discrete events, carries the
turbulence-driven physics (the cascade and the slow return to isotropy).
The strain coupling thus modifies only the continuous advancement and
leaves the discrete events unchanged; the discrete process is itself
modified later, by the relaxation mechanism of \S\,\ref{sec:limits}.
Standard ODT is recovered exactly when $A_{ij}=0$.
...
\subsection{The exact onset rate}\label{sec:onset}

The two canonical strains of \S\,\ref{sec:strain-forcing}---plane strain
\eqref{eq:plane-strain} and axisymmetric strain
\eqref{eq:axi-strain}---admit exact rapid-distortion solutions and
...
\begin{equation}
  ...
  = \mathcal{P}_{ij}+\Pi^{(\mathrm r),\mathrm{IP}}_{ij}
...
plane strain~\eqref{eq:plane-strain}, onset rates $\mp\tfrac{8}{15}k_ta$ for the stretched and
compressed components with the neutral component unchanged, and, under
axisymmetric strain~\eqref{eq:axi-strain}, $+\tfrac{4}{15}k_t\gamma$ for each lateral
...
than of the construction. For each canonical strain we integrate the
Reynolds-stress equation~\eqref{eq:rstransport} forward in time, closing
the rapid pressure--strain two ways---the isotropy-based form
\eqref{eq:crow} (IP) and the anisotropic closure \eqref{eq:lrr} (QI, or
LRR) of \S\,\ref{sec:iso-closure}---to obtain the component energies
$R_{ij}(e)$ as functions of the accumulated total strain
$e=\int_0^{t}S\,\mathrm{d}t'$, plotted below. The benchmark is the exact rapid-distortion
...
evolved modal covariance. This integration is independent of the IP and
LRR closures and reproduces the onset slope \eqref{eq:onset-rate} to within
...
\begin{figure}
  ...
  SC-ODT Reynolds-stress equation~\eqref{eq:rstransport} with the LRR closure \eqref{eq:lrr} and the IP
  closure \eqref{eq:crow}. Line style denotes the source (solid
...
\begin{figure}
  ...
  (lines; ensemble mean over $1000$ realisations, standard error within
  the line width) against the DNS of \citet{LeeReynolds1985} (open symbols, digitised
...
\begin{figure}
  ...
  strain: SC-ODT (solid, $1000$ realisations, grey standard-error band)
  against exact linear rapid distortion (long-dashed,
  wavevector-ensemble integration), the IP (dotted) and LRR
...
The two unstrained line spectra are isotropic and of comparable normalised
shape but not at matched Reynolds number---the $128^3$ box is a
low-Reynolds-number pilot, with a narrower spectrum than the resolved
ODT line---so it is their strain-induced change, referenced to each
system's own $e=0$ state, and not the absolute spectrum, that is compared.
...
\begin{equation}
  ...
  = \frac{\rho}{2\pi}\,A_{kl}\!\int
    ...
    \frac{\partial u_l}{\partial x'_k}(\boldsymbol{x}')\,
...
$\Pi^{(\mathrm r)}_{ij}
   = \overline{(p'_{\mathrm r}/\rho)\,(\partial u_i/\partial x_j
...
(the Fourier transform of $\overline{u_m(\boldsymbol{x})\,
u_n(\boldsymbol{x}+\boldsymbol{r})}$),
...
In the present formulation the line carries the fluctuation velocity
$\boldsymbol{u}'(y)$ about a prescribed uniform mean gradient---standard
ODT can also evolve a mean profile---on a single line aligned
...
and sets in at rapid strain. LRR, whose spanwise component is carried by
...
\begin{equation}
  ...
  \qquad
  f(\ell)
  = \frac{2L_p\,\ell^{-2}\,\mathrm{e}^{-2L_p/\ell}}
         {\mathrm{e}^{-2L_p/L_{\max}}-\mathrm{e}^{-2L_p/L_{\min}}},
...
(figure~\ref{fig:clock}\textit{b}). A single-realisation space--time
view of the line under standard ODT, SC-ODT and the clock-relaxed model,
in which the clock-relaxed line visibly retains finer structure at high
strain, is given in the supplementary material (\S\,S6).
...
same observables, the preliminary strain-coupled model, SC-ODT, carries
the correct integrated anisotropy but allocates it uniformly over wavenumber, while
...
sub-scales. The intervention broadly reproduces the phenomenology: depth that reaches the measured scales is necessary and
