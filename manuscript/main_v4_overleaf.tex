...
The paper falls into three parts. The strain-coupled formulation of the
first part modifies the model's \emph{continuous} advancement---the
...
part in turn modifies the \emph{discrete} process, giving relaxation its
own clock.
\begin{itemize}
...
exact production $-A_{ij}u_j$ (its contribution to the Reynolds-stress
equation is $\mathcal{P}_{ij}$ of \eqref{eq:rstransport}), and, in place of the
fluctuating pressure gradient, the modelled rapid pressure--strain
operator $B_{ij}u_j$ constructed in \S\,\ref{sec:rapid-kernel}. The
...
Reynolds-stress equation~\eqref{eq:rstransport} in time, closing the
rapid pressure--strain $\Pi^{(\mathrm r)}_{ij}$ two ways: the isotropisation-of-production
closure \eqref{eq:crow} (IP, with $C_2=3/5$) and the
