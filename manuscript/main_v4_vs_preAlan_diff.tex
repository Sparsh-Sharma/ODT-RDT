...
%DIF DEL C:/Users/shar_sp/AppData/Local/Temp/claude/C--Users-shar-sp-Documents-GitHub-ODT-RDT/ecedf73e-b62e-434b-9231-fb3e80255faa/scratchpad/old_preAlan.tex   Mon Sep 14 17:58:43 2026
%DIF ADD main_v4_overleaf.tex                                                                                                                                   Mon Sep 14 17:53:46 2026
%DIF < % =========================================================================
%DIF < %  main_v3.tex — the RECONSTRUCTED paper 1 (JFM-2026-1781 resubmission)
%DIF < %  Sharma & Kerstein.  Fresh skeleton, transplanted organs:
%DIF < %  V2 (Main_submission_V2.tex) is FROZEN as the quarry — transplant sources
%DIF < %  are named per section as  %% TRANSPLANT: <source> — <what/how much>.
%DIF < %  Structure follows docs/REVISION_MAP_jfm1781.md section 5 (spine = the
%DIF < %  single-line closure).  The LE-consequence section is DETACHABLE
%DIF < %  (Option C -> S) and is written LAST.
%DIF < %  Build: latexmk -pdf main_v3.tex
%DIF < % =========================================================================
%DIF -------
...
\hypersetup{
  ...
  citecolor=blue,
%DIF 24-25c13-14
%DIF <   linkcolor=jfmblue,
%DIF <   urlcolor=jfmblue
%DIF -------
  linkcolor=blue, %DIF > 
  urlcolor=blue %DIF > 
%DIF -------
...
%DIF < \renewcommand{\theassumption}{A\arabic{assumption}}
%DIF < 
%DIF < \newenvironment{assumption}[1]{%
%DIF <   \refstepcounter{assumption}%
%DIF <   \par\medskip
...
%DIF 50d20
%DIF < %% Title confirmed by Sparsh 2026-09-09 (physics-first hierarchy).
%DIF -------
\title{Strain-coupled one-dimensional turbulence: a single-line closure for rapid distortion, its measured limits, and the consequence for leading-edge noise}

\author{Sparsh Sharma\aff{1} \and Alan R. Kerstein\aff{2}}

%DIF 55-56c24-27
%DIF < \affiliation{\aff{1}German Aerospace Center (DLR), 38108 Braunschweig, Germany
%DIF < \aff{2}Consultant, Danville, CA 94526, USA}
%DIF -------
\affiliation{ %DIF > 
\aff{1}German Aerospace Center (DLR), 38108 Braunschweig, Germany %DIF > 
\aff{2}Consultant, Danville, CA 94526, USA %DIF > 
} %DIF > 
%DIF -------
...
\begin{abstract}
%DIF < % PROVISIONAL — rewrite last (map task M4).  Must claim ONLY:
%DIF < %  (1) strain-coupled ODT formulation, invariants preserved (V2 claims OK);
%DIF < %  (2) the single-line closure: exact obstruction stated, sharp LOS bounds,
%DIF < %      A1 measured (exact for axisymmetric distortion, quantified error
%DIF < %      under plane strain) — NOT "a closed functional";
%DIF < %  (3) the correct linear reference: exact projected RDT is broadband, NOT
%DIF < %      rigid translation (Referee 3.1 was right; own calculation);
%DIF < %  (4) independent spectral benchmark: strained-box DNS + exact-RDT
%DIF < %      companions, three-way comparison; kappa-uniformity deficiency
%DIF < %      measured, distance-from-linear-theory envelope vs Sk/eps;
%DIF < %  (5) scale-local relaxation (hierarchical kernels) as the measured
%DIF < %      constructive direction.  NO claim that the deficiency is fixed.
\DIFdelbegin \DIFdel{The rapid distortion of turbulence by a mean strain is central to flows
...
}\DIFdelend \DIFaddbegin \DIFadd{xxxx
}\DIFaddend \end{abstract}

\begin{keywords}
%DIF < % chosen at submission
\DIFaddbegin \DIFadd{one-dimensional turbulence; leading-edge noise; turbulence modelling
}\DIFaddend \end{keywords}

%DIF <  =========================================================================
\section{Introduction}\label{sec:intro}
%DIF <  =========================================================================
%DIF < % REWRITTEN 2026-09-08 (M4): V2 motivation compressed ~60%; novelty
%DIF < %   statement honest (dosSantos2023/Ribeiro2023/Piccolo2024 cited per
%DIF < %   R1); Sagaut & Cambon 2018 as the RDT spine (R2); new arc paragraph.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The distortion of turbulence by a mean velocity gradient is among the
...
nonlinear relaxation together, at negligible cost.
...
they reproduce a prescribed spectrum rather than evolving one. A model that \DIFdelbegin \emph{\DIFdel{predicts}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{predicts }\DIFaddend the distorted spectrum, including the nonlinear
relaxation that linear transforms omit, \DIFdelbegin \DIFdel{is the missing element---here
}\DIFdelend \DIFaddbegin \DIFadd{would supply an element that is
at present missing---here }\DIFaddend and wherever distorted turbulence feeds a
downstream prediction, from contraction design to the turbulence ingested
by rotors. The subject
...
\citep{SharmaKleinSchmidt2022,MedinaMendez2023}, ODT is an accurate and
inexpensive generator of scale-resolved turbulence \DIFdelbegin \DIFdel{statistics---exactly
what a model of strained turbulence must deliver}\DIFdelend \DIFaddbegin \DIFadd{statistics}\DIFaddend . But standard ODT
...
The paper \DIFdelbegin \DIFdel{makes six contributions, each stated so as to be falsifiable.
First, a }\DIFdelend \DIFaddbegin \DIFadd{reports six results the following aspects of model formulation and assessment:
}\begin{itemize}
\item \DIFadd{A }\DIFaddend strain-coupled formulation of ODT (SC-ODT\DIFdelbegin \DIFdel{, hereafter}\DIFdelend )
(\S\,\ref{sec:formulation}): production enters as an exact continuous
forcing, the rapid pressure--strain as an energy-conserving redistribution
operator built from an explicit spectral closure, and the wavevector
kinematics as a dilatation of the domain\DIFdelbegin \DIFdel{; the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend construction preserves the
model's structural invariants and\DIFaddbegin \DIFadd{, under the stated closure, }\DIFaddend reproduces
the rapid-distortion evolution of the component energies \DIFdelbegin \DIFdel{, under the stated closure, }\DIFdelend with no adjustable
constant (\S\,\ref{sec:verification}).
\DIFdelbegin \DIFdel{Second, the correct linear reference for what the modelactually predicts}\DIFdelend \DIFaddbegin 

\item \DIFadd{Validation against the strained-turbulence experiment of \mbox{%DIFAUXCMD
\citet{ChenMeneveauKatz2006}}\hskip0pt%DIFAUXCMD
~(CMK), summarized in (\S\,\ref{sec:cmk}) Sec. 4.5 with details provided in
\textcolor{red}{Supplementary Material (Sec. S1)}.
}

\item \DIFadd{The linear reference appropriate to the model's observables}\DIFaddend : exact
RDT \DIFdelbegin \DIFdel{, }\DIFdelend projected onto a line \DIFdelbegin \DIFdel{, is }\emph{\DIFdel{broadband}}%DIFAUXCMD
\DIFdel{---it does not translate the projected
spectra rigidly---so the }\DIFdelend \DIFaddbegin \DIFadd{is broadband rather than a rigid spectral
translation, so the }\DIFaddend reference against which any line model must be judged
\DIFaddbegin \DIFadd{already }\DIFaddend differs from the model's own \DIFdelbegin \DIFdel{rigid kinematics already }\DIFdelend \DIFaddbegin \DIFadd{kinematics }\DIFaddend at the linear level
(\S\,\ref{sec:spec-linref}).
\DIFdelbegin \DIFdel{Third, an }\DIFdelend \DIFaddbegin 

\item \DIFadd{An }\DIFaddend independent spectral benchmark \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{consisting of }\DIFaddend a strained-box direct
numerical simulation \DIFaddbegin \DIFadd{together }\DIFaddend with exact-RDT companion calculations,
reduced to the same one-dimensional observables\DIFdelbegin \DIFdel{,
yielding a }\DIFdelend \DIFaddbegin \DIFadd{. A }\DIFaddend three-way comparison
\DIFdelbegin \DIFdel{that }\DIFdelend localises the model's remaining deficiency in its wavenumber-uniform rapid
kinematics and \DIFaddbegin \DIFadd{supplies }\DIFaddend a distance-from-linear-theory envelope as a
function of strain rapidity $Sk_t/\varepsilon$ \DIFdelbegin \DIFdel{, the strain rate in eddy-turnover units
}\DIFdelend (\S\,\ref{sec:box-dns}).
\DIFdelbegin \DIFdel{Fourth, an answer to the question of principle---what can }\DIFdelend \DIFaddbegin 

\item \DIFadd{A treatment of what }\DIFaddend a single line \DIFdelbegin \DIFdel{say }\DIFdelend \DIFaddbegin \DIFadd{can determine }\DIFaddend about the rapid
pressure--strain term \DIFdelbegin \DIFdel{, which is exactly }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend a non-local three-dimensional object\DIFdelbegin \DIFdel{?---in a form
deliberately more modest than a closure claim: the }\DIFdelend \DIFaddbegin \DIFadd{). The
}\DIFaddend obstruction is stated exactly, sharp bounds \DIFdelbegin \DIFdel{on the rapid term are derived }\DIFdelend \DIFaddbegin \DIFadd{are obtained }\DIFaddend from line data
alone, and the axisymmetry hypothesis \DIFdelbegin \DIFdel{under which the term collapses is not assumedbut }\emph{\DIFdel{measured}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{is measured rather than assumed: it
is }\DIFaddend exact for axisymmetric distortion and \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{carries }\DIFaddend a quantified error under
plane strain (\S\,\ref{sec:gate}).
\DIFdelbegin \DIFdel{Fifth,
the }\DIFdelend \DIFaddbegin 

\item \DIFadd{The }\DIFaddend measured limits of the model and \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend mechanism that moves them
(\S\,\ref{sec:limits})\DIFdelbegin \DIFdel{: a }\DIFdelend \DIFaddbegin \DIFadd{. A }\DIFaddend transmission diagnostic shows the strain-induced
anisotropy crossing two triplet-map \DIFdelbegin \DIFdel{generations---a
}\DIFdelend \DIFaddbegin \DIFadd{generations (a }\DIFaddend factor of nine in
\DIFdelbegin \DIFdel{scale---undiminished,
and }\DIFdelend \DIFaddbegin \DIFadd{scale) undiminished; }\DIFaddend a controlled family of interventions \DIFdelbegin \DIFdel{falsifies }\DIFdelend \DIFaddbegin \DIFadd{rules out
}\DIFaddend acceptance gating and map reweighting as remedies. Hierarchical application
of the energy-redistributing kernel to eddy \DIFdelbegin \DIFdel{sub-scales---scale-local
relaxation---supplies the missing ingredient where it reaches the
}\DIFdelend \DIFaddbegin \DIFadd{sub-scales supplies scale-local
relaxation at the }\DIFaddend scales of the imbalance\DIFdelbegin \DIFdel{; the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend mechanism's \DIFdelbegin \DIFdel{own }\DIFdelend deep-strain fade
is traced to its event-locked timing and \DIFaddbegin \DIFadd{is }\DIFaddend removed by an independent,
size-capped relaxation clock, leaving a single measured allocation bound.
\DIFdelbegin \DIFdel{Sixth, the
}\DIFdelend \DIFaddbegin 

\item \DIFadd{The }\DIFaddend consequence for the motivating application
(\S\,\ref{sec:lenoise}): the computed distorted spectrum is supplied to
Amiet's leading-edge-noise theory in place of the frozen inflow of standard
practice, both as a ratio from which aerofoil geometry and observer cancel
\DIFdelbegin \DIFdel{exactly }\DIFdelend and as an \DIFdelbegin \DIFdel{assembled }\DIFdelend absolute prediction at the reference operating point\DIFdelbegin \DIFdel{, quantifying }\DIFdelend \DIFaddbegin \DIFadd{. The
calculation quantifies }\DIFaddend the $5$--$20$\,dB term \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{omitted by }\DIFaddend prescribed-spectrum
practice\DIFdelbegin \DIFdel{omits}\DIFdelend ; the assembled chain is \DIFdelbegin \DIFdel{confronted }\DIFdelend \DIFaddbegin \DIFadd{compared }\DIFaddend with four published experiments
\DIFdelbegin \DIFdel{, which
it reproduces to }\DIFdelend \DIFaddbegin \DIFadd{and lies within }\DIFaddend $1$--$2$\,dB rms where the facilities are cleanest.
\DIFdelbegin \DIFdel{Throughout, the posture is the same: }\DIFdelend \DIFaddbegin \end{itemize}
\DIFadd{Throughout we show }\DIFaddend where the model \DIFdelbegin \DIFdel{is right we show
against what, andwhere it is wrong we measure }\DIFdelend \DIFaddbegin \DIFadd{agrees with reference data and, where it
does not, }\DIFaddend by how much and \DIFdelbegin \DIFdel{locate
why}\DIFdelend \DIFaddbegin \DIFadd{for what reason}\DIFaddend .


The paper falls into three parts\DIFdelbegin \DIFdel{. The first is the }\DIFdelend \DIFaddbegin \DIFadd{:
}\begin{itemize}
\item \DIFadd{The }\DIFaddend model and what a line can know: \S\,\ref{sec:formulation}
reviews standard ODT and derives the strain-coupled formulation;
\S\,\ref{sec:verification} verifies it against exact rapid-distortion
solutions and validates it at the moment level against the DNS of
\citet{LeeReynolds1985}; \S\,\ref{sec:val-spectrum} constructs the linear
reference, examines the emergent distorted spectrum and its operating
point, and carries out the strained-box DNS benchmark; \S\,\ref{sec:gate}
treats the single-line closure question.
\DIFdelbegin \DIFdel{The second part,
}\DIFdelend \DIFaddbegin 

\item \DIFaddend \S\,\ref{sec:limits} \DIFdelbegin \DIFdel{, }\DIFdelend measures the model's limits and establishes
scale-local relaxation---timed by its own clock---as the mechanism that
moves them.
\DIFdelbegin \DIFdel{The third part, }\DIFdelend \DIFaddbegin 

\item \DIFaddend \S\,\ref{sec:lenoise} \DIFdelbegin \DIFdel{, carries }\DIFdelend \DIFaddbegin \DIFadd{applies }\DIFaddend the computed distortion \DIFaddbegin \DIFadd{as a
correction }\DIFaddend to the leading-edge-noise input.
%DIF >  \end{enumerate}
\DIFaddbegin \end{itemize}
\DIFaddend Section~\ref{sec:conclusions} concludes. Supplementary material contains
the validation against the strained-turbulence experiment of
\citet{ChenMeneveauKatz2006} \DIFaddbegin \DIFadd{(CMK) }\DIFaddend and the complete intervention family
behind the mechanism ledger.

% =========================================================================
\section{Strain-coupled one-dimensional turbulence}\label{sec:formulation}
% =========================================================================
%DIF < % TRANSPLANTED 2026-09-06 from V2 (sec:odt compressed to one review
%DIF < %   subsection per R2; sec:rapid-slow trimmed; sec:formulation ~80%;
%DIF < %   Propositions 1-2 demoted to consistency checks per R1 minor).
...
on a one-dimensional line; that incompatibility \DIFdelbegin \DIFdel{organises }\DIFdelend \DIFaddbegin \DIFadd{is the focus of }\DIFaddend this section.
...
(\S\,\ref{sec:domain-strain}). The slow, turbulence-driven pressure--strain and the inertial cascade
\DIFdelbegin \DIFdel{remain }\DIFdelend \DIFaddbegin \DIFadd{are captured by }\DIFaddend the discrete eddy events of the standard model, reviewed first.
...
field $u_i(y,t)$ about this mean\DIFaddbegin \DIFadd{, so that the total velocity is
$U_i+u_i$ with $\overline{u_i}=0$}\DIFaddend ; $R_{ij}\equiv\overline{u_iu_j}$ is the
...
\begin{scope}[shift={(0,0)}]
  \fill[black!6] (1.4,1.70) rectangle (2.9,4.85);
  \draw[black!25] (1.4,1.70) -- (1.4,4.85);
  \draw[black!25] (2.9,1.70) -- (2.9,4.85);
  \node[font=\fontsize{8}{8}\selectfont] at (1.4,5.06) {$y_0$};
  \node[font=\fontsize{8}{8}\selectfont] at (2.9,5.06) {$y_0+l$};
  ...
  \draw[black!40, line width=0.5pt] plot coordinates {(0.250,4.615) (0.263,4.615) (0.277,4.616) (0.290,4.616) (0.303,4.616) (0.317,4.615) (0.330,4.615) (0.343,4.614) (0.357,4.613) (0.370,4.612) (0.383,4.610) (0.397,4.609) (0.410,4.607) (0.423,4.605) (0.436,4.603) (0.450,4.601) (0.463,4.599) (0.476,4.597) (0.490,4.595) (0.503,4.592) (0.516,4.590) (0.530,4.588) (0.543,4.586) (0.556,4.583) (0.570,4.581) (0.583,4.579) (0.596,4.577) (0.610,4.574) (0.623,4.572) (0.636,4.570) (0.650,4.569) (0.663,4.567) (0.676,4.565) (0.690,4.564) (0.703,4.562) (0.716,4.561) (0.730,4.560) (0.743,4.559) (0.756,4.558) (0.769,4.558) (0.783,4.557) (0.796,4.557) (0.809,4.557) (0.823,4.556) (0.836,4.557) (0.849,4.557) (0.863,4.557) (0.876,4.558) (0.889,4.558) (0.903,4.559) (0.916,4.560) (0.929,4.561) (0.943,4.562) (0.956,4.563) (0.969,4.564) (0.983,4.565) (0.996,4.566) (1.009,4.567) (1.023,4.568) (1.036,4.570) (1.049,4.571) (1.063,4.572) (1.076,4.573) (1.089,4.574) (1.103,4.575) (1.116,4.576) (1.129,4.577) (1.142,4.578) (1.156,4.578) (1.169,4.579) (1.182,4.579) (1.196,4.579) (1.209,4.579) (1.222,4.579) (1.236,4.578) (1.249,4.577) (1.262,4.577) (1.276,4.575) (1.289,4.574) (1.302,4.572) (1.316,4.571) (1.329,4.569) (1.342,4.566) (1.356,4.564) (1.369,4.561) (1.382,4.558) (1.396,4.554) (1.409,4.551) (1.422,4.547) (1.436,4.543) (1.449,4.539) (1.462,4.534) (1.475,4.530) (1.489,4.525) (1.502,4.519) (1.515,4.514) (1.529,4.509) (1.542,4.503) (1.555,4.497) (1.569,4.491) (1.582,4.485) (1.595,4.479) (1.609,4.473) (1.622,4.466) (1.635,4.460) (1.649,4.453) (1.662,4.447) (1.675,4.441) (1.689,4.434) (1.702,4.428) (1.715,4.421) (1.729,4.415) (1.742,4.409) (1.755,4.402) (1.769,4.396) (1.782,4.390) (1.795,4.385) (1.808,4.379) (1.822,4.374) (1.835,4.368) (1.848,4.363) (1.862,4.358) (1.875,4.354) (1.888,4.349) (1.902,4.345) (1.915,4.342) (1.928,4.338) (1.942,4.335) (1.955,4.332) (1.968,4.329) (1.982,4.326) (1.995,4.324) (2.008,4.323) (2.022,4.321) (2.035,4.320) (2.048,4.319) (2.062,4.318) (2.075,4.318) (2.088,4.318) (2.102,4.318) (2.115,4.318) (2.128,4.319) (2.142,4.320) (2.155,4.321) (2.168,4.323) (2.181,4.325) (2.195,4.327) (2.208,4.329) (2.221,4.331) (2.235,4.334) (2.248,4.336) (2.261,4.339) (2.275,4.342) (2.288,4.345) (2.301,4.349) (2.315,4.352) (2.328,4.355) (2.341,4.359) (2.355,4.362) (2.368,4.366) (2.381,4.369) (2.395,4.373) (2.408,4.376) (2.421,4.380) (2.435,4.383) (2.448,4.387) (2.461,4.390) (2.475,4.393) (2.488,4.396) (2.501,4.399) (2.514,4.402) (2.528,4.405) (2.541,4.408) (2.554,4.410) (2.568,4.412) (2.581,4.415) (2.594,4.417) (2.608,4.419) (2.621,4.420) (2.634,4.422) (2.648,4.423) (2.661,4.425) (2.674,4.426) (2.688,4.427) (2.701,4.428) (2.714,4.428) (2.728,4.429) (2.741,4.429) (2.754,4.429) (2.768,4.429) (2.781,4.429) (2.794,4.429) (2.808,4.429) (2.821,4.429) (2.834,4.429) (2.847,4.428) (2.861,4.428) (2.874,4.428) (2.887,4.427) (2.901,4.427) (2.914,4.426) (2.927,4.426) (2.941,4.425) (2.954,4.425) (2.967,4.425) (2.981,4.425) (2.994,4.425) (3.007,4.425) (3.021,4.425) (3.034,4.425) (3.047,4.425) (3.061,4.426) (3.074,4.427) (3.087,4.428) (3.101,4.429) (3.114,4.430) (3.127,4.431) (3.141,4.433) (3.154,4.435) (3.167,4.437) (3.181,4.439) (3.194,4.442) (3.207,4.445) (3.220,4.448) (3.234,4.451) (3.247,4.454) (3.260,4.458) (3.274,4.462) (3.287,4.466) (3.300,4.470) (3.314,4.474) (3.327,4.479) (3.340,4.484) (3.354,4.489) (3.367,4.494) (3.380,4.499) (3.394,4.504) (3.407,4.510) (3.420,4.516) (3.434,4.521) (3.447,4.527) (3.460,4.533) (3.474,4.539) (3.487,4.545) (3.500,4.551) (3.514,4.556) (3.527,4.562) (3.540,4.568) (3.553,4.574) (3.567,4.580) (3.580,4.585) (3.593,4.591) (3.607,4.596) (3.620,4.601) (3.633,4.607) (3.647,4.611) (3.660,4.616) (3.673,4.621) (3.687,4.625) (3.700,4.629)};
  \draw[black, line width=0.6pt] plot coordinates {(0.250,2.685) (0.263,2.685) (0.277,2.686) (0.290,2.686) (0.303,2.686) (0.317,2.685) (0.330,2.685) (0.343,2.684) (0.357,2.683) (0.370,2.682) (0.383,2.680) (0.397,2.679) (0.410,2.677) (0.423,2.675) (0.436,2.673) (0.450,2.671) (0.463,2.669) (0.476,2.667) (0.490,2.665) (0.503,2.662) (0.516,2.660) (0.530,2.658) (0.543,2.656) (0.556,2.653) (0.570,2.651) (0.583,2.649) (0.596,2.647) (0.610,2.644) (0.623,2.642) (0.636,2.640) (0.650,2.639) (0.663,2.637) (0.676,2.635) (0.690,2.634) (0.703,2.632) (0.716,2.631) (0.730,2.630) (0.743,2.629) (0.756,2.628) (0.769,2.628) (0.783,2.627) (0.796,2.627) (0.809,2.627) (0.823,2.626) (0.836,2.627) (0.849,2.627) (0.863,2.627) (0.876,2.628) (0.889,2.628) (0.903,2.629) (0.916,2.630) (0.929,2.631) (0.943,2.632) (0.956,2.633) (0.969,2.634) (0.983,2.635) (0.996,2.636) (1.009,2.637) (1.023,2.638) (1.036,2.640) (1.049,2.641) (1.063,2.642) (1.076,2.643) (1.089,2.644) (1.103,2.645) (1.116,2.646) (1.129,2.647) (1.142,2.648) (1.156,2.648) (1.169,2.649) (1.182,2.649) (1.196,2.649) (1.209,2.649) (1.222,2.649) (1.236,2.648) (1.249,2.647) (1.262,2.647) (1.276,2.645) (1.289,2.644) (1.302,2.642) (1.316,2.641) (1.329,2.639) (1.342,2.636) (1.356,2.634) (1.369,2.631) (1.382,2.628) (1.396,2.624) (1.409,2.616) (1.422,2.603) (1.436,2.588) (1.449,2.571) (1.462,2.553) (1.475,2.534) (1.489,2.515) (1.502,2.495) (1.515,2.476) (1.529,2.458) (1.542,2.442) (1.555,2.427) (1.569,2.414) (1.582,2.404) (1.595,2.396) (1.609,2.391) (1.622,2.388) (1.635,2.388) (1.649,2.391) (1.662,2.395) (1.675,2.402) (1.689,2.410) (1.702,2.420) (1.715,2.430) (1.729,2.440) (1.742,2.451) (1.755,2.461) (1.769,2.470) (1.782,2.478) (1.795,2.485) (1.808,2.491) (1.822,2.495) (1.835,2.498) (1.848,2.499) (1.862,2.499) (1.875,2.499) (1.888,2.498) (1.902,2.497) (1.915,2.498) (1.928,2.499) (1.942,2.499) (1.955,2.499) (1.968,2.497) (1.982,2.494) (1.995,2.490) (2.008,2.484) (2.022,2.476) (2.035,2.468) (2.048,2.458) (2.062,2.448) (2.075,2.438) (2.088,2.427) (2.102,2.417) (2.115,2.408) (2.128,2.400) (2.142,2.394) (2.155,2.390) (2.168,2.388) (2.181,2.388) (2.195,2.392) (2.208,2.398) (2.221,2.406) (2.235,2.417) (2.248,2.431) (2.261,2.446) (2.275,2.463) (2.288,2.481) (2.301,2.500) (2.315,2.520) (2.328,2.539) (2.341,2.558) (2.355,2.576) (2.368,2.592) (2.381,2.606) (2.395,2.619) (2.408,2.617) (2.421,2.604) (2.435,2.589) (2.448,2.572) (2.461,2.554) (2.475,2.536) (2.488,2.516) (2.501,2.497) (2.514,2.478) (2.528,2.460) (2.541,2.443) (2.554,2.428) (2.568,2.415) (2.581,2.404) (2.594,2.396) (2.608,2.391) (2.621,2.388) (2.634,2.388) (2.648,2.390) (2.661,2.395) (2.674,2.402) (2.688,2.410) (2.701,2.419) (2.714,2.429) (2.728,2.440) (2.741,2.450) (2.754,2.460) (2.768,2.470) (2.781,2.478) (2.794,2.485) (2.808,2.491) (2.821,2.495) (2.834,2.498) (2.847,2.499) (2.861,2.499) (2.874,2.499) (2.887,2.498) (2.901,2.497) (2.914,2.496) (2.927,2.496) (2.941,2.495) (2.954,2.495) (2.967,2.495) (2.981,2.495) (2.994,2.495) (3.007,2.495) (3.021,2.495) (3.034,2.495) (3.047,2.495) (3.061,2.496) (3.074,2.497) (3.087,2.498) (3.101,2.499) (3.114,2.500) (3.127,2.501) (3.141,2.503) (3.154,2.505) (3.167,2.507) (3.181,2.509) (3.194,2.512) (3.207,2.515) (3.220,2.518) (3.234,2.521) (3.247,2.524) (3.260,2.528) (3.274,2.532) (3.287,2.536) (3.300,2.540) (3.314,2.544) (3.327,2.549) (3.340,2.554) (3.354,2.559) (3.367,2.564) (3.380,2.569) (3.394,2.574) (3.407,2.580) (3.420,2.586) (3.434,2.591) (3.447,2.597) (3.460,2.603) (3.474,2.609) (3.487,2.615) (3.500,2.621) (3.514,2.626) (3.527,2.632) (3.540,2.638) (3.553,2.644) (3.567,2.650) (3.580,2.655) (3.593,2.661) (3.607,2.666) (3.620,2.671) (3.633,2.677) (3.647,2.681) (3.660,2.686) (3.673,2.691) (3.687,2.695) (3.700,2.699)};
  \draw[jfmblue, line width=1.0pt] plot coordinates {(1.902,2.497) (1.915,2.498) (1.928,2.499) (1.942,2.499) (1.955,2.499) (1.968,2.497) (1.982,2.494) (1.995,2.490) (2.008,2.484) (2.022,2.476) (2.035,2.468) (2.048,2.458) (2.062,2.448) (2.075,2.438) (2.088,2.427) (2.102,2.417) (2.115,2.408) (2.128,2.400) (2.142,2.394) (2.155,2.390) (2.168,2.388) (2.181,2.388) (2.195,2.392) (2.208,2.398) (2.221,2.406) (2.235,2.417) (2.248,2.431) (2.261,2.446) (2.275,2.463) (2.288,2.481) (2.301,2.500) (2.315,2.520) (2.328,2.539) (2.341,2.558) (2.355,2.576) (2.368,2.592) (2.381,2.606) (2.395,2.619)};
  \draw[ev,black] (1.00,3.85) -- (1.00,3.05);
  \node[anchor=west,text width=1.55cm,align=left] at (1.14,3.45)
       {instantaneous eddy event};
  \draw[densely dotted] (1.9,2.90) -- (1.9,1.80);
  \draw[densely dotted] (2.4,2.90) -- (2.4,1.80);
  \node[anchor=west] at (0.10,1.34) {$K(y)=y-f(y)$};
  \draw[black!18] (1.4,0.85) -- (2.9,0.85);
  \draw[black, line width=0.5pt] plot coordinates {(1.4,0.85) (1.9,0.65) (2.4,1.05) (2.9,0.85)};
  ...
  \node at (1.65,2.02) {1};
  \node[jfmblue] at (2.15,2.02) {2};
  \node at (2.65,2.02) {3};
  \node[font=\itshape\fontsize{7.2}{8}\selectfont,jfmblue] at (2.15,1.78) {reversed};
  \draw[-{Latex[length=1.2mm,width=0.9mm]},jfmblue,line width=0.5pt]
       (2.52,1.86) .. controls (2.58,2.20) .. (2.33,2.42);
\end{scope}
...
\begin{scope}[shift={(9.05,0)}]
  ...
  \node[box]              (b2) at (1.35,2.95) {sample candidate eddy $(y_0,l)$; infer timescale $\tau$ from the local field};
  ...
  \draw[ev] (b1) -- (b2);
  \draw[ev] (b2) -- (b3);
  \draw[ev] (b3) -- (b4) node[midway,anchor=west,font=\itshape\fontsize{7.2}{8}\selectfont,black!60] {accept};
...
\end{scope}
\end{tikzpicture}
  \caption{\DIFaddFL{The model at a glance.
(}\textit{\DIFaddFL{a}}\DIFaddFL{) Instantaneous eddy event on $[y_0,y_0+l]$: the triplet map compresses and triplicates the local field; the additive kernel $K(y)$ redistributes energy among components, both conserving momentum and total energy.
(}\textit{\DIFaddFL{b}}\DIFaddFL{) Eddy events (Poisson), the vertical bars marking the spatial range of each event, punctuating the continuous diffusion and mean-strain forcing (grey arrows).
(}\textit{\DIFaddFL{c}}\DIFaddFL{) Advancement cycle at a state-dependent rate; the $k^{-5/3}$ cascade emerges as an outcome of repeated sampling, consistent with inertial-range phenomenology.}}
  \label{fig:odt-schematic}
\end{figure}
\DIFaddend This subsection reviews the one-dimensional turbulence model of
...
Two interleaved mechanisms advance the state
\DIFaddbegin \DIFadd{(figure~\ref{fig:odt-schematic})}\DIFaddend : between events the fields
...
standard eddy time-scale construction \DIFaddbegin \DIFadd{applied to the quantities $Q_i$}\DIFaddend , with a rate constant, a viscous
...
$\Pi^{(\mathrm r)}_{ij}$ is \DIFdelbegin \DIFdel{essential, not optional}\DIFdelend \DIFaddbegin \DIFadd{necessary}\DIFaddend .
...
$\Pi^{(\mathrm r)}_{ij}$ \DIFdelbegin \DIFdel{,
...
One point of identification \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{should }\DIFaddend to be kept in view throughout this section
...
figures \DIFdelbegin \DIFdel{write }\DIFdelend \DIFaddbegin \DIFadd{refer to }\DIFaddend ``SC-ODT'' they mean this formulation; ``standard ODT''
...
no adjustable constant. This is \DIFdelbegin \DIFdel{the sharpest verification available: }\DIFdelend a
closed-form prediction the model must satisfy identically\DIFdelbegin \DIFdel{, not
approximately}\DIFdelend . Evaluated for the two canonical strains it gives, under
...
bounds the accuracy attainable from any single rapid operator. Third, \DIFdelbegin \DIFdel{and most important to state plainly, }\DIFdelend for plane strain both closures miss
...
\begin{figure}
  ...
  the LRR and IP closures, and production alone ($B_{ij}=0$). 
  \DIFdelbeginFL \DIFdelFL{Production
  ...
  the rapid term restores the physical, bounded amplification.}\DIFdelendFL %DIF > Production without the rapid term overstates the onset amplification rate by $\tfrac52$ and forces almost all the energy into the upwash component; the rapid term restores the physical, bounded amplification.
  }
...
\emph{closure}, and it should---with the events \DIFaddbegin \DIFadd{and diffusion }\DIFaddend suppressed the line
...
exact solution is quantified \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{here }\DIFaddend at every strain reached; the
sensitivity of the spectral conclusions to this closure choice is not
estimated but measured, through the exact-RDT \DIFdelbegin \DIFdel{companions }\DIFdelend \DIFaddbegin \DIFadd{comparisons }\DIFaddend of
...
uncertainty rather than leaving it implicit. \DIFdelbegin \DIFdel{Reaching that figure required }\DIFdelend \DIFaddbegin \DIFadd{That assessment requires }\DIFaddend a resolved spectral
...
isotropically, so the energy panel is what \DIFdelbegin \DIFdel{exposes }\DIFdelend \DIFaddbegin \DIFadd{distinguishes }\DIFaddend them.
...
the \DIFdelbegin \DIFdel{supplementary }\DIFdelend CMK comparison; higher-Reynolds-number strained
...
they reach ($c\le1.65$\DIFaddbegin \DIFadd{, where $c=\exp\int S\,\mathrm{d}t$ is the deformation ratio, and $\ln c$ is the total strain $e$}\DIFaddend ). A direct simulation of the
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeauthor{ChenMeneveauKatz2006} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{CMK }\DIFaddend straining cycle
...
The
comparison \DIFdelbegin \DIFdel{quantity is the }\DIFdelend \DIFaddbegin \DIFadd{is based on the $c$ dependence of the }\DIFaddend anisotropy $b_{ij}$ \DIFdelbegin \DIFdel{against }\DIFdelend \DIFaddbegin \DIFadd{as a function of }\DIFaddend the deformation
ratio\DIFdelbegin \DIFdel{$c=\exp\int S\,\mathrm{d}t$ (so $\ln c$ is the total strain $e$ of
the present notation), }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > $c=\exp\int S\,\mathrm{d}t$ (so $\ln c$ is the total strain $e$ of the present notation), 
}\DIFaddend computed from the single-point component
...
$\exp(A_{22}t_f)$\DIFaddbegin \DIFadd{, where $t_f$ is the total straining time, }\DIFaddend to better than $0.3\%$, confirming the kinematic
...
breaking the transverse symmetry with a residual split
$b_{22}=+0.102$ against $b_{33}=+0.113$ that the symmetry-respecting
mean $\tfrac12(b_{22}+b_{33})$ removes. Figure~\ref{fig:lr-axisym-fig}
...
\begin{figure}
  ...
  SC-ODT (lines; ensemble mean over $1000$ realisations, standard-error
  band) against the DNS of \citet{LeeReynolds1985} (open markers) and the
  ...
  growth of both, and the two DNS agree where they overlap\DIFdelbeginFL \DIFdelFL{; the
  magnitude is under-predicted at large strain by the closure gap common
  to the second-moment family (\S\,\ref{sec:finite-strain}), not by the
  line}\DIFdelendFL .}
...
family's known limitation \DIFaddbegin \DIFadd{(\S\,\ref{sec:finite-strain}) }\DIFaddend rather than any line-specific artefact.
...
\begin{figure}
  ...
  strain: SC-ODT (solid, $1000$ realisations, standard-error band)
...
(\S\S\,\ref{sec:spec-bvc}--\ref{sec:spec-op}); and it closes with the
benchmark that \DIFdelbegin \DIFdel{meets }\DIFdelend \DIFaddbegin \DIFadd{addresses }\DIFaddend the model-versus-itself \DIFdelbegin \DIFdel{objection head on---a
}\DIFdelend \DIFaddbegin \DIFadd{comparison---a
}\DIFaddend strained-box DNS with an exact rapid-distortion companion, reduced to
...
combination \DIFdelbegin \DIFdel{transfers no energy between scales: }\DIFdelend \DIFaddbegin \DIFadd{induces }\DIFaddend a rigid translation of the
line spectrum towards higher wavenumber \DIFaddbegin \DIFadd{that transfer no energy between scales}\DIFaddend . We emphasise at the outset that this
rigid translation is a property of the SC-ODT \emph{line}---uniform
...
\DIFaddbegin \begin{figure}
  ...
  \caption{\DIFaddFL{Compression without cascade (eddy events suppressed, inviscid).
  (}\textit{\DIFaddFL{a}}\DIFaddFL{) Component line spectra $E_i(\kappa_2)$ at total strains
  $e=0,1,2,3,3.9$ (lighter to darker); the spectra translate to higher
  wavenumber without changing shape.
  (}\textit{\DIFaddFL{b}}\DIFaddFL{) Spectral-centroid migration $\bar\kappa(e)/\bar\kappa(0)$
  ...
  \label{fig:spec-rdt}
\end{figure}
\DIFaddend Figure~\ref{fig:spec-rdt} confirms it. The component spectra translate toward
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\textwidth]{Figures/fig_spec_kinematic.pdf}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Compression without cascade (eddy events suppressed, inviscid).
  (}\textit{\DIFdelFL{a}}%DIFAUXCMD
\DIFdelFL{) Component line spectra $E_i(\kappa_2)$ at total strains
  $e=0,1,2,3,3.9$ (lighter to darker); the spectra translate to higher
  wavenumber without changing shape.
  (}\textit{\DIFdelFL{b}}%DIFAUXCMD
\DIFdelFL{) Spectral-centroid migration $\bar\kappa(e)/\bar\kappa(0)$
  for each component (symbols) against the kinematic prediction
  $\mathrm{e}^{-A_{22}e}$ (dashed). The centroid follows the prediction
  and the three components coincide, confirming purely geometric
  compression of the line.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:spec-rdt}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %% TRANSPLANTED 2026-09-07: sec:spec-bvc (reframed post-M2 version,
...
extends to substantially higher wavenumber than the baseline---energy is
genuinely carried to small scales by compression and cascade---but the
...
near-equilibrium limit applies and where \DIFdelbegin \DIFdel{beating }\DIFdelend \DIFaddbegin \DIFadd{improvement over }\DIFaddend linear theory is
meaningful. The spectral \emph{peak} corroborates the regime reading
...
wavenumber, not only the spectral shape at high $\kappa$. 
The \DIFdelbegin \DIFdel{cost is
the practical justification of the approach: the }\DIFdelend operating-point
ensemble completes in a few hundred core-hours, roughly two orders of
magnitude below a strained DNS ensemble at comparable Reynolds number,
which \DIFdelbegin \DIFdel{is what makes }\DIFdelend \DIFaddbegin \DIFadd{permits }\DIFaddend parametric sweeps over inflow condition\DIFdelbegin \DIFdel{feasible}\DIFdelend .
...
\begin{figure}
  ...
  spectral peak (dashed) against the rigid-translation law (grey dotted)\DIFaddbeginFL \DIFaddFL{, with the geometric $\pm\sigma$ ensemble spread shaded}\DIFaddendFL :
  the broadband distortion that linear theory cannot represent, at the
  operating point and with the noise averaged out.}
  \label{fig:spec-op}
\end{figure}

\subsection{Validation against the \DIFaddbegin \DIFadd{CMK }\DIFaddend strained-turbulence experiment\DIFdelbegin \DIFdel{of
Chen, Meneveau \& Katz}\DIFdelend }\label{sec:cmk}
...
benchmark at laboratory Reynolds number is provided by the \DIFaddbegin \DIFadd{CMK }\DIFaddend plane-strain
experiment\DIFdelbegin \DIFdel{of \mbox{%DIFAUXCMD
\citet{ChenMeneveauKatz2006} }\hskip0pt%DIFAUXCMD
(CMK)}\DIFdelend , whose straining phase
...
kinetic-energy amplification $1.72$ against the exact $1.69$, the
residual \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{attributed to }\DIFaddend the moment closure\DIFdelbegin \DIFdel{'s}\DIFdelend , not the implementation \DIFdelbegin \DIFdel{'s
}\DIFdelend (\S\,\ref{sec:level1a}). With eddies and viscosity active the component
...
line. Figure~\ref{fig:threeway} and \DIFdelbegin \DIFdel{table}\DIFdelend \DIFaddbegin \DIFadd{Table}\DIFaddend ~\ref{tab:threeway} give the
...
\begin{table}
  ...
  state. \DIFdelbeginFL \DIFdelFL{Bands }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The band }\DIFaddendFL below $0.75\,\kappa_c$ \DIFdelbeginFL \DIFdelFL{rest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rests }\DIFaddendFL on a few box modes \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{so those results }\DIFaddendFL are
  indicative only.}
  \label{tab:threeway}
\end{table}

The structure of the \DIFdelbegin \DIFdel{table }\DIFdelend \DIFaddbegin \DIFadd{tabulated results }\DIFaddend is the finding. Exact linear theory concentrates
...
splitting $\phi_{11}/\phi_{33}-1$\DIFdelbegin \DIFdel{(figure~\ref{fig:threeway}}\textit{\DIFdel{d}}%DIFAUXCMD
\DIFdel{)
}\DIFdelend \DIFaddbegin \DIFadd{, a combination of the two transverse
components just shown, }\DIFaddend carries the same reading \DIFdelbegin \DIFdel{in a purely transverse observable}\DIFdelend \DIFaddbegin \DIFadd{(not plotted)}\DIFaddend : exact RDT
%DIF >  splitting $\phi_{11}/\phi_{33}-1$ (figure~\ref{fig:threeway}\textit{d})
%DIF >  carries the same reading in a purely transverse observable: exact RDT
...
\S\,\ref{sec:gate}, whose \DIFdelbegin \DIFdel{exercise }\DIFdelend \DIFaddbegin \DIFadd{introduction }\DIFaddend is thereby motivated by a measured
deficit rather than by \DIFdelbegin \DIFdel{completeness}\DIFdelend \DIFaddbegin \DIFadd{its association with an identified physical process}\DIFaddend .
...
\begin{figure}
  ...
  $\Delta b_{33}$\DIFdelbeginFL \DIFdelFL{, (}\textit{\DIFdelFL{d}}%DIFAUXCMD
\DIFdelFL{) the transverse splitting
  $\phi_{11}/\phi_{33}-1$}\DIFdelendFL . SC-ODT ($\blacksquare$, $1024$ realisations)
  against exact projected RDT ($\blacklozenge$) and DNS ($128^3$,
  $\bullet$, four realisations). The SC-ODT curves are \DIFdelbeginFL \DIFdelFL{near-flat }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{relatively flat }\DIFaddendFL in every
...
\begin{figure}
  ...
  exact-RDT-distorted von K\'arm\'an fit, against total strain, for \DIFdelbeginFL \DIFdelFL{slow
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{low
  }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{rapid }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{high }\DIFaddendFL onset strain-rate parameters (medians over $1024$
...
\eqref{eq:poisson} \DIFdelbegin \DIFdel{--}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend \eqref{eq:Mijkl} are exact; no modelling has entered. The
...
\begin{equation}
  ...
  \label{eq:phi1d}
\end{equation}
\DIFdelbegin \DIFdel{the one-dimensional spectra }\DIFdelend obtained by integrating $\Phi_{mn}$ over the two
...
case of \DIFaddbegin \DIFadd{assumption }\DIFaddend A1 \DIFaddbegin \DIFadd{(see Appendix~\ref{app:twoscalar})}\DIFaddend , and uniaxial contraction preserves axisymmetry about the
...
\begin{equation}
  ...
  \label{eq:angular}
\end{equation}
with $\kappa^2=\kappa_2^2+\kappa_\perp^2$, and \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend weights mixing a resolved
and an unresolved index (e.g.\ $\kappa_1\kappa_2/\kappa^2$) averaging to zero.
\DIFdelbegin \DIFdel{Substituting }\DIFdelend \DIFaddbegin \DIFadd{Substitution of }\DIFaddend \eqref{eq:angular} into \eqref{eq:Mijkl} removes the azimuthal
...
themselves. 
Equations \eqref{eq:Piclosed}--\eqref{eq:g3} are therefore the \DIFdelbegin \emph{\DIFdel{collapsed form}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{collapsed form }\DIFaddend on which any single-line statement must be built\DIFdelbegin \DIFdel{; the single-line content itself---what the line data pin down, and how sharply---is }\DIFdelend \DIFaddbegin \DIFadd{. What the line data actually determine—and how sharply—is }\DIFaddend established next.
...
suite \DIFdelbegin \DIFdel{pinning }\DIFdelend \DIFaddbegin \DIFadd{confirming }\DIFaddend tracelessness, the isotropic angular averages
...
%DIF >  \begin{center}\small
%DIF >  \begin{tabular}{lccc}
%DIF >  constraint level & $\mathcal I_{11}$ & $\mathcal I_{22}$ & $\mathcal I_{33}$\\[1pt]
%DIF >  line data $+$ solenoidality $+$ realisability & 0.77 & 1.28 & 0.86\\
%DIF >  $+$ log-Lipschitz slope cap ($\lambda=4$) & \multicolumn{3}{c}{$<5\%$ narrower}\\
%DIF >  $+$ polarisation cap $|c|\le\gamma a$, $\gamma=0$ & 0.34 & 0.58 & 0.24\\
%DIF >  \end{tabular}
%DIF >  \end{center}
\begin{table}
  \centering\DIFaddendFL \small
  \begin{tabular}{lccc}
  constraint level & \DIFdelbeginFL \DIFdelFL{$\mathcal I_{11}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathcal I_1$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\mathcal I_{22}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathcal I_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\mathcal I_{33}$}%DIFDELCMD < \\[1pt]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathcal I_3$}\\[2pt]
  \DIFaddendFL line data $+$ solenoidality $+$ realisability & 0.77 & 1.28 & 0.86\\
  $+$ log-Lipschitz slope cap ($\lambda=4$) & \multicolumn{3}{c}{$<5\%$ narrower}\\
  $+$ polarisation cap $|c|\le\gamma a$, $\gamma=0$ \DIFaddbeginFL \DIFaddFL{(i.e.\ $c=0$) }\DIFaddendFL & 0.34 & 0.58 & 0.24\\
  \end{tabular}
  \DIFdelbeginFL %DIFDELCMD < \end{center}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{Kinematic indeterminacy
  $\mathcal I_n=(\Pi^{+}_{nn}-\Pi^{-}_{nn})/2k_tS$ of the three rapid
  pressure--strain components under successively tighter side conditions,
  calibrated on an isotropic von K\'arm\'an spectrum. Line data with
  solenoidality and realisability leave an $O(1)$ indeterminacy; the
  log-Lipschitz smoothness cap barely narrows it, whereas the polarisation
  cap ($\gamma=0$, i.e.\ $c=0$) roughly halves it.}}
  \label{tab:indeterminacy}
\end{table}
\DIFaddend Two features of \DIFdelbegin \DIFdel{this table }\DIFdelend \DIFaddbegin \DIFadd{the Table~\ref{tab:indeterminacy} }\DIFaddend matter. Smoothness assumptions buy almost
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As showm in Appenix~\ref{app:forward}, the }\DIFaddend representation also supplies two \emph{free falsifiers} that the line
...
An \DIFdelbegin \DIFdel{$O(b^2)$ error estimate }\DIFdelend \DIFaddbegin \DIFadd{error estimate quadratic in the anisotropy ($O(b^2)$)}\DIFaddend , which the symmetric structure of the discarded
harmonic suggests, is superseded by this measurement.
%DIF > An $O(b^2)$ error estimate, which the symmetric structure of the discarded harmonic suggests, is superseded by this measurement. 
Second, the
...
\begin{figure}
  ...
  splitting $\phi_{11}/\phi_{33}-1$ \DIFdelbeginFL \DIFdelFL{per }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for each }\DIFaddendFL geometry, and the upwash
  anisotropy $b_{22}$ (grey dash-dot, indistinguishable between
  geometries)---the azimuthal structure \DIFaddbeginFL \DIFaddFL{that }\DIFaddendFL A1 assumes away is what differs,
...
\eqref{eq:g1}--\eqref{eq:g3}; the moment reduction \DIFdelbegin \DIFdel{discards }\DIFdelend \DIFaddbegin \DIFadd{omits }\DIFaddend it. This
...
justifies the closure used in the simulations and fixes the prefactor in an explicit convention \DIFaddbegin \DIFadd{(the coefficient of \eqref{eq:crow-spectral} and the remark following it)}\DIFaddend .
...
\DIFdelbegin \DIFdel{discards---and both discards }\DIFdelend \DIFaddbegin \DIFadd{omits---and the effects of both omissions }\DIFaddend are now measured: the wavenumber dependence
...
agree to leading order, and the terms in which they \DIFdelbegin \DIFdel{depart }\DIFdelend \DIFaddbegin \DIFadd{differ }\DIFaddend are exactly
...
\item Independently of A1's validity \DIFaddbegin \DIFadd{DNS}\DIFaddend , line data bound the rapid term
  sharply; the bound is \DIFdelbegin \DIFdel{honest---and }\DIFdelend \DIFaddbegin \DIFadd{valid---and }\DIFaddend verified to bracket the exact
...
\item Rapid and slow redistribution are disjoint: verified by the
  strain-off null and by the rapid-limit invariance of every \DIFaddbegin \DIFadd{tested }\DIFaddend kernel-energy
  allocation rule.
...
anisotropy is allocated uniformly across scales \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend linear theory and
...
The \DIFaddbegin \DIFadd{whole investigation is carried out at a single, moderate turbulence
Reynolds number, chosen so that the diagnostic has room to separate scales
but the cascade is short: the molecular viscosity
($\nu=10^{-4}$; Appendix~\ref{app:numerics}) is set so that more than a
decade of scales is resolved below the energy peak with wide margin. This
regime is the referent of the ``at this Reynolds number'' qualifications
that follow, and the caveat is real---the scale separation, not the eddy
statistics, is what the negative results below turn on.
The }\DIFaddend configuration is a scale-diagnostic variant of the strained line: unit
strain rate (so $e=t$), an initial spectrum peaked at a wavenumber
$\kappa_p$ \DIFdelbegin \DIFdel{resolved by more than a }\DIFdelend \DIFaddbegin \DIFadd{with the resolved }\DIFaddend decade of smaller scales \DIFaddbegin \DIFadd{below it}\DIFaddend , and two
\DIFdelbegin \DIFdel{measurement bands, $\kappa_2\in[0.6,2]\,\kappa_p$ (the }\DIFdelend %DIF >  $\kappa_p$ resolved by more than a decade of smaller scales, and two
%DIF >  measurement bands, $\kappa_2\in[0.6,2]\,\kappa_p$ (the energy-containing
%DIF >  band) and $[6,16]\,\kappa_p$ (roughly two triplet-map generations below
%DIF >  it, since each map compresses by a factor of three). On each band the
\DIFaddbegin \DIFadd{measurement bands in wavenumber $\kappa_2$: a low-wavenumber,
}\DIFaddend energy-containing band \DIFdelbegin \DIFdel{) and }\DIFdelend \DIFaddbegin \DIFadd{$\kappa_2\in[0.6,2]\,\kappa_p$ (subscript
$\mathrm{lo}$) and a high-wavenumber, fine-scale band
}\DIFaddend $[6,16]\,\kappa_p$ (\DIFaddbegin \DIFadd{subscript $\mathrm{hi}$)---the latter }\DIFaddend roughly two
triplet-map generations \DIFdelbegin \DIFdel{below
it}\DIFdelend \DIFaddbegin \emph{\DIFadd{finer}}\DIFaddend , since each map compresses \DIFaddbegin \DIFadd{scales
}\DIFaddend by a factor of three\DIFdelbegin \DIFdel{)}\DIFdelend . On each band the
...
is set out here, with the two decisive figures (the relaxation clock
...
drawn---and then recursively to $9$ and $27$ sub-regions. 
One \DIFaddbegin \DIFadd{sub-}\DIFaddend level ($\ell/3$ for an eddy of size $\ell$) is \DIFdelbegin \DIFdel{null}\DIFdelend \DIFaddbegin \DIFadd{inconsequential}\DIFaddend : for the large eddies that
feed the band, $\ell/3$ still sits above it. Two \DIFaddbegin \DIFadd{sub-}\DIFaddend levels produce the first statistically significant fine-scale isotropisation of the family ($\mathcal A_{\rm hi}$ from $1.43$ to $1.27$ at $e=1$, \DIFdelbegin \DIFdel{both bands'
paired contrasts individually significant}\DIFdelend \DIFaddbegin \DIFadd{the change significant in both the fine-scale and energy-containing bands}\DIFaddend ), and three \DIFaddbegin \DIFadd{sub-}\DIFaddend levels extend the effect to deep strain: the \DIFaddbegin \DIFadd{sub-}\DIFaddend levels that work, $\ell/9$ and $\ell/27$, are exactly \DIFdelbegin \DIFdel{the first }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend that reach the measured band. \DIFdelbegin \DIFdel{The bookkeeping is clean
throughout---band }\DIFdelend \DIFaddbegin \DIFadd{Band }\DIFaddend energies move by \DIFaddbegin \DIFadd{only }\DIFaddend a few per cent \DIFaddbegin \DIFadd{throughout}\DIFaddend , the upwash energy fraction and the intermittency tails are \DIFdelbegin \DIFdel{unchanged---and the pre-strain
}\DIFdelend \DIFaddbegin \DIFadd{unchanged, and even at $e=0$, before any strain, the }\DIFaddend fine-scale \DIFdelbegin \DIFdel{floor moves }\DIFdelend \DIFaddbegin \DIFadd{imbalance is pulled }\DIFaddend towards component equality \DIFdelbegin \DIFdel{immediately
}\DIFdelend ($\mathcal A_{\rm hi}$ from $0.83$ to $0.95$\DIFdelbegin \DIFdel{at $e=0$}\DIFdelend ).

Four further variants \DIFdelbegin \DIFdel{separate the }\DIFdelend \DIFaddbegin \DIFadd{instantiate distinct }\DIFaddend candidate explanations for the one remaining limitation, the fading of the gains beyond $e\approx3$. Tying
...
strain. \DIFdelbegin \DIFdel{Delocalising }\DIFdelend \DIFaddbegin \DIFadd{Spatially delocalising }\DIFaddend the relaxation---after each eddy, applying the
...
the limiter: $N=1$ is \DIFdelbegin \DIFdel{null}\DIFdelend \DIFaddbegin \DIFadd{inconsequential}\DIFaddend , and $N=3$ is significant in both bands at
...
strain it overshoots the depth-only variant significantly. \DIFdelbegin \DIFdel{The ledger
is then complete }\DIFdelend \DIFaddbegin \DIFadd{Thus, }\DIFaddend for every scheme locked to the eddy clock: depth sets
...
imbalance continuously. \DIFdelbegin \DIFdel{Whether that is fundamental or merely
structural is decided by }\DIFdelend \DIFaddbegin \DIFadd{This motivates }\DIFaddend the final intervention.

\subsection{Decoupling the relaxation clock}\label{sec:clock}

The one \DIFdelbegin \DIFdel{axis }\DIFdelend \DIFaddbegin \DIFadd{feature }\DIFaddend the eddy-locked \DIFdelbegin \DIFdel{family cannot test }\DIFdelend \DIFaddbegin \DIFadd{formulations do not modify }\DIFaddend is \emph{when} relaxation acts. We therefore give it its own clock\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. Whereas the eddy
events are sampled from the state-dependent rate density
$\lambda(y_0,\ell;t)$ of \S\,\ref{sec:odt-review}, the relaxation-only
events form }\DIFaddend an independent Poisson stream \DIFdelbegin \DIFdel{of relaxation-only events at a prescribed rate
, each
event an interval drawn from the }\DIFdelend \DIFaddbegin \DIFadd{with the prescribed rate
density
}\begin{equation}
  \DIFadd{\lambda_{\mathrm c}(y_0,\ell)
  = \frac{R\,f(\ell)}{L-\ell},
  \qquad
  f(\ell)
  = \frac{2L_p\,\ell^{-2}\,\mathrm{e}^{-2L_p/\ell}}{\mathrm{e}^{-2L_p/L_{\max}}-\mathrm{e}^{-2L_p/L_{\min}}},
  \qquad \ell\in[L_{\min},L_{\max}],
  \label{eq:clock}
}\end{equation}
\DIFadd{with $R$ the total event rate, the placement $y_0$ uniform over the
$L-\ell$ admissible positions on the domain of length $L$, and $f$ the
presumed candidate-size density of the model's own eddy sampling
\mbox{%DIFAUXCMD
\citep{Kerstein1999,StephensLignell2021}}\hskip0pt%DIFAUXCMD
, its most-probable size $L_p$
and range $[L_{\min},L_{\max}]$ taken from the baseline run---so no
empirically gathered size statistics enter and, unlike $\lambda$,
\eqref{eq:clock} carries no dependence on the flow state. This candidate density is the }\DIFaddend model's \DIFdelbegin \DIFdel{eddy-size distribution, placed
uniformly on the domain, and relaxed by the same conservative kernel pass with the }\DIFdelend \DIFaddbegin \DIFadd{presumed }\emph{\DIFadd{sampling}} \DIFadd{distribution, not the realised distribution of accepted eddies, which the acceptance rule reshapes; it is used because it is convenient to sample and carries the gross features of the physical eddy-size distribution. Each event applies the kernel of \eqref{eq:event} to its interval---the
interval's own triplet-map displacement as the kernel shape, no map
applied, amplitudes \eqref{eq:ci} from the interval's own
integrals---followed by the }\DIFaddend adaptive-depth hierarchy \DIFdelbegin \DIFdel{inside it. Because each event
acts on an interval, momentum and kinetic energy are conserved exactly per event; because the stream has its own rate, its pacing is decoupled
from the eddy statistics entirely. The rate }\DIFdelend \DIFaddbegin \DIFadd{of
\S\,\ref{sec:interventions} inside it; the first identity of
\eqref{eq:kernel-identities} conserves each momentum component and
the transfer matrix of \eqref{eq:redistribution} their summed kinetic
energy, exactly and per event. The mean rate of the relaxation events }\DIFaddend is matched to the baseline's
mean eddy rate, \DIFaddbegin \DIFadd{$R=\bar N_{\mathrm e}/T$ with $\bar N_{\mathrm e}$ the mean number of eddy events over an SC-ODT baseline realisation of duration
$T$, }\DIFaddend so the comparison against the eddy-locked variants is
dose-matched by construction.

%DIF >  The result (figure~\ref{fig:clock}) decides the question. With the
%DIF >  clock, the fine-scale imbalance is reduced significantly at
%DIF >  \emph{every} strain and the effect grows rather than fades---the paired
%DIF >  contrasts reach $-0.30$ (low band) and remain significant in the high
%DIF >  band at $e=3.9$---because a constant-rate stream keeps pace precisely
%DIF >  where the eddy rate could not: event-locked timing, not any per-event
%DIF >  property, was the deep-strain limiter. But the stream drawing the full candidate density---capped only at the model's default largest scale $L_{\max}$---is scale-blind and overcorrects: it also suppresses the one-point
%DIF >  anisotropy that the strain should produce, pulling $u_2^2/2k_t$ from
%DIF >  $0.58$ to $0.41$ at $e=3.9$ and thereby breaking the moment-level
%DIF >  behaviour validated in \S\,\ref{sec:lr-validation}. Restricting the stream to sub-band interval
%DIF >  sizes---replacing $f$ in \eqref{eq:clock} by its restriction to
%DIF >  $\ell\le\ell^{*}$, renormalised (over-cap draws are resampled), with
%DIF >  $\ell^{*}$ the scale separating the two measurement bands---resolves
%DIF >  the tension: the
%DIF >  size-capped clock---the ``clock-relaxed ODT'' of the figures---holds the
%DIF >  fine scales at every strain with no
%DIF >  fade---$\mathcal A_{\rm hi}$ at $1.30$--$1.44$ against the baseline's
%DIF >  $1.63$--$1.94$, contrasts a reduction that is significant throughout and strongest at $e=3$ (figure~\ref{fig:clock}\textit{a})---while the one-point trajectory, trend in Fig.~\ref{fig:clock}\textit{b},  is essentially untouched in the moderate-strain regime ($u_2^2/2k_t$ within $0.005$ of the baseline at $e=1$) and only mildly reduced at the deepest strain (figure~\ref{fig:clock}\textit{b}). The mean rate of relaxation events directly controls the strain dependence: quadrupling of rate is a single control the rate extends the upward trend in Fig.~\ref{fig:clock}\textit{a} at a proportionate cost in Fig.~\ref{fig:clock}\textit{b}.
\DIFaddbegin 

\DIFaddend The result (figure~\ref{fig:clock}) decides the question. With the
clock, the fine-scale imbalance is reduced significantly at \DIFdelbegin \emph{\DIFdel{every}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{every
}\DIFaddend strain and the effect grows rather than \DIFdelbegin \DIFdel{fades---the }\DIFdelend \DIFaddbegin \DIFadd{fades: the }\DIFaddend paired contrasts
reach $-0.30$ \DIFdelbegin \DIFdel{(low band ) }\DIFdelend \DIFaddbegin \DIFadd{in the low band }\DIFaddend and remain significant in the high band
at $e=3.9$\DIFdelbegin \DIFdel{---because a }\DIFdelend \DIFaddbegin \DIFadd{. A }\DIFaddend constant-rate stream keeps pace precisely where the
eddy rate could not\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{, confirming that }\DIFaddend event-locked \DIFdelbegin \DIFdel{timing, not }\DIFdelend \DIFaddbegin \DIFadd{timing---not }\DIFaddend any
per-event \DIFdelbegin \DIFdel{property, was }\DIFdelend \DIFaddbegin \DIFadd{property---was }\DIFaddend the deep-strain limiter.
\DIFdelbegin \DIFdel{But the uncapped stream }\DIFdelend \DIFaddbegin 

\DIFadd{However, a stream that draws from the full candidate density (capped
only at the model's default largest scale $L_{\max}$) }\DIFaddend is scale-blind
and \DIFdelbegin \DIFdel{overcorrects: it }\DIFdelend \DIFaddbegin \DIFadd{over-corrects. It }\DIFaddend also suppresses the one-point anisotropy that
the strain should produce, pulling $u_2^2/2k_t$ from $0.58$ to $0.41$
at $e=3.9$ and thereby breaking the moment-level behaviour validated
in \S\,\ref{sec:lr-validation}.
\DIFaddbegin 

\DIFaddend Restricting the stream to sub-band interval \DIFdelbegin \DIFdel{sizes ($\ell\le\ell^{*}$, the scale
separating the two
measurement bands) resolves the tension: the
}\DIFdelend \DIFaddbegin \DIFadd{sizes---replacing $f$ in
\eqref{eq:clock} by its restriction to $\ell\le\ell^*$, renormalised
(over-cap draws are resampled), where $\ell^*$ separates the two
measurement bands---resolves the tension. The }\DIFaddend size-capped \DIFdelbegin \DIFdel{clock---the }\DIFdelend \DIFaddbegin \DIFadd{clock (the
}\DIFaddend ``clock-relaxed ODT'' of the \DIFdelbegin \DIFdel{figures---holds }\DIFdelend \DIFaddbegin \DIFadd{figures) holds }\DIFaddend the fine scales at every
strain with no \DIFdelbegin \DIFdel{fade---$\mathcal A_{\rm hi}$ at $1.30$}\DIFdelend \DIFaddbegin \DIFadd{fade ($\mathcal{A}_{\rm hi}=1.30$}\DIFaddend --$1.44$ against the
baseline's $1.63$--$1.94$\DIFdelbegin \DIFdel{, contrasts }\DIFdelend \DIFaddbegin \DIFadd{), with contrasts that remain }\DIFaddend significant
throughout and \DIFaddbegin \DIFadd{are }\DIFaddend strongest at $e=3$ \DIFdelbegin \DIFdel{---while the }\DIFdelend \DIFaddbegin \DIFadd{(figure~\ref{fig:clock}}\textit{\DIFadd{a}}\DIFadd{).
The }\DIFaddend one-point trajectory is essentially untouched in the
moderate-strain regime ($u_2^2/2k_t$ within $0.005$ of the baseline at
$e=1$) and only mildly reduced at the deepest strain
\DIFdelbegin \DIFdel{. The rate is a
clean dial: four times the rate holds the fine scales harder }\DIFdelend \DIFaddbegin \DIFadd{(figure~\ref{fig:clock}}\textit{\DIFadd{b}}\DIFadd{).
}

\DIFadd{The mean rate of relaxation events is a single control: quadrupling
the rate extends the upward trend in figure~\ref{fig:clock}}\textit{\DIFadd{a}}
\DIFaddend at a proportionate \DIFdelbegin \DIFdel{one-point cost }\DIFdelend \DIFaddbegin \DIFadd{cost in figure~\ref{fig:clock}}\textit{\DIFadd{b}}\DIFaddend .
...
\begin{figure}
  ...
  price: upwash energy \DIFdelbeginFL \DIFdelFL{fraction---the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fraction. The reference is the SC-ODT baseline
  trajectory---the moment-level behaviour validated in
  \S\,\ref{sec:lr-validation}---which the }\DIFaddendFL uncapped clock suppresses\DIFaddbeginFL \DIFaddFL{;
  }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{physical one-point anisotropy, the }\DIFdelendFL size cap \DIFdelbeginFL \DIFdelFL{restores }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{at matched rate preserves }\DIFaddendFL it\DIFaddbeginFL \DIFaddFL{, and the $4\times$ rate
  reduces it in proportion}\DIFaddendFL . Medians
...
The \DIFdelbegin \DIFdel{arc closes the loop opened }\DIFdelend \DIFaddbegin \DIFadd{deficiency measured }\DIFaddend by the three-way \DIFdelbegin \DIFdel{comparison. The
deficiency measured there---anisotropy }\DIFdelend \DIFaddbegin \DIFadd{comparison---anisotropy
}\DIFaddend allocated uniformly over wavenumber---is confirmed \DIFdelbegin \DIFdel{here in transmission
form, }\DIFdelend \DIFaddbegin \DIFadd{by the transmission
diagnostic of \S\,\ref{sec:transmission} in a different observable: the
band-imbalance ratio $\mathcal A_{\rm hi}/\mathcal A_{\rm lo}$ stays
near unity, so the strain-induced imbalance reaches the fine scales
undiminished. It }\DIFaddend is unfixable by gating or map reweighting, and is
cured by exactly two ingredients, each isolated by a falsified
alternative: relaxation applied \emph{at the scale of the imbalance}
(depth, not dosage or location) and relaxation applied \emph{on its
own clock}\DIFdelbegin \DIFdel{(the size-capped stream}\DIFdelend \DIFaddbegin \DIFadd{, with the event-size distribution capped at
$\ell\le\ell^{*}$ (the clock-relaxed ODT of \S\,\ref{sec:clock})}\DIFaddend ,
which alone survives deep strain\DIFdelbegin \DIFdel{)}\DIFdelend . One limit remains, and it is
structural rather than curable by any amount of relaxation: an
isotropising mechanism is bounded at component equality, whereas exact
...
The \DIFdelbegin \DIFdel{head-to-head }\DIFdelend \DIFaddbegin \DIFadd{comparison }\DIFaddend that summarises this part is
...
integrated anisotropy---is starkest precisely where the eddies matter
least. Second, at moderate rapidity the mechanism tilts the model's
...
scheme manufactures more of it. The remaining, sign-level gap belongs to the
\DIFdelbegin \DIFdel{wavenumber-dependent operator identified above}\DIFdelend \DIFaddbegin \DIFadd{wavenumber- and angle-dependent rapid operator of
\eqref{eq:Piclosed}--\eqref{eq:g3}, identified above as the next step}\DIFaddend .
...
the aerofoil geometry and response. This is the sharpest question the
distortion model can be asked without importing an acoustic solver:
...
$(A_0,\kappa_e)$ per strain, fitted to the median line spectra of the \DIFaddbegin \DIFadd{SC-ODT
}\DIFaddend $1024$-realisation ensembles at $Sk_t/\varepsilon\approx0.4$ and
$\approx8$. The \DIFaddbegin \DIFadd{fit is to SC-ODT; the clock-relaxed inflow of
\S\,\ref{sec:clock}, fitted identically, is carried alongside from
figure~\ref{fig:expval} onward, and the radiated level does not
discriminate between the two---the clock's gain appears in the
uncertainty the input carries (figure~\ref{fig:amiet-rapid}). The
}\DIFaddend line data pin the resolved-wavenumber structure; the
...
dividing it out isolates the spectral reallocation, and the variance ratio itself is taken from the measured line spectra
rather than from the fit. \DIFaddbegin \DIFadd{The chain is laid out step by step in the
...
\begin{figure}
  ...
  dominates the transit \DIFaddbeginFL \DIFaddFL{time}\DIFaddendFL ; $+0.5$ to $+1.9$\,dB at the rapid rate, where
  the upwash amplification survives). Geometry, aerofoil response and
  observer position cancel identically in this ratio.}
  \label{fig:lenoise}
\end{figure}

Figure~\ref{fig:lenoise} shows the result, and its sign and structure
are the finding. \DIFaddbegin \DIFadd{Each panel plots, against reduced streamwise
wavenumber $K_x/\kappa_e(0)$, the change in the noise kernel when the
frozen inflow is replaced by the distorted one: the $\Delta\mathrm{SPL}=0$
line at the top is the frozen reference, so every curve lying below it
marks a weaker source, and the curves darken with increasing total
strain. The solid traces are the shape change at fixed upwash variance
and the dotted traces add the measured variance term, so the offset
between a solid curve and its dotted partner is that variance
contribution---deepening the drop at the slow rate (panel~}\textit{\DIFadd{a}}\DIFadd{)
and slightly lifting it at the rapid rate (panel~}\textit{\DIFadd{b}}\DIFadd{). }\DIFaddend At matched
upwash variance the distorted spectrum is a
...
proceeds. The distortion, in short, drains the very content the
aerofoil responds to. The total change adds the variance term: at the
...
transit \DIFaddbegin \DIFadd{time }\DIFaddend deepens it.
...
response\DIFaddbegin \DIFadd{~ \mbox{%DIFAUXCMD
\citep{Sears1941}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The spectra enter in physical units through two matching
...
\emph{is} the standard Amiet--von K\'arm\'an prediction, not a
recalibration of it: the $e=0$ member of the representation is the
...
spectrum shown as an independent check of the unit bridge. \DIFaddbegin \DIFadd{The figure
is read vertically, at fixed frequency: the two upper curves---dashed
and dotted, all but coincident---are the frozen baselines, and their
overlap is that unit-bridge check made visible; the three solid curves
below are the same chain fed the distorted inflow at $e=0.5$, $1$ and
$2$, so the downward offset of each from the dashed baseline is the
level reduction the closure predicts at that strain, deepening as the
strain increases. }\DIFaddend Against
...
\DIFaddbegin \DIFadd{Both this figure and the kernel ratio of figure~\ref{fig:lenoise}
carry the SC-ODT inflow as the representative computed spectrum: at
this operating point the clock-relaxed variant gives an
indistinguishable far-field prediction---its gain lies below the
size cap, where the noise kernel carries little weight, so it appears
in the input uncertainty rather than the radiated level
(figure~\ref{fig:amiet-rapid})---so the two computed inflows are
carried separately only from the validation of figure~\ref{fig:expval}
onward, where the comparison is the point.
}\DIFaddend 
...
\begin{figure}
  ...
  checks ($\pm0.5$\,dB). The effective strain is a geometric estimate (potential-flow stagnation-line accumulation truncated at the eddy scale\DIFaddbeginFL \DIFaddFL{, varied over $\Lambda/4$--$\Lambda$}\DIFaddendFL ); the \DIFdelbeginFL \DIFdelFL{shaded band spans truncation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{resulting spread in predicted level is }\DIFaddendFL at \DIFdelbeginFL \DIFdelFL{$\Lambda/4$--$\Lambda$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{most ${\approx}1$\,dB, comparable to the digitisation uncertainty, and is not shown}\DIFaddendFL .}
  \label{fig:expval}
\end{figure}
...
effective strain is negligible and \DIFdelbegin \DIFdel{the model correctly predicts
}\DIFdelend \DIFaddbegin \DIFadd{both computed inflows correctly predict }\DIFaddend \emph{no} correction to the frozen spectrum, while for the NACA0012 \DIFdelbegin \DIFdel{it
}\DIFdelend \DIFaddbegin \DIFadd{they
}\DIFaddend lowers the prediction towards the data at every speed
(bias $+5.7\!\to\!+5.3$\,dB at $40$\,m\,s$^{-1}$,
$+1.8\!\to\!+1.4$\,dB at $60$, $+5.1\!\to\!+4.7$\,dB at $90$), by the
modest amount \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend geometric effective strain implies; the shift
...
the finite-thickness effect \DIFaddbegin \DIFadd{that }\DIFaddend \citet{PatersonAmiet1976} themselves
identified (breakdown near $U/f t_A\approx1$) and
\DIFaddbegin \DIFadd{that }\DIFaddend \citet{Gershfeld2004} modelled, of a piece with the blocking mechanism
excluded from the present scope; it is \DIFdelbegin \DIFdel{unchanged in rank by any }\DIFdelend \DIFaddbegin \DIFadd{insensitive to the }\DIFaddend choice
of inflow spectrum.
...
\begin{figure}
  ...
  frozen von K\'arm\'an inflow of standard practice. \DIFdelbeginFL \DIFdelFL{Grey }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Black with markers }\DIFaddendFL (circles:
  SC-ODT; triangles: clock-relaxed ODT): the two computed inflows
  (fitted representations, measured ensemble levels), each with a \DIFaddbeginFL \DIFaddFL{grey }\DIFaddendFL band
  giving its measured spectral rms distance from the RDT family mapped
  to decibels; the \DIFdelbeginFL \DIFdelFL{two share }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bands overlap sufficiently that }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{band and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{two inflows }\DIFaddendFL are indistinguishable at this level.}
  \label{fig:amiet-rapid}
\end{figure}

Where the mechanism does reach the prediction is shown in
figure~\ref{fig:amiet-rapid}, and the claim is worded exactly as the
evidence permits. \DIFaddbegin \DIFadd{Each panel shows absolute SPL against frequency at
one application strain; the black solid curve is the exact-RDT
reference that the strained-box DNS validates at this rate, so the
vertical distance of any other curve from it, read at fixed frequency,
is that inflow's error. The dashed frozen-von-K\'arm\'an curve sits
above the reference---the over-prediction of standard practice---while
the two computed inflows (circles, SC-ODT; triangles, clock-relaxed)
fall on the reference within a shared grey band, close enough that they
cannot be told apart at this level. }\DIFaddend At total strains of $1$--$2$ the frozen-spectrum
...
\S\,\ref{sec:envelope}---so an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{outperforms }\DIFaddend a prescribed one
at \DIFdelbegin \DIFdel{precisely }\DIFdelend the strains where the correction matters, whichever
variant of the model evolves it. Between the two variants the
...
rotors\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{MajumdarPeake1998}}\hskip0pt%DIFAUXCMD
}\DIFaddend , where the streamtube contraction supplies the strain and the
blade response the fine-scale weighting---and is left as \DIFdelbegin \DIFdel{the
falsifiable target of the application study. }\DIFdelend \DIFaddbegin \DIFadd{a falsifiable future target.
}\DIFaddend 

\DIFaddbegin \begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{Figures/fig_frozen_cost.pdf}
  \caption{\DIFaddFL{The cost of the frozen-inflow assumption against accumulated
  distortion: signed far-field prediction error, judged by the best
  available reference at each strain. Laboratory window (shaded): the
  eight measured cases of figure~\ref{fig:expval}, bias over each
  case's comparison band---flat plates filled, NACA0012 open (its
  common overprediction is the finite-thickness effect); horizontal
  bars span the effective-strain truncation range, and the three
  inflow chains are offset slightly in $e$ for legibility. Beyond the
  window: band-mean error over $100$--$1000$\,Hz against the exact-RDT
  reference of figure~\ref{fig:amiet-rapid} at $e=0.5$, $1$ and $2$;
  shaded bands give the $\pm10\log_{10}(1+\mathrm{rms})$
  spectral-distance uncertainty where measured ($e=1$, $2$).}}
  \label{fig:frozen-cost}
\end{figure}

\DIFadd{Figure~\ref{fig:frozen-cost} assembles the two regimes on a single
axis and summarises the section. In the laboratory window the frozen
and computed inflows are indistinguishable and both are right: the
window in which Amiet's theory has been validated is precisely the
window in which its frozen-spectrum input costs nothing, which is why
the practice has survived unchallenged. What the
methodology adds is the right-hand half of the figure: an inflow that
continues to predict where the frozen assumption fails. Beyond the window the frozen
error grows monotonically---$+0.6$, $+1.4$, $+3.4$\,dB in the band
mean at $e=0.5$, $1$, $2$---with no measurement there to catch it,
while the computed inflows remain within $1.1$--$2.7$\,dB of the
reference and carry their measured uncertainty with them.
}

\DIFaddend The scope of this statement is set out as carefully as its content.
It concerns the \emph{free} distortion by the mean strain: the
...
partially compensating mechanism outside the \DIFaddbegin \DIFadd{scope of the }\DIFaddend present line model\DIFdelbegin \DIFdel{, and
}\DIFdelend \DIFaddbegin \DIFadd{. This mechanism replaces }\DIFaddend the strain history along individual streamlines \DIFdelbegin \DIFdel{is replaced }\DIFdelend by the
canonical constant-rate distortion. What the calculation establishes
is therefore not a noise prediction but the magnitude and sign of the
term that frozen-spectrum practice omits: order $5$--$20$\,dB at the
energy-containing reduced frequencies for total strains of
$0.5$--$2$---far too large to neglect, and of the opposite sign to the
blocking correction with which it must ultimately be combined.
\DIFaddbegin 

\DIFaddend Figure~\ref{fig:amiet-abs} \DIFdelbegin \DIFdel{shows that the chain from model output to }\DIFdelend \DIFaddbegin \DIFadd{carries the kernel result through to an
}\DIFaddend absolute far-field \DIFdelbegin \DIFdel{level is complete and cheap}\DIFdelend \DIFaddbegin \DIFadd{spectrum in physical units, the frozen baseline
coinciding with the standard Amiet--von K\'arm\'an prediction}\DIFaddend ; what
remains for a predictive comparison against measurement is the
blocking correction and the streamline strain history, for which the
present model supplies the distorted-inflow input\DIFdelbegin \DIFdel{at negligible cost}\DIFdelend , with its
...
precisely in the wavenumbers \DIFdelbegin \DIFdel{the response weights}\DIFdelend \DIFaddbegin \DIFadd{with the greatest influence on the response}\DIFaddend .

%DIF >  Figure~\ref{fig:amiet-abs} shows that the chain from model output to
%DIF >  absolute far-field level is complete and cheap; what remains for a
%DIF >  predictive comparison against measurement is the blocking correction
%DIF >  and the streamline strain history, for which the present model
%DIF >  supplies the distorted-inflow input at negligible cost, with its
%DIF >  uncertainty quantified by the envelope of \S\,\ref{sec:envelope} and
%DIF >  its fidelity improved by the mechanism of \S\,\ref{sec:limits}
%DIF >  precisely in the wavenumbers the response weights.
\DIFaddbegin 
...
single-point closure discards, not to the line. \DIFaddbegin \DIFadd{The laboratory
counterpart is the plane-strain experiment of
\mbox{%DIFAUXCMD
\citet{ChenMeneveauKatz2006}}\hskip0pt%DIFAUXCMD
: the model reproduces the qualitative
departure from rapid distortion that the experiment reports, the
component centroids held far below the rigid-translation law and the
upwash held back most (\S\,\ref{sec:cmk}; supplementary material,
\S\,S1).
}\DIFaddend 
...
rapid-distortion theory, projected onto a line, \DIFdelbegin \DIFdel{redistributes }\DIFdelend \DIFaddbegin \DIFadd{rescales }\DIFaddend the
...
integrated anisotropy but allocates it uniformly over wavenumber, \DIFdelbegin \DIFdel{where
}\DIFdelend \DIFaddbegin \DIFadd{while
}\DIFaddend linear theory and DNS concentrate it at the energy-containing scales;
the deficiency is localised in the wavenumber-uniform rapid operator
and grows with strain rapidity, a measured \DIFdelbegin \DIFdel{validity envelope }\DIFdelend \DIFaddbegin \DIFadd{artefact }\DIFaddend that
...
closure claim: line data bound the rapid term sharply, the bounds are
\DIFdelbegin \DIFdel{honest }\DIFdelend \DIFaddbegin \DIFadd{valid }\DIFaddend up to $e\approx0.25$--$0.5$ under plane strain and at all
...
The measured limits point to a specific missing ingredient \DIFdelbegin \DIFdel{, which the
paper identifies and supplies}\DIFdelend \DIFaddbegin \DIFadd{that is thereupon identified and supplied}\DIFaddend . The strain-induced fine-scale
...
move it is \DIFaddbegin \DIFadd{supplementary }\DIFaddend relaxation applied at the scale of the imbalance---the
model's own energy-equalising kernel, applied hierarchically to eddy
sub-scales. The intervention \DIFdelbegin \DIFdel{family maps the mechanism's envelope
precisely}\DIFdelend \DIFaddbegin \DIFadd{broadly reproduced the phenomenology}\DIFaddend : depth that reaches the measured scales is necessary and
...
closure is the natural \DIFaddbegin \DIFadd{future }\DIFaddend continuation, now motivated by a measured
deficit and pointed at a measured target.

For the application that motivated the work, the model delivers what
the prediction chain \DIFaddbegin \DIFadd{otherwise }\DIFaddend lacks: a distorted, anisotropic, scale-resolved
...
reference at application strains, an evolved inflow \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{outperforms }\DIFaddend the frozen
one by a margin that grows with strain. Relative to the
...
aerofoil responds to. The omitted
term is far too large to neglect and opposite in sign to the
surface-blocking correction \DIFdelbegin \DIFdel{with which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend a complete prediction must
\DIFdelbegin \DIFdel{combine it}\DIFdelend \DIFaddbegin \DIFadd{likewise include}\DIFaddend ; predictions inherit a spectral-shape uncertainty of order
the distance envelope at the operating point's strain and rapidity,
\DIFdelbegin \DIFdel{stated here rather than discovered later}\DIFdelend \DIFaddbegin \DIFadd{a falsifiable model prediction}\DIFaddend .
...
two, supported on the plane normal to $\boldsymbol{\kappa}$. \DIFdelbegin \DIFdel{Under A1
(}\DIFdelend \DIFaddbegin \DIFadd{Upon invoking }\DIFaddend axisymmetry about $\boldsymbol{e}_2$, \DIFdelbegin \DIFdel{mirror-symmetric, helicity-free) }\DIFdelend \DIFaddbegin \DIFadd{mirror symmetry, and zero helicity, here denoted Assumption~A1, }\DIFaddend it is fixed by two scalar functions of $(\kappa_2,\kappa_\perp)$
